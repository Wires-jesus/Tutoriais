CREATE OR REPLACE FUNCTION NFE_RODAPE_ENTRADA(P_TRANSACAO NUMBER)
  RETURN TABELA_NFE_RODAPE IS
  CURSOR CR_RODAPE IS
    SELECT *
    FROM   SQL_NFE_RODAPE_ENTRADA RODAPE
    WHERE  RODAPE.NUM_TRANSACAO = P_TRANSACAO;
  CURSOR CR_RODAPE_COMPL IS
    SELECT *
    FROM   SQL_NFE_RODAPE_ENTRADA_COMP RODAPE
    WHERE  RODAPE.NUM_TRANSACAO = P_TRANSACAO;
...
BEGIN
  ...
  LOOP
    ...
    RETORNO(RETORNO.COUNT).NVOL                  := RODAPE.NVOL;
    RETORNO(RETORNO.COUNT).INFOADICIONALFISCO    := RODAPE.INFORMACAOADICIONALFISCO;
    RETORNO(RETORNO.COUNT).NUM_VOLUME_EMB        := RODAPE.NUM_VOLUME_EMB;
    RETORNO(RETORNO.COUNT).DESC_PL_PAGTO         := NULL;
    RETORNO(RETORNO.COUNT).VLTOTALIMPOSTOS       := 0;
    RETORNO(RETORNO.COUNT).VALOR_TOTAL_SEGURO    := RODAPE.VALOR_SEGURO;
    ...
    RETORNO(RETORNO.COUNT).VIBSESTCRED                 := RODAPE.VIBSESTCRED;
    RETORNO(RETORNO.COUNT).VCBSESTCRED                 := RODAPE.VCBSESTCRED;
    RETORNO(RETORNO.COUNT).VDIFIBSUF                   := RODAPE.VDIFIBSUF;
    RETORNO(RETORNO.COUNT).VDIFIBSMUN                  := RODAPE.VDIFIBSMUN;
    RETORNO(RETORNO.COUNT).VDIFCBS                     := RODAPE.VDIFCBS;
    RETORNO(RETORNO.COUNT).VDEVTRIBIBSMUN              := RODAPE.VDEVTRIBIBSMUN;
    RETORNO(RETORNO.COUNT).VDEVTRIBIBSUF               := RODAPE.VDEVTRIBIBSUF;
    RETORNO(RETORNO.COUNT).VDEVTRIBCBS                 := RODAPE.VDEVTRIBCBS;
    RETORNO(RETORNO.COUNT).VCBSCREDPRES                := RODAPE.VCBSCREDPRES;
    RETORNO(RETORNO.COUNT).VCBSCREDPRESCONDSUS         := RODAPE.VCBSCREDPRESCONDSUS;
    RETORNO(RETORNO.COUNT).VIBSCREDPRES                := RODAPE.VIBSCREDPRES;
    RETORNO(RETORNO.COUNT).VIBSCREDPRESCONDSUS         := RODAPE.VIBSCREDPRESCONDSUS;
  END LOOP;
  IF RETORNO.COUNT = 0 THEN
    ...
    LOOP
      ...
      RETORNO(RETORNO.COUNT).FINALIDADENFE         := RODAPE.FINALIDADENFE;
      RETORNO(RETORNO.COUNT).MODALIDADE_FRETE      := CASE WHEN NVL(RODAPE.FINALIDADENFE, 'X') = 'A' THEN 9 ELSE 0 END;
      ...
      RETORNO(RETORNO.COUNT).PLACA                 := NULL;
      ...
      RETORNO(RETORNO.COUNT).VALOR_TOTAL           := RODAPE.VALOR_TOTAL;
      RETORNO(RETORNO.COUNT).VALOR_TOTAL_PRODUTOS  := RODAPE.VALOR_TOTAL_PRODUTOS;
      RETORNO(RETORNO.COUNT).NVOL                  := NULL;
      ...
      RETORNO(RETORNO.COUNT).VIBSESTCRED                 := RODAPE.VIBSESTCRED;
      RETORNO(RETORNO.COUNT).VCBSESTCRED                 := RODAPE.VCBSESTCRED;
      RETORNO(RETORNO.COUNT).VDEVTRIBIBSMUN              := 0;
      RETORNO(RETORNO.COUNT).VDEVTRIBIBSUF               := 0;
      RETORNO(RETORNO.COUNT).VDEVTRIBCBS                 := 0;
      RETORNO(RETORNO.COUNT).VCBSCREDPRES                := 0;
      RETORNO(RETORNO.COUNT).VCBSCREDPRESCONDSUS         := 0;
      RETORNO(RETORNO.COUNT).VIBSCREDPRES                := 0;
      RETORNO(RETORNO.COUNT).VIBSCREDPRESCONDSUS         := 0;      
    END LOOP;
  END IF;
  RETURN RETORNO;
EXCEPTION
