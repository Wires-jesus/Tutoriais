...
BEGIN
  FOR REG IN (SELECT 1 FROM PCMOV WHERE NUMTRANSVENDA = P_TRANSACAO AND TRIM(DFEREF_CHAVEACESSO) IS NOT NULL UNION
              SELECT 1 FROM PCMOVPREFAT WHERE NUMTRANSVENDA = P_TRANSACAO AND TRIM(DFEREF_CHAVEACESSO) IS NOT NULL)
  LOOP
