...
BEGIN
   ...
   LOOP
      ...
      BEGIN
          SELECT INICIOINTERVALOPROMOCAOMED,PCPROMOCAOMED.TIPOPROMOCAO,PCPROMOCAOMED.TIPOPOLITICA,NVL(PCDESCONTO.QTCOMBOMED,0)
          ...
          IF ((vvTIPOPROMOCAOMED = 'K') AND 
            (vvTIPOLITICAMED = 'Q') AND 
            (vnQTDECOMBOMED > 1)) THEN
            IF (MOD((CR.QT/vnQTDECOMBOMED) , vnINICIOINTERVALOPROMOCAOMED) > 0) THEN
              vnQTDECOMBO := 0;
              EXIT;
            END IF;    
            ...
            IF vnQTDECOMBO = 0 THEN
              vnQTDECOMBO := (CR.QT/vnQTDECOMBOMED) / vnINICIOINTERVALOPROMOCAOMED;
            ELSE
  
              IF  (vnQTDECOMBO <> ((CR.QT/vnQTDECOMBOMED) / vnINICIOINTERVALOPROMOCAOMED)) THEN
                 vnQTDECOMBO := 0;
                 EXIT;
              END IF;
            END IF;
