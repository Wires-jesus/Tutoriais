  ...
  ,P_MOV IN VARCHAR2 DEFAULT 'S'
   ...
   CURSOR CR_IDENTIFICACAO IS
      SELECT PCNFBASE.CODFISCAL AS CFOP
            ...
            ,DECODE(NVL(PARAMFILIAL.OBTERCOMOVARCHAR2('FIL_OPTANTESIMPLESNAC', PCFILIAL.CODIGO), 'N'), 'S', '1', 'N', '3', 'E', '2') CRT
            ,PCNFSAID.TPENTEGOV
            ,NVL(PCNFSAID.PREDUTOR, 0) AS PREDUTOR
            ,NVL(PCNFSAID.VLIBSMUN, 0) AS VLIBSMUN
            ,NVL(PCNFSAID.VLIBSUF, 0) AS VLIBSUF
            ,NVL(PCNFSAID.VLCBS, 0) AS VLCBS
            ,(SELECT CASE WHEN (MAX(PCPREST.DTVENC) >= PCNFSAID.DTSAIDA) THEN '1'
                          WHEN ((MAX(PCPREST.DTPAG) IS NOT NULL) AND (MAX(PCPREST.DTPAG) < PCNFSAID.DTSAIDA)) THEN '3'
                          ELSE '1' END FROM PCPREST WHERE PCPREST.NUMTRANSVENDA = P_TRANSACAO) AS TPOPERGOV
      FROM   PCNFSAID
            ,PCFILIAL
            ,PCNFBASE
            ,PCCFO
      ...
      SELECT PCNFBASE.CODFISCAL AS CFOP
            ...
            ,DECODE(NVL(PARAMFILIAL.OBTERCOMOVARCHAR2('FIL_OPTANTESIMPLESNAC', PCFILIAL.CODIGO), 'N'), 'S', '1', 'N', '3', 'E', '2') CRT
            ,PCNFENT.TPENTEGOV
            ,NVL(PCNFENT.PREDUTOR, 0) AS PREDUTOR
            ,NVL(PCNFENT.VLIBSMUN, 0) AS VLIBSMUN
            ,NVL(PCNFENT.VLIBSUF, 0) AS VLIBSUF
            ,NVL(PCNFENT.VLCBS, 0) AS VLCBS         
            ,'1' AS TPOPERGOV   
      FROM   PCNFENT
            ,PCFILIAL
            ,PCNFBASE
...
BEGIN
   ...
   --PROCESSO NORMAL
     ...
     BEGIN
      SELECT CASE WHEN ((PCNFSAID.TIPOSERVICOCTE = 1) AND (NVL(PCNFSAID.CODCIDADECOLETA, 0) > 0)) THEN
     ...
     BEGIN
       SELECT NVL(PCNFSAID.CODMUNFIMCTE, CODIGO_MUNICIPIO) AS CODIGO_MUNICIPIO_FIM,
              NVL(FIM.NOMECIDADE, NOME_MUNICIPIO) AS NOME_MUNICIPIO_FIM,
              NVL(FIM.UF, SIGLA_UF) AS SIGLA_UF_FIM
       INTO   VCODIGO_MUNICIPIO_FIM,
              VNOME_MUNICIPIO_FIM,
              VSIGLA_UF_FIM
       FROM PCNFSAID,
            PCCIDADE FIM,
            TABLE(CAST(CTE_DESTINATARIO(NUMTRANSACAO_TEMP) AS TABELA_CTE_DESTINATARIO))
      WHERE PCNFSAID.NUMTRANSVENDA = NUMTRANSACAO_TEMP
        AND PCNFSAID.CODMUNFIMCTE = FIM.CODIBGE(+);
     EXCEPTION
           WHEN OTHERS THEN
   ...
   FOR IDENTIFICACAO IN CR_IDENTIFICACAO LOOP
      ...
      RETORNO(RETORNO.COUNT).CRT                         := IDENTIFICACAO.CRT;
      RETORNO(RETORNO.COUNT).TPENTEGOV                   := IDENTIFICACAO.TPENTEGOV;
      RETORNO(RETORNO.COUNT).PREDUTOR                    := IDENTIFICACAO.PREDUTOR;
      RETORNO(RETORNO.COUNT).VLIBSMUN                    := IDENTIFICACAO.VLIBSMUN;
      RETORNO(RETORNO.COUNT).VLIBSUF                     := IDENTIFICACAO.VLIBSUF;
      RETORNO(RETORNO.COUNT).VLCBS                       := IDENTIFICACAO.VLCBS;
      RETORNO(RETORNO.COUNT).TPOPERGOV                   := IDENTIFICACAO.TPOPERGOV;
