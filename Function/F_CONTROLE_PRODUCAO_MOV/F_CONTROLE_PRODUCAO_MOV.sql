...
---------------------------------------------------------------------------------
 ...
 return TABELA_CONTROLE_PRODUCAO
  ...
  OUTROW TIPO_CONTROLE_PRODUCAO := TIPO_CONTROLE_PRODUCAO(NULL,
                                                          NULL,
                                                          NULL,
                                                          NULL,
                                                          NULL,
...
BEGIN
  ...
  FOR DADOS IN (SELECT 'E' TIPO,
                 ...
                 WHERE PCMOV.NUMNOTA = PCNFENT.NUMNOTA
                   ...
                   AND NVL(PCNFENT.CODFILIALNF, PCNFENT.CODFILIAL) =
                       PCODFILIAL
                   ...
                   AND PCMOV.QTCONT > 0
                   AND DECODE(PGERA_NF_ENTRADA_CANC,
                              'N',
                              PCMOV.DTCANCEL,
                              NULL) IS NULL
                   ...
                   AND (CASE
                         ...
                         WHEN PSTATUSPROD = 'A' AND
                              (PCPRODUT.DTEXCLUSAO IS NULL OR
  ...
  FOR DADOS IN (SELECT TAB.TIPO,
                  ...
                  FROM (SELECT 'EC' TIPO, --- NF ENTRADA CANCELADA
                               ...
                               CASE
                                 ...
                                 ELSE
                                  (SELECT NVL(C.CUSTOULTENTCONT,
                                              ROUND(((((GREATEST(NVL(V.QTESTANT,
                                                                     0),
                                                                 0) +
                                                    V.QTCONT) *
                                                    NVL(V.CUSTOCONT, 0)) -
                                                    (GREATEST(NVL(V.QTESTANT,
                                                                    0),
                                                                0) *
                                                    NVL(V.CUSTOCONTANT, 0))) /
                                                    V.QTCONT),
                                                    6))
                              ...
                              ,
                               ...
                               PCMOV.ROTINACAD,
                               (NVL(PCMOV.VLFRETECONHEC, 0) -
                               (DECODE(NVL(PCMOVCOMPLE.VLICMSCUSTOFRETEFOB,
                                            0),
                                        0,
                                        NVL(PCMOVCOMPLE.VLICMSFRETEFOB, 0),
                                        NVL(PCMOVCOMPLE.VLICMSCUSTOFRETEFOB, 0)) +
                               ...
                               NVL(ROUND(((NVL(PCMOVCOMPLE.PTABELACUSTO,
                                         ...
                                         (NVL(PCMOV.VLFRETECONHEC, 0) -
                                         (NVL(VLICMSCUSTOFRETEFOB, 0) +
                                         NVL(PCMOVCOMPLE.VLCREDPISFRETEFOB,
                                                0) + NVL(PCMOVCOMPLE.VLCREDCOFINSFRETEFOB,
                                                            0)))),
                                         6),
                                   0) CUSTO_SEM_IPI_PIS_COFINS,
                               PCMOV.ROWID ID_PCMOV -- 63
                              ...
                              ,
                               NVL(PCMOV.VLCREDICMS,
                                   NVL(PCMOVCOMPLE.VLICMS, 0)) VLR_CRED_ICMS,
                               NVL(PCMOVCOMPLE.CUSTOULTENTCONT, 0) CUSTOULTENTCONT,
                               NVL(PCMOV.NUMSEQ, 1) NUMSEQ,
                               (NVL(PCMOV.BASEICMS, 0) *
                               NVL(PCMOV.PERCICM, 0) / 100) VLR_ICMS_REAL,
                               NVL(PCMOV.VLCREDCOFINS, PCMOV.VLCOFINS) VLR_CRED_COFINS,
                               NVL(PCMOV.VLCREDPIS, PCMOV.VLPIS) VLR_CRED_PIS,
                               NVL(PCMOV.NBM, PCPRODUT.NBM) AS NCM,
  ...
  IF NOT PSOMENTEENTRADAS = 'S' THEN
    FOR DADOS IN (SELECT 'S' TIPO,
                   ...
                   WHERE PCMOV.NUMNOTA = PCNFSAID.NUMNOTA
                     ...
                     AND DECODE(PGERA_NF_SAIDA_CANC,
                                'N',
                                PCMOV.DTCANCEL,
                                NULL) IS NULL
                     ...
                     AND ((PDESCONSIDERANFEDENEGADA = 'N') OR
                         ((PDESCONSIDERANFEDENEGADA = 'S') AND
                         NVL(PCNFSAID.SITUACAONFE, '0') NOT IN
                         (110, 205, 301, 302)))
                     AND PCMOV.CODPROD BETWEEN PCODPROD1 AND PCODPROD2
                     AND CASE
  ...
  FOR DADOS IN (SELECT 'AE' TIPO,
                       ...
                       DECODE(SUBSTR(PCMOV.CODOPER, 1, 1),
                              'E',
                              0,
                              PCMOV.PUNITCONT) VALORITEMNOTA_SAID,
                       DECODE(SUBSTR(PCMOV.CODOPER, 1, 1),
                              'E',
                              PCMOV.NUMTRANSENT,
                              0) NUMTRANSENT,
                       DECODE(SUBSTR(PCMOV.CODOPER, 1, 1),
                              'E',
                              0,
                              PCMOV.NUMTRANSVENDA) NUMTRANSVENDA,
                       NVL(PCMOV.VLIPI, 0) VLIPI,
                       NVL(PCMOV.ST, 0) ST,
                       NVL(PCMOV.VLDESPADICIONAL, 0) STGUIA,
                       PCMOV.DTCANCEL,
  ...
  FOR DADOS IN (SELECT 'AC' TIPO, ---- Entrada produção cancelada
                 ...
                 WHERE PCMOV.NUMTRANSITEM = PCMOVCOMPLE.NUMTRANSITEM(+)
                      --AND PCMOV.DTMOV between PDTINICIO AND PDTFIM
                   AND PCMOV.DTCANCEL between PDTINICIO AND
                       V_DATA_CANCELAMENTO_FIM
                   AND PCMOV.QTCONT < 0
                   AND NVL(PCMOV.CODFILIALNF, PCMOV.CODFILIAL) = PCODFILIAL
                   AND PCMOV.CODPROD = PCPRODUT.CODPROD
  ...
  IF NOT PSOMENTEENTRADAS = 'S' THEN
    FOR DADOS IN (SELECT 'SM' TIPO,
                   ...
                   WHERE PCMOV.NUMTRANSITEM = PCMOVCOMPLE.NUMTRANSITEM(+)
                     ...
                     AND PCMOV.CODPROD = PCPRODUT.CODPROD
                     AND PCMOV.QTCONT > 0
                     AND DECODE(PGERA_SM_KARDEX_CANC,
                                'N',
                                PCMOV.DTCANCEL,
                                NULL) IS NULL
                     AND PCMOV.STATUS IN ('A', 'AB')
