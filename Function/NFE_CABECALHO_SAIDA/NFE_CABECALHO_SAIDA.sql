CREATE OR REPLACE FUNCTION NFE_CABECALHO_SAIDA(P_TIPO                 NUMBER DEFAULT 1,
  ...
  CURSOR CR_ENDERECO_RETIRA(PC_TRANSACAO NUMBER) IS
    SELECT EMITENTE.CGC AS CNPJ_CPF_RET
          ,EMITENTE.FORNECEDOR AS RAZAO_SOCIAL_RET
          ,EMITENTE.FANTASIA AS NOME_FANTASIA_RET
          ,EMITENTE.ENDER AS LOGRADOURO_RET
          ,NVL(EMITENTE.NUMEROEND, 'S\N') AS NUMERO_RET
          ,EMITENTE.COMPLEMENTOEND AS COMPLEMENTO_RET
          ,EMITENTE.BAIRRO AS BAIRRO_RET
          ,NVL(CIDADE_RET.CODIBGE, 0) AS CODIGO_MUNICIPIO_RET
          ,CIDADE_RET.NOMECIDADE AS NOME_MUNICIPIO_RET
          ,UF_RET.UF AS SIGLA_UF_RET
          ,NVL(PAIS_RET.CODPAIS, 0) AS CODIGO_PAIS_RET
          ,PAIS_RET.DESCRICAO AS PAIS_RET
          ,EMITENTE.CEP AS CEP_RET
          ,EMITENTE.TELFAB AS TELEFONE_RET
          ,EMITENTE.EMAIL AS EMAIL_RET
          ,EMITENTE.IE AS INSCRICAO_ESTADUAL_RET    
      FROM PCFILIAL FI
          --ENDERECO RETIRA
          ,PCFORNEC EMITENTE
          ,PCPAIS PAIS_RET
          ,PCCIDADE CIDADE_RET
          ,PCESTADO UF_RET
     WHERE FI.CODIGO IN (SELECT M.CODFILIALRETIRA FROM PCMOV M WHERE M.NUMTRANSVENDA = PC_TRANSACAO AND M.CODFILIALRETIRA IS NOT NULL GROUP BY M.CODFILIALRETIRA UNION
                         SELECT M.CODFILIALRETIRA FROM PCMOVPREFAT M WHERE M.NUMTRANSVENDA = PC_TRANSACAO AND M.CODFILIALRETIRA IS NOT NULL GROUP BY M.CODFILIALRETIRA)
       --ENDERECO RETIRA
       AND FI.CODFORNEC = EMITENTE.CODFORNEC
       AND EMITENTE.CODCIDADE = CIDADE_RET.CODCIDADE(+)
      ...
      SELECT DISTINCT NUMTRANSVENDA,
        ...
        FROM (SELECT PCNFSAID.NUMTRANSVENDA,
              ...
              SELECT PCNFSAIDPREFAT.NUMTRANSVENDA,
               ...
               WHERE ((PCNFSAIDPREFAT.ESPECIE = 'NE') OR
               ...
               ORDER BY NOTA.INDVAREJO, NOTA.TIPOEMISSAO, NOTA.ORDEM, NOTA.NUMTRANSVENDA
               );
      ...
      SELECT NUMTRANSVENDA, 
        ...
        FROM (SELECT DISTINCT NUMTRANSVENDA,
                ...
                FROM (SELECT PCNFSAIDPREFAT.NUMTRANSVENDA,
                             ...
                             NVL(PCNFSAIDPREFAT.INDVAREJO, 2) INDVAREJO
                        FROM PCNFSAIDPREFAT, PCFILIAL
                       WHERE ((PCNFSAIDPREFAT.ESPECIE = 'NF') AND
                             (NVL(PCNFSAIDPREFAT.USASRVTERCEIROS,
  ...
  LOOP
    ...
    FOR FILIAIS IN (SELECT VALOR
      ...
      FOR CABECALHO IN CR_CABECALHOS_DANFE(NOVO_NUMERO, FILIAIS.VALOR) LOOP
        ...
        RETORNO(RETORNO.COUNT)                        := TIPO_NFE_CABECALHO();                                                     
        ...
        RETORNO(RETORNO.COUNT).DESTAQUEENDRETIRADANFE  := 'N';
        IF (CABECALHO.TEMFILIALRETIRA = 'S') THEN
          RETORNO(RETORNO.COUNT).DESTAQUEENDRETIRADANFE   := CABECALHO.DESTAQUEENDRETIRADANFE;
          FOR ENDERECO_RETIRA IN CR_ENDERECO_RETIRA(CABECALHO.NUM_TRANSACAO) LOOP
            RETORNO(RETORNO.COUNT).CNPJ_CPF_RET           := ENDERECO_RETIRA.CNPJ_CPF_RET;
            RETORNO(RETORNO.COUNT).LOGRADOURO_RET         := ENDERECO_RETIRA.LOGRADOURO_RET;
            RETORNO(RETORNO.COUNT).NUMERO_RET             := ENDERECO_RETIRA.NUMERO_RET;
            RETORNO(RETORNO.COUNT).COMPLEMENTO_RET        := ENDERECO_RETIRA.COMPLEMENTO_RET;
            RETORNO(RETORNO.COUNT).BAIRRO_RET             := ENDERECO_RETIRA.BAIRRO_RET;
            RETORNO(RETORNO.COUNT).CODIGO_MUNICIPIO_RET   := ENDERECO_RETIRA.CODIGO_MUNICIPIO_RET;
            RETORNO(RETORNO.COUNT).NOME_MUNICIPIO_RET     := ENDERECO_RETIRA.NOME_MUNICIPIO_RET;
            RETORNO(RETORNO.COUNT).SIGLA_UF_RET           := ENDERECO_RETIRA.SIGLA_UF_RET;
            RETORNO(RETORNO.COUNT).CEP_RET                := ENDERECO_RETIRA.CEP_RET;
            RETORNO(RETORNO.COUNT).INSCRICAO_ESTADUAL_RET := ENDERECO_RETIRA.INSCRICAO_ESTADUAL_RET;
            RETORNO(RETORNO.COUNT).TELEFONE_RET           := ENDERECO_RETIRA.TELEFONE_RET;
            RETORNO(RETORNO.COUNT).CODIGO_PAIS_RET        := ENDERECO_RETIRA.CODIGO_PAIS_RET;
            RETORNO(RETORNO.COUNT).RAZAO_SOCIAL_RET       := ENDERECO_RETIRA.RAZAO_SOCIAL_RET;
            RETORNO(RETORNO.COUNT).NOME_FANTASIA_RET      := ENDERECO_RETIRA.NOME_FANTASIA_RET;
            RETORNO(RETORNO.COUNT).EMAIL_RET              := ENDERECO_RETIRA.EMAIL_RET;
            RETORNO(RETORNO.COUNT).PAIS_RET               := ENDERECO_RETIRA.PAIS_RET;
          END LOOP;
        ELSE
          ...
          RETORNO(RETORNO.COUNT).NOME_FANTASIA_RET      := NULL;
        ...
        RETORNO(RETORNO.COUNT).CONSUMIUNUMNFE          := CABECALHO.CONSUMIUNUMNFE;
        ...
        RETORNO(RETORNO.COUNT).TIPONOTACREDDEB         := CABECALHO.TIPONOTACREDDEB;     
        RETORNO(RETORNO.COUNT).INDVAREJO               := CABECALHO.INDVAREJO;
        RETORNO(RETORNO.COUNT).TPOPERGOV               := CABECALHO.TPOPERGOV;
        RETORNO(RETORNO.COUNT).TPENTEGOV               := CABECALHO.TPENTEGOV;
        RETORNO(RETORNO.COUNT).PREDUTOR                := CABECALHO.PREDUTOR;
        RETORNO(RETORNO.COUNT).ESTORNODECREDITO        := CABECALHO.ESTORNODECREDITO;
		RETORNO(RETORNO.COUNT).OCULTAREMAILXMLNFE      := CABECALHO.OCULTAREMAILXMLNFE;
      END LOOP;
     END LOOP;
    END IF;
  END LOOP;
