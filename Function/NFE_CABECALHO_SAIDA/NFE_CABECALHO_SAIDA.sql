  ...
  LOOP
    ...
    FOR FILIAIS IN (SELECT VALOR
      ...
      FOR CABECALHO IN CR_CABECALHOS_DANFE(NOVO_NUMERO, FILIAIS.VALOR) LOOP
        ...
        IF ((CABECALHO.LOGRADOURO_D = CABECALHO.LOGRADOURO_ENT) 
          AND (CABECALHO.CODIGO_MUNICIPIO_D = CABECALHO.CODIGO_MUNICIPIO_ENT)) THEN
          RETORNO(RETORNO.COUNT).CNPJ_CPF_ENT           := '';
          RETORNO(RETORNO.COUNT).LOGRADOURO_ENT         := '';
          RETORNO(RETORNO.COUNT).NUMERO_ENT             := '';
          RETORNO(RETORNO.COUNT).COMPLEMENTO_ENT        := '';
          RETORNO(RETORNO.COUNT).BAIRRO_ENT             := '';
          RETORNO(RETORNO.COUNT).NOME_MUNICIPIO_ENT     := '';
          RETORNO(RETORNO.COUNT).CODIGO_MUNICIPIO_ENT   := '';
          RETORNO(RETORNO.COUNT).SIGLA_UF_ENT           := '';
          RETORNO(RETORNO.COUNT).CEP_ENT                 := '';
          RETORNO(RETORNO.COUNT).INSCRICAO_ESTADUAL_ENT  := '';
          RETORNO(RETORNO.COUNT).TELEFONE_ENT            := '';
          RETORNO(RETORNO.COUNT).CODIGO_PAIS_ENT         := '';
          RETORNO(RETORNO.COUNT).RAZAO_SOCIAL_REC_ENT    := '';
          RETORNO(RETORNO.COUNT).EMAIL_ENT               := '';
          RETORNO(RETORNO.COUNT).PAIS_ENT                := '';
        ELSE  
