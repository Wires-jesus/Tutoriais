        ...
        CASE WHEN NF.TIPOENVIO = 'N'
             AND EXISTS (select count(*), vnf.protocoloenvio, vnf.NROEMPRESA, vnf.NROCHECKOUT
                          ...
                          where vnf.protocoloenvio = nf.protocoloenvio
                            and vnf.NROEMPRESA = nf.NROEMPRESA
                            and vnf.NROCHECKOUT = nf.NROCHECKOUT
                            and vnf.DOCEMISSAO = nf.DOCEMISSAO
                          group by vnf.protocoloenvio, vnf.NROEMPRESA, vnf.NROCHECKOUT
                         having count(*) > 1)
                  THEN 'S'
             ELSE 'N'
...
(
  SELECT distinct
    ...
    ((I.VLRUNITARIO 
      - (NVL(I.VLRDESCONTO,0)/NVL(I.QUANTIDADE,1)) 
      + (NVL(I.VLRACRESCIMO,0)/NVL(I.QUANTIDADE,1)) 
      - (NVL(fnc_int_c5_vlrdeliveryrateio(i.nroempresa, i.nrocheckout, i.seqdocto, i.seqitem), 0)/NVL(I.QUANTIDADE,1))
     )/NVL(I.QTDEMBALAGEM, 1)) PVENDA,
    A.CODST,
    nvl(v.pcomrep1,0) PERCOM,
    A.ALIQICMS1,
    A.ALIQICMS2,
