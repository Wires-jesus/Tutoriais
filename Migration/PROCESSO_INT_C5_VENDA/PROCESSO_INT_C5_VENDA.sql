...
CREATE OR REPLACE   VIEW VW_INT_C5_FORMAPAGTO AS 
  (SELECT f.codfilial,
        f.codfinalizadora,
        f.descricao,
        f.especie,
        NVL(f.codcobintegracao,f.codcob) idref,
        NVL(f.codplpagintegracao,f.codplpag) codplpag,
        LEAST(p.numdias,999) nrodiasvencto,
        NVL(o.permitecontravale,'N') emitevaletroco,
        NVL(f.imprimevinculado,'N') emitecomprovante,
        NVL(o.cartao,'N') usatef,
        o.modalidadetef,
        o.tipooperacaotef,
        o.tipopagtoecf,
        LEAST(NVL(f.vlminimo,0), 999999) vlrminimo,
        LEAST(NVL(f.vlmaximo,0), 999999) vlrmaximo,
        NVL(f.valorminimoparcela,0) vlrminimoparcela,
        NVL(f.permitetroco,'N') permitetroco,
        NVL(pertxfim,0) pertxfim,
        NVL(prazo1,0) prazo1,
        NVL(prazo2,0) prazo2,
        NVL(prazo3,0) prazo3,
        NVL(prazo4,0) prazo4,
        ...
        NVL(prazo8,0) prazo8,
        NVL(prazo9,0) prazo9,
        NVL(prazo10,0) prazo10,
        NVL(prazo11,0) prazo11,
        NVL(prazo12,0) prazo12,
        p.numdias,
        f.dtalterc5,
        (CASE
             WHEN f.dtinativacao IS NULL
  ...
  FROM  pcfinalizadora f,
        ...
        (SELECT codplpag,
                ...
                dtalterc5
           FROM pcplpag
          WHERE NVL(usaplpagautoservico,'N') = 'S'
            AND codplpag > 0) p
 WHERE  f.codcob = o.codcob(+)
   AND  f.codplpag = p.codplpag(+)
   AND  f.codfinalizadora >= 0
   AND  NVL(f.usapdvsupermercado,'N') = 'S')
