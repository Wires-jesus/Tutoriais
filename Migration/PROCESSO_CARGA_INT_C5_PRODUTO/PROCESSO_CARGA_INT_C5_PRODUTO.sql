...
SELECT
  ...
  (CASE
       ...
       WHEN NVL(SUM(CESTA.PERCDESC), 0) > 0 THEN
            SUM(ROUND(CESTA.PRECO - (ABS(CESTA.PRECO * CESTA.PERCDESC / 100)), 3))
       ELSE SUM(CESTA.PRECO)
   END)PRECO
FROM (  
      SELECT 
         F.CODPRODACAB,
      ...
      AND   P.ATIVO = 'S'
      AND   P.CODPROD = F.CODPRODMP
      AND   F.CODFILIAL = PP.NROEMPRESA
      AND   E.CODAUXILIAR = F.CODAUXILIARMP
      AND   E.CODFILIAL   = F.CODFILIAL
      AND   E.CODPROD     = F.CODPRODMP
      AND   E.CODPROD     = P.CODPROD
      AND   E.QTUNIT      = PP.QTDEMBALAGEM
      AND   P.CODPROD = I.CODPRODMP(+)
      AND   I.CODPRECOCESTA = C.CODPRECOCESTA(+)
      AND   PP.NROEMPRESA = NVL(I.CODFILIAL, PP.NROEMPRESA)
      AND   F.CODPRODACAB = NVL(I.CODPRODACAB, F.CODPRODACAB)
      AND   F.CODFILIAL = NVL(I.CODFILIAL, F.CODFILIAL)
      AND   F.CODPRODMP = NVL(I.CODPRODMP, F.CODPRODMP)
      )CESTA,
      PCDEPARAPRODC5 C
WHERE CESTA.PRECO > 0  
