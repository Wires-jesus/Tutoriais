...
(
  ...
  WHERE 
    (
	  ...
	  AND E.QTUNIT = (SELECT MIN(QTUNIT) FROM VW_INT_C5_EMBPROD WHERE E.CODFILIAL = CODFILIAL AND E.CODPROD = CODPROD )
...
(
  ...
  WHERE 
    ...
    AND PCOFERTAPROGRAMADAC.PRIORIDADEOFERTA = (
      ...
      WHERE 
        ...
        AND SYSDATE BETWEEN
          TO_DATE(P.DTINICIAL || ' ' || NVL(TO_CHAR(P.HORAINICIAL, 'HH:MI:SS'), '00:00:01' ), 'DD-MM-YY HH24:MI:SS', 'NLS_DATE_LANGUAGE = AMERICAN')
          AND TO_DATE(P.DTFINAL || ' ' || NVL(TO_CHAR(P.HORAFINAL, 'HH:MI:SS'), '23:59:59' ), 'DD-MM-YY HH24:MI:SS', 'NLS_DATE_LANGUAGE = AMERICAN')
        AND I.VLOFERTA > 0
    )
 ...
 SELECT
    ...
    1 NROSEGMENTO,  
    PCOFERTAPROGRAMADAI.VLOFERTAATAC PRECO,
    'S' PROMOCAO,
    'S' ATIVO,
    decode(PCOFERTAPROGRAMADAC.HORAINICIAL, null, 0, 1) PRIORIDADE,
