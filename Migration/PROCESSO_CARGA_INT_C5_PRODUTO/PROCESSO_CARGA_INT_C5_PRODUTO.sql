...
(
SELECT
  --(SELECT X.SEQPRODUTO FROM PCDEPARAPRODC5 X WHERE X.CODAUXILIAR = CESTA.CODAUXILIAR_PRODACAB) SEQPRODCOMPOSTO,
...
FROM(/*SELECT BASE, SEM A VALIDAÇÃO COM A PRECOCESTAC E PRECOCESTA I QUE NEM SEMPRE ESTARÃO POPULADAS*/
     SELECT DISTINCT
       ...
       (CASE
         WHEN FERRAMENTAS.F_BUSCARPARAMETRO_ALFA('FIL_PRECOPOREMBALAGEM', F.CODFILIAL, 'N') = 'S' THEN
              COLUNA_PRECO(BUSCAPRECOS(F.CODFILIAL, 0, P.CODAUXILIAR, TRUNC(SYSDATE)),'PVENDA')
         ELSE COLUNA_PRECO(BUSCAPRECOS(F.CODFILIAL, FERRAMENTAS.F_BUSCARPARAMETRO_NUM('NUMREGIAOPADRAOVAREJO', F.CODFILIAL, 1), P.CODAUXILIAR, TRUNC(SYSDATE)),'PVENDA1')
       END) PRECO,
     ...
     --AND   P.CODAUXILIAR = E.codauxiliar
     --AND   P.CODAUXILIAR = F.CODAUXILIARMP
...
WHERE C.CODPRODACAB = I.CODPRODACAB(+)
AND  C.CODFILIAL = I.CODFILIAL(+)
AND  CESTA.CODPRODACAB = C.CODPRODACAB(+)
AND  CESTA.NROEMPRESA = C.CODFILIAL(+)
AND  CESTA.CODPRODACAB = NVL(I.CODPRODACAB, CESTA.CODPRODACAB)
AND  CESTA.CODAUXILIAR = NVL(I.CODPRODMP, CESTA.CODAUXILIAR)
