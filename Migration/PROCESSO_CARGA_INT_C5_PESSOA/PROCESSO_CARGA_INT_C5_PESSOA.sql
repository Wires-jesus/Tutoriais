...
(
  ...
  LEFT JOIN (SELECT DISTINCT MIN(SEQPESSOA) SEQPESSOA , UPPER(PKG_CNPJ_VALIDATOR.limpar_cnpj(CNPJCPF)) CNPJCPF, 
                    MAX(DTAHORALTERACAO) DTAHORALTERACAO 
               FROM MONITORPDVMIDDLE.TB_PESSOA 
              WHERE ATIVO = 'S' 
              GROUP BY UPPER(PKG_CNPJ_VALIDATOR.limpar_cnpj(CNPJCPF)) ) T 
  ON (UPPER(PKG_CNPJ_VALIDATOR.limpar_cnpj(R.CPF)) = T.CNPJCPF),
...
SELECT 
  G.CODGRUPOFIDELIDADE SEQGRUPOPESSOA,
  G.GRUPOFIDELIDADE DESCRICAO,
  'S' ATIVO
  FROM PCGRUPOFIDELIDADE G,
 (SELECT MIN(S.ULTIMAEXECUCAO) ULTIMAEXECUCAO
      FROM PCCONTROLECONSINCO S
      WHERE (UPPER(S.OBJETOREFERENCIA) = 'PKG_SINC_PDV_CONSINCO.CARREGA_TB_GRUPOPESSOA')
	  or (upper(s.objetoreferencia) =    'PKG_SINC_PDV_CONSINCO.CARREGA_TB_PESSOAGRUPO') ) DATAPADRAO
  WHERE NVL(G.DTALTERC5, DATAPADRAO.ULTIMAEXECUCAO) >= DATAPADRAO.ULTIMAEXECUCAO
)
...
SELECT 
  C.CODGRUPOFIDELIDADE SEQGRUPOPESSOA,
  C.CODCLI SEQPESSOA,
  'S' ATIVO
  FROM PCGRUPOFIDELIDADECLIENTE C,
 (SELECT MIN(S.ULTIMAEXECUCAO) ULTIMAEXECUCAO
      FROM PCCONTROLECONSINCO S
      WHERE (UPPER(S.OBJETOREFERENCIA) = 'PKG_SINC_PDV_CONSINCO.CARREGA_TB_GRUPOPESSOA')
	  or (upper(s.objetoreferencia) =    'PKG_SINC_PDV_CONSINCO.CARREGA_TB_PESSOAGRUPO') ) DATAPADRAO
  WHERE  NVL(C.DTALTERC5, DATAPADRAO.ULTIMAEXECUCAO) >= DATAPADRAO.ULTIMAEXECUCAO
)
