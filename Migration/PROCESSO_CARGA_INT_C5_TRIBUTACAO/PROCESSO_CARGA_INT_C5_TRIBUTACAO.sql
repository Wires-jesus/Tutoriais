...
SELECT DISTINCT
  ...
  T.CODOBSERVACAO||(SELECT TO_NUMBER(CODIBGE) FROM PCESTADO WHERE UF = T.UFORIGEM) SEQOBSSPED, --TABEXCECAO.SEQOBSSPED,
  E.CODCADASTROPRINC,
  E.CODCADASTROEXCECAO CODAJUSTEEFD,
  T.UFORIGEM UF,--TABEXCECAO.UFORIGEM UF,
  E.TIPO1,
  E.VALOR1,
  'S' ATIVO,
...
FROM
  (
    ...
    WHERE T.IDREF IS NOT NULL
    AND   T.IDREF = F.CODCADASTROPRINC
    AND   F.ROTINA = 'PCSIS4008'
--    GROUP BY T.CODOBSERVACAO, T.UFORIGEM
  )TABEXCECAO,
...
AND   R.CODPROD = FAM.SEQFAMILIA
AND   P.CODPROD = FAM.SEQFAMILIA
AND   E.CODEXCECAO = TABEXCECAO.CODEXCECAO
AND   T.NROTRIBUTACAO = R.CODST
AND   T.IDREF = E.CODCADASTROPRINC
AND   ((E.TIPO1 = 'FT' AND E.VALOR1 = R.CODST) OR
       (E.TIPO1 = 'CM' AND E.VALOR1 = P.NBM) OR
       (E.TIPO1 = 'PR' AND E.VALOR1 = P.CODPROD)
      )
)