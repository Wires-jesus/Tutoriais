...
(
     ...
     (SELECT DISTINCT
      ...
      FROM MONITORPDVMIDDLE.TB_FAMILIA TF,
           ...
           (SELECT PR.CODPROD, PR.CODST, PR.NUMREGIAO
            ...
            WHERE NVL(PR.DTALTERC5, DTPADRAO.ULTIMAEXECUCAO) >= DTPADRAO.ULTIMAEXECUCAO -720/24/60
      ...
      FROM MONITORPDVMIDDLE.TB_FAMILIA TF,
           ...
           (SELECT TABTRIB.CODPROD, TABTRIB.CODST, TABTRIB.UFDESTINO, TABTRIB.CODFILIALNF
            FROM PCTABTRIB TABTRIB,
                 (select s.ultimaexecucao from pccontroleconsinco s where upper(s.objetoreferencia) = 'PKG_SINC_PDV_CONSINCO.CARREGA_TB_FAMDIVISAO') DTPADRAO
            WHERE NVL(TABTRIB.DTALTERC5, DTPADRAO.ULTIMAEXECUCAO) >= DTPADRAO.ULTIMAEXECUCAO -720/24/60
            UNION
 ...
 FROM
     (SELECT DISTINCT
      ...
      AND   R.NUMREGIAO = D.NUMREGIAO
      AND   C.NUMREGIAO = D.NUMREGIAO
      AND   TRIB.CODST = F.nrotributacao
      AND   TRIB.CODST = r.CODST
      AND   TRIB.CODST = C.CODST
      AND   R.CODST IS NOT NULL
      ...
      AND   GREATEST(NVL(R.DTALTERC5, DTPADRAO.ULTIMAEXECUCAO),
                     NVL(TRIB.DTALTERC5, DTPADRAO.ULTIMAEXECUCAO)) >= DTPADRAO.ULTIMAEXECUCAO -1440/24/60
      GROUP BY T.SEQFAMILIA, R.CODPROD, R.CODST, D.NRODIVISAO, D.NUMREGIAO
      ...
      --AND   NVL(TB.DTALTERC5, DTPADRAO.ULTIMAEXECUCAO)  >= DTPADRAO.ULTIMAEXECUCAO
      AND   GREATEST(NVL(TB.DTALTERC5, DTPADRAO.ULTIMAEXECUCAO),
                     NVL(TRIB.DTALTERC5, DTPADRAO.ULTIMAEXECUCAO)) >= DTPADRAO.ULTIMAEXECUCAO -1440/24/60
      GROUP BY T.SEQFAMILIA, TB.CODPROD, TB.CODST, C5.CODFILIALINTEGRACAO
      )PRODTRIB */   
  )
