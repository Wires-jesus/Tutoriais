...
CREATE OR REPLACE VIEW VW_INT_C5_FAT_GIFTCARD AS
(  
  SELECT P.NROCREDITOCUPOM,
        D.DTAMOVIMENTO AS DATATRANSACAO,
        C5.CODFILIAL AS CODFILIAL,
        P.NROCHECKOUT AS NUMCAIXA,
        P.NROCHECKOUT,
        P.SEQDOCTO,
        P.NROEMPRESA,
        P.VALOR
  FROM MONITORPDVMIDDLE.TB_DOCTO D,
	 ...
	 FROM MONITORPDVMIDDLE.TB_DOCTOPAGTO P1,
		  MONITORPDVMIDDLE.TB_DOCTO D1
	 WHERE P1.SEQDOCTO = D1.SEQDOCTO
	 AND P1.NROEMPRESA = D1.NROEMPRESA
	 AND P1.NROCHECKOUT = D1.NROCHECKOUT
	 AND D1.ESPECIE = 'VC'
	 AND P1.NROCREDITOCUPOM IS NOT NULL
	 GROUP BY P1.SEQDOCTO,
		  P1.NROCHECKOUT,
		  P1.NROEMPRESA,
		  P1.NROCARTAO,
		  P1.NROCREDITOCUPOM) P,
      VW_INT_C5_OBTER_FILIAIS_C5 C5
  WHERE D.NROEMPRESA = P.NROEMPRESA
  AND D.NROCHECKOUT = P.NROCHECKOUT
  AND D.SEQDOCTO = P.SEQDOCTO
  AND D.ESPECIE = 'VC'
  AND C5.CODFILIALINTEGRACAO = D.NROEMPRESA
)