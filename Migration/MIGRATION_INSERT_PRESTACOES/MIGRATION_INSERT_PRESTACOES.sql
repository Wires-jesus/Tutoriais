DECLARE
  ...
  dNUMCHECKOUT     PCPREST.NUMCHECKOUT%TYPE;
  dVLTOTALPREST    PCPREST.VALOR%TYPE;
  dCODFUNCCHECKOUT PCPREST.CODFUNCCHECKOUT%TYPE;
  ...
  FUNCTION GET_NEWCODLOG 
         RETURN NUMBER IS
              dCODLOG PCLOGFATURAMENTO.CODLOG%TYPE;
              V_COUNT PLS_INTEGER;
  BEGIN
    SELECT DFSEQ_PCLOGFATURAMENTO.NEXTVAL
      INTO dCODLOG
    FROM DUAL;
    ...
    BEGIN
      SELECT 1
        INTO V_COUNT
      FROM PCLOGFATURAMENTO
      WHERE CODLOG = dCODLOG;
...
BEGIN
  FOR DADOS IN (
    SELECT
      ...
      ,NFSAID.NUMCAR
      ,NVL(NFSAID.CODEMITENTE,-1) CODEMITENTE
    ...
    WHERE 1=1
      ...
      AND NFSAID.DTSAIDA >= DATE '2025-10-01'
      AND NFSAID.SITUACAONFE = '100'
      AND NFSAID.CODCOB IS NOT NULL
      AND NFSAID.CODPLPAG IS NOT NULL
      AND NFSAID.CONDVENDA = 1
      AND NVL(NFSAID.GERACP,'N') = 'N') LOOP

    dNUMCHECKOUT     := 0;
    dCODFUNCCHECKOUT := 0;

    IF DADOS.CODEMITENTE >= 0 THEN
    BEGIN
        SELECT
           NVL(E.NUMCAIXABALCAO,0)
          ,DECODE(NVL(E.NUMCAIXABALCAO,0),0,
                  NULL,
                  E.MATRICULA)
         INTO
           dNUMCHECKOUT
          ,dCODFUNCCHECKOUT
        FROM PCEMPR E
        WHERE E.MATRICULA = DADOS.CODEMITENTE;
      EXCEPTION
      WHEN NO_DATA_FOUND THEN
        dNUMCHECKOUT     := 0;
        dCODFUNCCHECKOUT := 0;
    ...
    IF dNUMCHECKOUT > 0 THEN
      tPCPREST.NUMCHECKOUT     := dNUMCHECKOUT;
      tPCPREST.CODFUNCCHECKOUT := dCODFUNCCHECKOUT;
    END IF;
    ...
    tPCLOGFATURAMENTO.CODLOG              := GET_NEWCODLOG;
