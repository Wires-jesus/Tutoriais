...
WITH CTE_FAMDIVISAOCATEGORIA AS (
SELECT
    T.NRODIVISAO,
    F.SEQFAMILIA,
    T.SEQCATEGORIA,
      ...
      (SELECT --DISTINCT
       ...
       WHERE (((CODEPTO IS NOT NULL) AND (CODSEC IS NOT NULL) AND (CODCATEGORIA IS NOT NULL) AND (CODSUBCATEGORIA IS NOT NULL)) --SUBCATEGORIA
         ...
         AND (  NVL(P.DTALTERC5, DTPADRAO.ULTIMAEXECUCAO) >= DTPADRAO.ULTIMAEXECUCAO
          OR    NVL((select max(e.dtalterc5)
                       from pcembalagem e
                      where e.codprod = P.codprod), DTPADRAO.ULTIMAEXECUCAO) >= DTPADRAO.ULTIMAEXECUCAO )
      )PRODCATEGORIA,

      PCDEPARACATEGORIAC5 C
 WHERE PRODCATEGORIA.NUMREGIAO||TO_NUMBER(PRODCATEGORIA.SEQCATEGORIA) = TO_NUMBER(C.SEQCATEGORIAWINTHOR)
 AND   C.SEQCATEGORIAC5 =  T.SEQCATEGORIA
 AND   F.ATIVO = 'S'
 AND   PRODCATEGORIA.CODPROD = F.IDREF
 AND   T.NRODIVISAO = PRODCATEGORIA.NRODIVISAO/*VINCULAR POR REGIAO*/
) 
  SELECT  C.NRODIVISAO,
    C.SEQFAMILIA,
    C.SEQCATEGORIA,
    C.ATIVO,
    C.IDREF
  FROM CTE_FAMDIVISAOCATEGORIA C