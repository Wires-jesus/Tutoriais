...
SELECT ALIQUOTA_COFINS
     ...
     ,CASE WHEN NVL(FATOR_CONVERSAO_EX,0) > 0 THEN
      ...
      ,VALOR_FRETE
      ,VALOR_ICMS
      ,NVL(VLICMS_COMPLE, 0) AS VLICMS_COMPLE
     ...
     ,CASE
      ...
      ,CASE WHEN ((SITUACAO_TRIBUTARIA = '51') AND
       ...
       END VALORICMSDIF
      ,ALIQUOTAICMSDIF
      ,(CASE WHEN ((VLICMSDESONERACAO > 0) AND (ROUND(VLICMSDESONERACAO, 2) = 0)) THEN 0.01 ELSE ROUND(VLICMSDESONERACAO, 2) END) VALOR_ICMS_DESONERADO
...
FROM   (SELECT PCMOV.NUMTRANSVENDA AS NUM_TRANSACAO
              ...
              ,CASE WHEN (PCMOVCOMPLE.USAUNIDADEMASTER = 'S') THEN
                  CASE WHEN LENGTH(NVL(PCPRODUT.CODAUXILIAR2, '')) IN (NVL(PCPRODUT.GTINCODAUXILIAR2, 0)) THEN
              ...
              TRIM((
               ...
               ||' '||
                  DECODE(NVL(PCPRODUT.ENVIAINFTECNICANFE, 'S'),'S', PCPRODUT.INFORMACOESTECNICAS, NULL) ||
                    ...
                    ||
            CASE WHEN (PCMOVCOMPLE.NUMFCI IS NOT NULL) AND (NVL(PARAMFILIAL.OBTERCOMOVARCHAR2('NUMFCIDANFE',PCFILIAL.CODIGO),'N') = 'S') AND
                      (NVL(PCMOVCOMPLE.ORIGMERCTRIB,PCPRODFILIAL.ORIGMERCTRIB) IN ('3','5','8')) THEN
...
------------------------------------------------------------------------------------------------------------
           ...
           CASE WHEN (PCMOV.CODOPER = 'SD' AND
            ...
            ,CASE WHEN PCMOVCOMPLE.SOMARVALORDIFBCICMS = 'S' THEN 0 ELSE
                ...
                END
            END AS VALOR_ICMS
          ,ROUND(NVL(PCMOVCOMPLE.VLICMS, 0) * PCMOV.QTCONT , 2) AS VLICMS_COMPLE
              ,CASE WHEN (PCMOV.CODOPER ='SD' AND PARAMFILIAL.OBTERCOMOVARCHAR2('ENVIASIMPNACDEVFORNECNFE', PCFILIAL.CODIGO) = 'N') AND
                      PARAMFILIAL.OBTERCOMOVARCHAR2('FIL_OPTANTESIMPLESNAC', PCFILIAL.CODIGO) IN  ('E', 'S') THEN
                 0
           ELSE
                ...
                DECODE(NVL(PCMOVCOMPLE.USAPMCBASEST, 'N'),
                       ...
                       DECODE(NVL(PCMOV.TIPOTRIBUTMEDIC, 'OM'),
                              ...
                              CASE
                                ...
                                WHEN ((NVL(PCMOV.IVA, 0) + NVL(PCMOV.PERCIVA, 0)) = 0 AND NVL(PCMOV.BASEICST, 0) > 0 AND NVL(PCMOV.PAUTA, 0) = 0) THEN
                                    6
                                ELSE
                                    4
                              END))
           ...
           ,CASE  WHEN NVL(PCMOVCOMPLE.QTLITRAGEM, PCPRODUT.LITRAGEM) > 0
            ...
            ,CASE WHEN (PCMOV.CODOPER = 'SD') THEN
              CASE WHEN (PCMOVCOMPLE.CODMOTIVOICMSDESONERADO = '7') THEN
                 ...
                 WHEN (NVL(PCMOV.VLDESCICMISENCAO,0) > 0) THEN
                  (PCMOV.QTCONT * NVL(PCMOV.VLDESCICMISENCAO, 0))
                 WHEN (NVL(PCMOVCOMPLE.VLICMSDESONERACAO,0) > 0) THEN
                  (PCMOV.QTCONT * NVL(PCMOVCOMPLE.VLICMSDESONERACAO, 0))
              ELSE
                 0
              END
            ELSE
            ...
            ,CASE
               WHEN NVL(PARAMFILIAL.ObterComoVarchar2('PERMITEINFOLEITRANSPCONSFINAL', PCFILIAL.CODIGO),'N') = 'S'
                    AND NVL(PCCLIENT.CONSUMIDORFINAL,'N') = 'S'
               THEN
                   'Trib. Aprox: ' ||
                   'R$ '||TO_CHAR(NVL(PCMOVCOMPLE.VLITEMTRIBUTOS,0)*PCMOV.QTCONT,'FM999999999999990.90')||' - Fed., '||
                   'R$ '||TO_CHAR(NVL(PCMOVCOMPLE.VLITEMTRIBUTOSEST,0)*PCMOV.QTCONT,'FM999999999999990.90')||' - Est., '||
                   'R$ '||TO_CHAR(NVL(PCMOVCOMPLE.VLITEMTRIBUTOSMUNIC,0)*PCMOV.QTCONT,'FM999999999999990.90')||' - Munic. '||
                   'Total: R$ '||TO_CHAR((NVL(PCMOVCOMPLE.VLITEMTRIBUTOS,0)+NVL(PCMOVCOMPLE.VLITEMTRIBUTOSEST,0)+NVL(PCMOVCOMPLE.VLITEMTRIBUTOSMUNIC,0))*PCMOV.QTCONT,'FM999999999999990.90')
               ELSE
                   CASE
                     WHEN NVL(PARAMFILIAL.ObterComoVarchar2('PERMITEINFOLEITRANSPCONSFINAL', PCFILIAL.CODIGO),'N') = 'N'
                     THEN
                         'Trib. Aprox: ' ||
                         'R$ '||TO_CHAR(NVL(PCMOVCOMPLE.VLITEMTRIBUTOS,0)*PCMOV.QTCONT,'FM999999999999990.90')||' - Fed., '||
                         'R$ '||TO_CHAR(NVL(PCMOVCOMPLE.VLITEMTRIBUTOSEST,0)*PCMOV.QTCONT,'FM999999999999990.90')||' - Est., '||
            ...
            ,PCMOV.NUMTRANSITEM
            ,PCMOVCOMPLE.CCREDPRESUMIDO
            ,NVL(PCMOV.PERCCREDICMPRESUMIDO, 0) PERCCREDICMPRESUMIDO
            ,ROUND(PCMOV.QTCONT * NVL(PCMOV.VLCREDPRESUMIDO, 0),2) AS VLCREDPRESUMIDO
      ,PCMOV.CODIGOTRIBUTACAOIS
      ,PCMOV.CSTIS
      ,PCMOV.CCLASSTRIBIS
      ,ROUND(PCMOV.QTCONT * NVL(PCMOV.VLBASEIS, 0), 2) AS VLBASEIS
      ,NVL(PCMOV.ALIQIS, 0) AS ALIQIS
      ,NVL(PCMOV.ALIQESPECIFICAIS, 0) AS ALIQESPECIFICAIS
      ,ROUND(PCMOV.QTCONT * NVL(PCMOV.VLIS, 0), 2) AS VLIS
            ,PCMOV.CODIGOTRIBUTACAOCBSIBS
            ,PCMOV.CSTIBSCBS
            ,PCMOV.CCLASSTRIBIBSCBS
            ,ROUND(PCMOV.QTCONT * NVL(PCMOV.VLBASEIBSCBS, 0), 2) AS VLBASEIBSCBS
            ,NVL(PCMOV.IBSUF, 0) AS IBSUF
            ,NVL(PCMOV.PDIFIBSUF, 0) AS PDIFIBSUF
            ,ROUND(PCMOV.QTCONT * NVL(PCMOV.VDIFIBSUF, 0), 2) AS VDIFIBSUF
            ,NVL(PCMOV.PREDALIQIBSUF, 0) AS PREDALIQIBSUF
            ,NVL(PCMOV.PALIQEFETIBSUF, 0) AS PALIQEFETIBSUF
            ,ROUND(PCMOV.QTCONT * NVL(PCMOV.VIBSUF, 0), 2) AS VIBSUF
            ,NVL(PCMOV.PIBSMUN, 0) AS PIBSMUN
            ,NVL(PCMOV.PDIFIBSMUN, 0) AS PDIFIBSMUN
            ,ROUND(PCMOV.QTCONT * NVL(PCMOV.VDIFIBSMUN, 0), 2) AS VDIFIBSMUN
            ,NVL(PCMOV.PREDALIQIBSMUN, 0) AS PREDALIQIBSMUN
            ,NVL(PCMOV.PALIQEFETIBSMUN, 0) AS PALIQEFETIBSMUN
            ,ROUND(PCMOV.QTCONT * NVL(PCMOV.VIBSMUN, 0), 2) AS VIBSMUN
            ,NVL(PCMOV.ALIQCBS, 0) AS ALIQCBS
            ,NVL(PCMOV.PDIFCBS, 0) AS PDIFCBS
            ,ROUND(PCMOV.QTCONT * NVL(PCMOV.VDIFCBS, 0), 2) AS VDIFCBS
            ,NVL(PCMOV.PREDALIQCBS, 0) AS PREDALIQCBS
            ,NVL(PCMOV.PALIQEFETCBS, 0) AS PALIQEFETCBS
            ,ROUND(PCMOV.QTCONT * NVL(PCMOV.VLCBS, 0), 2) AS VLCBS
            ,PCMOV.CSTTRIBREG
            ,PCMOV.CCLASSTRIBREG
            ,NVL(PCMOV.PALIQEFETREGIBSUF, 0) AS PALIQEFETREGIBSUF
            ,ROUND(PCMOV.QTCONT * NVL(PCMOV.VTRIBREGIBSUF, 0), 2) AS VTRIBREGIBSUF
            ,NVL(PCMOV.ALIQEFETREGIBSMUN, 0) AS ALIQEFETREGIBSMUN
            ,ROUND(PCMOV.QTCONT * NVL(PCMOV.VTRIBREGIBSMUN, 0), 2) AS VTRIBREGIBSMUN
            ,NVL(PCMOV.PALIQEFETREGCBS, 0) AS PALIQEFETREGCBS
            ,ROUND(PCMOV.QTCONT * NVL(PCMOV.VTRIBREGCBS, 0), 2) AS VTRIBREGCBS
            ,NVL(PCMOV.PIBSUFCOMPRAGOV, 0) AS PIBSUFCOMPRAGOV
            ,ROUND(PCMOV.QTCONT * NVL(PCMOV.VIBSUFCOMPRAGOV, 0), 2) AS VIBSUFCOMPRAGOV
            ,NVL(PCMOV.PIBSMUNCOMPRAGOV, 0) AS PIBSMUNCOMPRAGOV
            ,ROUND(PCMOV.QTCONT * NVL(PCMOV.VIBSMUNCOMPRAGOV, 0), 2) AS VIBSMUNCOMPRAGOV
            ,NVL(PCMOV.PCBGCOMPRAGOV, 0) AS PCBGCOMPRAGOV
            ,ROUND(PCMOV.QTCONT * NVL(PCMOV.VCBSCOMPRAGOV, 0), 2) AS VCBSCOMPRAGOV
            ,NVL(PCMOV.SOMATOTALNF_CBS, 'N') AS SOMATOTALNF_CBS
            ,NVL(PCMOV.SOMATOTALNF_IBS, 'N') AS SOMATOTALNF_IBS
            ,NVL(PCMOV.SOMATOTALNF_IS, 'N') AS SOMATOTALNF_IS
            ,ROUND(PCMOV.QTCONT * NVL(PCMOV.VIBSESTCRED,0), 2) AS VIBSESTCRED
            ,ROUND(PCMOV.QTCONT * NVL(PCMOV.VCBSESTCRED,0), 2) AS VCBSESTCRED
            ,ROUND(PCMOV.QTCONT * NVL(PCMOV.VDEVTRIBIBSMUN,0), 2) AS VDEVTRIBIBSMUN
            ,ROUND(PCMOV.QTCONT * NVL(PCMOV.VDEVTRIBIBSUF,0), 2) AS VDEVTRIBIBSUF
            ,ROUND(PCMOV.QTCONT * NVL(PCMOV.VDEVTRIBCBS,0), 2) AS VDEVTRIBCBS
            ,PCMOV.CCREDPRES
            ,ROUND(PCMOV.QTCONT * NVL(PCMOV.VBCCREDPRES,0), 2) AS VBCCREDPRES
            ,NVL(PCMOV.PERCBSCREDPRES,0) AS PERCBSCREDPRES
            ,ROUND(PCMOV.QTCONT * NVL(PCMOV.VCBSCREDPRES,0), 2) AS VCBSCREDPRES
            ,ROUND(PCMOV.QTCONT * NVL(PCMOV.VCBSCREDPRESCONDSUS,0), 2) AS VCBSCREDPRESCONDSUS
            ,NVL(PCMOV.PERIBSCREDPRES,0) AS PERIBSCREDPRES
            ,ROUND(PCMOV.QTCONT * NVL(PCMOV.VIBSCREDPRES,0), 2) AS VIBSCREDPRES
            ,ROUND(PCMOV.QTCONT * NVL(PCMOV.VIBSCREDPRESCONDSUS,0), 2) AS VIBSCREDPRESCONDSUS           
            ,PCMOV.DFEREF_CHAVEACESSO
            ,PCMOV.DFEREF_NITEM
            ,PCMOV.CPFRESPTECNICOAGRICOLA
            ,PCMOV.NUMRECEITUARIOAGRICOLA
            ,PCMOVCOMPLE.CBENEFRBC
      FROM   PCMOV
            ,PCPRODUT
            ,PCMOVCOMPLE
            ,PCEMBALAGEM
              ...
              TRIM((
               ...
               ||' '||
                  DECODE(NVL(PCPRODUT.ENVIAINFTECNICANFE, 'S'),'S', PCPRODUT.INFORMACOESTECNICAS, NULL) ||
                    ...
                    ||
            CASE WHEN (PCMOVCOMPLEPREFAT.NUMFCI IS NOT NULL) AND (NVL(PARAMFILIAL.OBTERCOMOVARCHAR2('NUMFCIDANFE',PCFILIAL.CODIGO),'N') = 'S') AND
                      (NVL(PCMOVCOMPLEPREFAT.ORIGMERCTRIB,PCPRODFILIAL.ORIGMERCTRIB) IN ('3','5','8')) THEN
          ...
          , CASE WHEN PCMOVCOMPLEPREFAT.SOMARVALORDIFBCICMS = 'S' THEN 0 ELSE
              CASE WHEN (PCMOVPREFAT.CODOPER ='SD' AND PARAMFILIAL.OBTERCOMOVARCHAR2('ENVIASIMPNACDEVFORNECNFE', PCFILIAL.CODIGO) = 'N') AND
               ...
               ELSE
                ...
                END
            END AS VALOR_ICMS
              ,ROUND(NVL(PCMOVCOMPLEPREFAT.VLICMS, 0) * PCMOVPREFAT.QTCONT , 2) AS VLICMS_COMPLE
              ,CASE WHEN (PCMOVPREFAT.CODOPER ='SD' AND PARAMFILIAL.OBTERCOMOVARCHAR2('ENVIASIMPNACDEVFORNECNFE', PCFILIAL.CODIGO) = 'N') AND
                      PARAMFILIAL.OBTERCOMOVARCHAR2('FIL_OPTANTESIMPLESNAC', PCFILIAL.CODIGO) IN  ('E', 'S') THEN
                 0
           ELSE
           ...
           END AS VALOR_ICMS_OPERACAO
            ...
            ,NVL(PCMOVCOMPLEPREFAT.CODSITTRIBIPI,DECODE(NVL(DECODE(NVL(PCMOVPREFAT.CALCCREDIPI,
                    ...
                    'S',
                    50,
                    NVL((SELECT CODSITTRIBIPISAID FROM PCFIGURATRIBIPI F WHERE F.CODFIGURAIPI IN (SELECT CODFIGURAIPI FROM PCTRIBIPI WHERE CODPROD = PCMOVPREFAT.CODPROD AND NVL(PCMOVPREFAT.CODFILIALNF,PCMOVPREFAT.CODFILIAL) = PCTRIBIPI.CODFILIAL AND ROWNUM = 1) AND ROWNUM = 1), 99))) SITUACAO_TRIBUTARIA_IPI
                       ...
                       DECODE(NVL(PCMOVPREFAT.TIPOTRIBUTMEDIC, 'OM'),
                              ...
                              CASE
                                WHEN (ROUND(NVL(PCMOVPREFAT.PAUTA, 0) * (DECODE(NVL(PCMOVPREFAT.PERCBASEREDST,0), 0, 1, PCMOVPREFAT.PERCBASEREDST/100)), 6) = NVL(PCMOVPREFAT.BASEICST, 0)) THEN
                                    5
                                --Se IVA = 0 E BASE DE ST = BASE ICMS E SEM PAUTA E SEM LISTAS – GERAR TIPO 6
                                WHEN ((NVL(PCMOVPREFAT.IVA, 0) + NVL(PCMOVPREFAT.PERCIVA, 0)) = 0 AND NVL(PCMOVPREFAT.BASEICST, 0) > 0 AND NVL(PCMOVPREFAT.PAUTA, 0) = 0) THEN
                                    6
                                ELSE
                                    4
                              END))
           ...
           ,CASE  WHEN NVL(PCMOVCOMPLEPREFAT.QTLITRAGEM, PCPRODUT.LITRAGEM) > 0
            ...
            ,CASE WHEN (PCMOVPREFAT.CODOPER = 'SD') THEN
              CASE WHEN (PCMOVCOMPLEPREFAT.CODMOTIVOICMSDESONERADO = '7') THEN
               ...
               WHEN (NVL(PCMOVPREFAT.VLDESCICMISENCAO,0) > 0) THEN
                (PCMOVPREFAT.QTCONT * NVL(PCMOVPREFAT.VLDESCICMISENCAO, 0))
               WHEN (NVL(PCMOVCOMPLEPREFAT.VLICMSDESONERACAO,0) > 0) THEN
                  (PCMOVPREFAT.QTCONT * NVL(PCMOVCOMPLEPREFAT.VLICMSDESONERACAO, 0))
              ELSE
               0
              END
            ELSE
            ...
            ,CASE
               ...
               THEN
                   'Trib. Aprox: ' ||
                   'R$ '||TO_CHAR(NVL(PCMOVCOMPLEPREFAT.VLITEMTRIBUTOS,0)*PCMOVPREFAT.QTCONT,'FM999999999999990.90')||' - Fed., '||
                   'R$ '||TO_CHAR(NVL(PCMOVCOMPLEPREFAT.VLITEMTRIBUTOSEST,0)*PCMOVPREFAT.QTCONT,'FM999999999999990.90')||' - Est., '||
                   'R$ '||TO_CHAR(NVL(PCMOVCOMPLEPREFAT.VLITEMTRIBUTOSMUNIC,0)*PCMOVPREFAT.QTCONT,'FM999999999999990.90')||' - Munic. '||
                   'Total: R$ '||TO_CHAR((NVL(PCMOVCOMPLEPREFAT.VLITEMTRIBUTOS,0)+NVL(PCMOVCOMPLEPREFAT.VLITEMTRIBUTOSEST,0)+NVL(PCMOVCOMPLEPREFAT.VLITEMTRIBUTOSMUNIC,0))*PCMOVPREFAT.QTCONT,'FM999999999999990.90')
               ELSE
                   CASE
                     WHEN NVL(PARAMFILIAL.ObterComoVarchar2('PERMITEINFOLEITRANSPCONSFINAL', PCFILIAL.CODIGO),'N') = 'N'
                     THEN
            ...
            ,PCMOVCOMPLEPREFAT.CCREDPRESUMIDO
            ,NVL(PCMOVPREFAT.PERCCREDICMPRESUMIDO, 0) PERCCREDICMPRESUMIDO
            ,ROUND(PCMOVPREFAT.QTCONT * NVL(PCMOVPREFAT.VLCREDPRESUMIDO, 0),2) AS VLCREDPRESUMIDO
            ,PCMOVPREFAT.CODIGOTRIBUTACAOIS
            ,PCMOVPREFAT.CSTIS
            ,PCMOVPREFAT.CCLASSTRIBIS
            ,ROUND(PCMOVPREFAT.QTCONT * NVL(PCMOVPREFAT.VLBASEIS, 0), 2) AS VLBASEIS
            ,NVL(PCMOVPREFAT.ALIQIS, 0) AS ALIQIS
            ,NVL(PCMOVPREFAT.ALIQESPECIFICAIS, 0) AS ALIQESPECIFICAIS
            ,ROUND(PCMOVPREFAT.QTCONT * NVL(PCMOVPREFAT.VLIS, 0), 2) AS VLIS
            ,PCMOVPREFAT.CODIGOTRIBUTACAOCBSIBS
            ,PCMOVPREFAT.CSTIBSCBS
            ,PCMOVPREFAT.CCLASSTRIBIBSCBS
            ,ROUND(PCMOVPREFAT.QTCONT * NVL(PCMOVPREFAT.VLBASEIBSCBS, 0), 2) AS VLBASEIBSCBS
            ,NVL(PCMOVPREFAT.IBSUF, 0) AS IBSUF
            ,NVL(PCMOVPREFAT.PDIFIBSUF, 0) AS PDIFIBSUF
            ,ROUND(PCMOVPREFAT.QTCONT * NVL(PCMOVPREFAT.VDIFIBSUF, 0), 2) AS VDIFIBSUF
            ,NVL(PCMOVPREFAT.PREDALIQIBSUF, 0) AS PREDALIQIBSUF
            ,NVL(PCMOVPREFAT.PALIQEFETIBSUF, 0) AS PALIQEFETIBSUF
            ,ROUND(PCMOVPREFAT.QTCONT * NVL(PCMOVPREFAT.VIBSUF, 0), 2) AS VIBSUF
            ,NVL(PCMOVPREFAT.PIBSMUN, 0) AS PIBSMUN
            ,NVL(PCMOVPREFAT.PDIFIBSMUN, 0) AS PDIFIBSMUN
            ,ROUND(PCMOVPREFAT.QTCONT * NVL(PCMOVPREFAT.VDIFIBSMUN, 0), 2) AS VDIFIBSMUN
            ,NVL(PCMOVPREFAT.PREDALIQIBSMUN, 0) AS PREDALIQIBSMUN
            ,NVL(PCMOVPREFAT.PALIQEFETIBSMUN, 0) AS PALIQEFETIBSMUN
            ,ROUND(PCMOVPREFAT.QTCONT * NVL(PCMOVPREFAT.VIBSMUN, 0), 2) AS VIBSMUN
            ,NVL(PCMOVPREFAT.ALIQCBS, 0) AS ALIQCBS
            ,NVL(PCMOVPREFAT.PDIFCBS, 0) AS PDIFCBS
            ,ROUND(PCMOVPREFAT.QTCONT * NVL(PCMOVPREFAT.VDIFCBS, 0), 2) AS VDIFCBS
            ,NVL(PCMOVPREFAT.PREDALIQCBS, 0) AS PREDALIQCBS
            ,NVL(PCMOVPREFAT.PALIQEFETCBS, 0) AS PALIQEFETCBS
            ,ROUND(PCMOVPREFAT.QTCONT * NVL(PCMOVPREFAT.VLCBS, 0), 2) AS VLCBS
            ,PCMOVPREFAT.CSTTRIBREG
            ,PCMOVPREFAT.CCLASSTRIBREG
            ,NVL(PCMOVPREFAT.PALIQEFETREGIBSUF, 0) AS PALIQEFETREGIBSUF
            ,ROUND(PCMOVPREFAT.QTCONT * NVL(PCMOVPREFAT.VTRIBREGIBSUF, 0), 2) AS VTRIBREGIBSUF
            ,NVL(PCMOVPREFAT.ALIQEFETREGIBSMUN, 0) AS ALIQEFETREGIBSMUN
            ,ROUND(PCMOVPREFAT.QTCONT * NVL(PCMOVPREFAT.VTRIBREGIBSMUN, 0), 2) AS VTRIBREGIBSMUN
            ,NVL(PCMOVPREFAT.PALIQEFETREGCBS, 0) AS PALIQEFETREGCBS
            ,ROUND(PCMOVPREFAT.QTCONT * NVL(PCMOVPREFAT.VTRIBREGCBS, 0), 2) AS VTRIBREGCBS
            ,NVL(PCMOVPREFAT.PIBSUFCOMPRAGOV, 0) AS PIBSUFCOMPRAGOV
            ,ROUND(PCMOVPREFAT.QTCONT * NVL(PCMOVPREFAT.VIBSUFCOMPRAGOV, 0), 2) AS VIBSUFCOMPRAGOV
            ,NVL(PCMOVPREFAT.PIBSMUNCOMPRAGOV, 0) AS PIBSMUNCOMPRAGOV
            ,ROUND(PCMOVPREFAT.QTCONT * NVL(PCMOVPREFAT.VIBSMUNCOMPRAGOV, 0), 2) AS VIBSMUNCOMPRAGOV
            ,NVL(PCMOVPREFAT.PCBGCOMPRAGOV, 0) AS PCBGCOMPRAGOV
            ,ROUND(PCMOVPREFAT.QTCONT * NVL(PCMOVPREFAT.VCBSCOMPRAGOV, 0), 2) AS VCBSCOMPRAGOV
            ,NVL(PCMOVPREFAT.SOMATOTALNF_CBS, 'N') AS SOMATOTALNF_CBS
            ,NVL(PCMOVPREFAT.SOMATOTALNF_IBS, 'N') AS SOMATOTALNF_IBS
            ,NVL(PCMOVPREFAT.SOMATOTALNF_IS, 'N') AS SOMATOTALNF_IS
            ,ROUND(PCMOVPREFAT.QTCONT * NVL(PCMOVPREFAT.VIBSESTCRED,0), 2) AS VIBSESTCRED
            ,ROUND(PCMOVPREFAT.QTCONT * NVL(PCMOVPREFAT.VCBSESTCRED,0), 2) AS VCBSESTCRED    
            ,ROUND(PCMOVPREFAT.QTCONT * NVL(PCMOVPREFAT.VDEVTRIBIBSMUN,0), 2) AS VDEVTRIBIBSMUN
            ,ROUND(PCMOVPREFAT.QTCONT * NVL(PCMOVPREFAT.VDEVTRIBIBSUF,0), 2) AS VDEVTRIBIBSUF
            ,ROUND(PCMOVPREFAT.QTCONT * NVL(PCMOVPREFAT.VDEVTRIBCBS,0), 2) AS VDEVTRIBCBS
            ,PCMOVPREFAT.CCREDPRES
            ,ROUND(PCMOVPREFAT.QTCONT * NVL(PCMOVPREFAT.VBCCREDPRES,0), 2) AS VBCCREDPRES
            ,NVL(PCMOVPREFAT.PERCBSCREDPRES,0) AS PERCBSCREDPRES
            ,ROUND(PCMOVPREFAT.QTCONT * NVL(PCMOVPREFAT.VCBSCREDPRES,0), 2) AS VCBSCREDPRES
            ,ROUND(PCMOVPREFAT.QTCONT * NVL(PCMOVPREFAT.VCBSCREDPRESCONDSUS,0), 2) AS VCBSCREDPRESCONDSUS
            ,NVL(PCMOVPREFAT.PERIBSCREDPRES,0) AS PERIBSCREDPRES
            ,ROUND(PCMOVPREFAT.QTCONT * NVL(PCMOVPREFAT.VIBSCREDPRES,0), 2) AS VIBSCREDPRES
            ,ROUND(PCMOVPREFAT.QTCONT * NVL(PCMOVPREFAT.VIBSCREDPRESCONDSUS,0), 2) AS VIBSCREDPRESCONDSUS            
            ,PCMOVPREFAT.DFEREF_CHAVEACESSO
            ,PCMOVPREFAT.DFEREF_NITEM
            ,PCMOVPREFAT.CPFRESPTECNICOAGRICOLA
            ,PCMOVPREFAT.NUMRECEITUARIOAGRICOLA
            ,PCMOVCOMPLEPREFAT.CBENEFRBC
      FROM   PCMOVPREFAT
            ,PCPRODUT
            ,PCMOVCOMPLEPREFAT
            ,PCEMBALAGEM
