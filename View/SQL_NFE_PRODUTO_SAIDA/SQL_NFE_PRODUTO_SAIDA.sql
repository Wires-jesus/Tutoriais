          ...
          ,CASE WHEN (PCMOVPREFAT.CODOPER ='SD' AND PARAMFILIAL.OBTERCOMOVARCHAR2('ENVIASIMPNACDEVFORNECNFE', PCFILIAL.CODIGO) = 'N') AND
           ...
           END AS VALOR_ICMS_OPERACAO
            ...
            ,NVL(PCMOVCOMPLEPREFAT.CODSITTRIBIPI,DECODE(NVL(DECODE(NVL(PCMOVPREFAT.CALCCREDIPI,
                    ...
                    'S',
                    50,
                    NVL((SELECT CODSITTRIBIPISAID FROM PCFIGURATRIBIPI F WHERE F.CODFIGURAIPI IN (SELECT CODFIGURAIPI FROM PCTRIBIPI WHERE CODPROD = PCMOVPREFAT.CODPROD AND NVL(PCMOVPREFAT.CODFILIAL, PCMOVPREFAT.CODFILIALNF) = PCTRIBIPI.CODFILIAL AND ROWNUM = 1) AND ROWNUM = 1), 99))) SITUACAO_TRIBUTARIA_IPI
