...
SELECT  D.SEQDOCTO,
        ...
        CASE
          WHEN  D.ESPECIE = 'RP' THEN
                PG.NSUTEF
          ELSE (SELECT DT.NSUTEF FROM MONITORPDVMIDDLE.TB_DOCTOTEF DT WHERE DT.SEQDOCTO = D.SEQDOCTO AND DT.NROEMPRESA = D.NROEMPRESA AND DT.NROCHECKOUT = D.NROCHECKOUT)
        END VALOR,
        
        CASE
          WHEN  D.ESPECIE = 'RP' THEN
                P.VALOR
          ELSE (SELECT DT.VLRTOTAL FROM MONITORPDVMIDDLE.TB_DOCTOTEF DT WHERE DT.SEQDOCTO = D.SEQDOCTO AND DT.NROEMPRESA = D.NROEMPRESA AND DT.NROCHECKOUT = D.NROCHECKOUT)
        END VALOR, 
