...
SELECT NUM_TRANSACAO
      ...
      ,ROUND((CASE WHEN (NVL(FINALIDADENFE,'O') = 'C') THEN
         0
       else QUANTIDADE_COMERCIAL end), 4) QUANTIDADE_COMERCIAL
      ,VALOR_COMERCIAL
      ,VALOR_LIQUIDO
...
SELECT PCMOV.NUMTRANSENT AS NUM_TRANSACAO
              ...
              ,ROUND(PCMOV.QTCONT, 6) QUANTIDADE_COMERCIAL
              ,CASE WHEN (NVL(PARAMFILIAL.OBTERCOMOVARCHAR2('UTILIZAPRECOPERSNFIMP', PCFILIAL.CODIGO),'N') = 'S') AND
                         ...
                         --MELHORIA PARA PASSAR A FORMAR O VALOR DE PRODUTOS DE ACORDO COM PARAMETROS DA 132 - HIS.02054.2015
                           ...
                           + DECODE(NVL(PARAMFILIAL.OBTERCOMOVARCHAR2('PRECOPERSNFIMP_ST', PCFILIAL.CODIGO),'N'), 'S', NVL(PCMOV.ST, 0) + NVL(PCMOVCOMPLE.VLFECP, 0), 0)
               ...
               ELSE
                 CASE WHEN (PCNFENT.TIPODESCARGA = 'F') THEN
                             ...
                             - DECODE(NVL(PCNFENT.FINALIDADENFE, 'N'), 'C', NVL(PCMOV.VLIPI,0), ((ROUND(NVL(PCMOV.VLIPI,0) * PCMOV.QTCONT,NVL(PARAMFILIAL.ObterComoNumber('QTDCASASVLUNITARIONFE'),2))) / PCMOV.QTCONT))
              ...
              ,CASE WHEN (LENGTH(PCPRODUT.CODAUXILIARTRIB) <= NVL(PCPRODUT.GTINCODAUXILIARTRIB,0)) THEN
               ...
               ELSE
                  NULL
               END AS EAN_UNIDADE
              ,NVL(PCMOVCOMPLE.UNIDADETRIB, PCMOV.UNIDADE) AS UNIDADE_TRIBUTAVEL
              ...
              ,CASE WHEN ((PCNFENT.TIPODESCARGA = 'F') AND (NVL(PCMOVCOMPLE.PERCICMBKPDIFPR, 0) > 0)) THEN ROUND(NVL(PCMOVCOMPLE.VLICMS, 0) * PCMOV.QTCONT, 2)
               ELSE
              ...
              ,(PCMOV.QTCONT * PCMOVCOMPLE.VLICMSDESONERACAO) AS VLICMSDESONERACAO
        ...
        AND     (PCNFENT.TIPODESCARGA NOT IN ('P', 'N')))
        ...
        SELECT PCMOV.NUMTRANSENT AS NUM_TRANSACAO
              ...
              ,PCMOV.CODFISCAL AS CFOP
              ,PCMOV.UNIDADE AS UNIDADE_COMERCIAL
              ,ROUND(PCMOV.QTCONT, 6) AS QUANTIDADE_COMERCIAL
              ,CASE WHEN (NVL(PARAMFILIAL.OBTERCOMOVARCHAR2('UTILIZAPRECOPERSNFIMP', PCFILIAL.CODIGO),'N') = 'S') AND
               ...
               ELSE
                             ...
                             (CASE WHEN (((NVL(PCMOV.VLIPI,0) + NVL(PCMOV.ST,0) + NVL(PCMOVCOMPLE.VLFECP, 0) + NVL(PCMOVCOMPLE.VLDESCONTONF, 0)) > 0) AND (NVL(PCMOV.CODOPER, 'E') = 'ED') ) THEN
                                 ...
                                 / PCMOV.QTCONT)
                             ELSE
                                 DECODE(PCMOVCOMPLE.BONIFIC, 'S', PCMOV.PBONIFIC, ROUND(PCMOV.PUNITCONT * PCMOV.QTCONT, (CASE WHEN PCMOV.QTCONT < 1 THEN 6 ELSE NVL(PARAMFILIAL.ObterComoNumber('QTDCASASVLUNITARIONFE'),2) END)) / PCMOV.QTCONT )
                             END
              ...
              ,ROUND((DECODE(NVL(PCMOVCOMPLE.PRECOUTILIZADONFE, NVL(PCCLIENT.PRECOUTILIZADONFE,NVL(PARAMFILIAL.OBTERCOMOVARCHAR2('PRECOUTILIZADONFE',
                        ...
                        DECODE(NVL(PCMOVCOMPLE.VLDESCONTONF, 0), 0,
                            ...
                            (CASE WHEN ((PCNFENT.TIPODESCARGA IN ('6', '8', 'T')) AND NOT(NVL(PCMOVCOMPLE.CODMOTIVOICMSDESONERADO, 'X') IN ('7', '8'))) THEN
                                   DECODE(PKG_TRIBUTACAO.GET_CLIENTE_SUFRAMADO(PCNFENT.CODFORNEC, PCNFENT.DTENT), 'S',
                                     (ROUND(NVL(PCMOV.VLDESCSUFRAMA, 0) * PCMOV.QTCONT, NVL(PARAMFILIAL.OBTERCOMONUMBER('QTDCASASVLUNITARIONFE'), 2)) / PCMOV.QTCONT), 0)
                   * PCMOV.QTCONT
                                 ELSE 0 END) +
              ...
              ,NVL(NVL(PCMOV.PERCICMCP, PCMOV.PERCICM), 0) AS ALIQUOTA_ICMS
              ,CASE WHEN ((PCNFENT.TIPODESCARGA in ('N', 'P')) AND (NVL(PCMOVCOMPLE.PERCICMBKPDIFPR, 0) > 0)) THEN ROUND(NVL(PCMOVCOMPLE.VLICMS, 0) * PCMOV.QTCONT, 2)
               ELSE
                    ROUND(ROUND(PCMOV.QTCONT * (NVL(PCMOV.BASEICMS, 0) + NVL(PCMOVCOMPLE.VLBASEFRETE,0) + NVL(PCMOVCOMPLE.VLBASEOUTROS,0)), 2) *
                          (NVL(NVL(PCMOV.PERCICMCP, PCMOV.PERCICM), 0) / 100), 2)
                     -
        ...
        AND      (PCNFENT.TIPODESCARGA IN ('P', 'N', '6'))))
        AND    PCMOV.NUMTRANSENT = PCNFENT.NUMTRANSENT
        AND   PCMOV.CODFISCAL = PCCFO.CODFISCAL(+))
ORDER  BY NUMSEQ
