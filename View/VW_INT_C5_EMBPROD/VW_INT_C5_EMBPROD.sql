...
WITH
  DTPADRAO AS (SELECT /*+ INLINE */ MIN(S.ULTIMAEXECUCAO) -1440/24/60 AS ULTIMAEXECUCAO
               ...
               OR    (upper(s.objetoreferencia) = 'PKG_SINC_PDV_CONSINCO.CARREGA_TB_PRODPRECO')*/
			         (upper(s.objetoreferencia) = 'PKG_SINC_PDV_CONSINCO.CARREGA_PRODUTOS')
  ...
  PARAM_REGIAO AS (
    SELECT /*+ INLINE */ 
		   ...
		   FERRAMENTAS.F_BUSCARPARAMETRO_ALFA('FIL_PRECOPOREMBALAGEM', F.CODFILIAL, 'N') AS USA_PRECO_EMB,
           FERRAMENTAS.F_BUSCARPARAMETRO_ALFA('FIL_TIPOPRECIFICACAO', '99', 'V') AS TIPO_PRECIFICACAO
    FROM FILIAIS F
  ),
...
PROD_ALTERADOS AS (SELECT * FROM(
    ...
    SELECT  /*+
        ...
        */
        ...
        CASE
          WHEN PR.USA_PRECO_EMB = 'S' THEN
               NVL(e.pvenda, 0)
          WHEN PR.TIPO_PRECIFICACAO = 'A' THEN
               (NVL(TPR.PVENDAATAC1, 0) * NVL(E.QTUNIT, 1) * DECODE(E.FATORPRECO, 0, 1, NULL, 1, E.FATORPRECO))
          ELSE (NVL(TPR.PVENDA1, 0) * NVL(E.QTUNIT, 1) * DECODE(E.FATORPRECO, 0, 1, NULL, 1, E.FATORPRECO))
        END PVENDA,

        CASE
           WHEN PR.USA_PRECO_EMB = 'S' THEN
                E.DTULTALTPVENDA
           ELSE TPR.DTULTALTPVENDA
        END DTULTALTPVENDA,
        ...
        CASE
          WHEN PR.USA_PRECO_EMB = 'S' THEN
               NVL(e.pvendaatac,0)
          WHEN PR.TIPO_PRECIFICACAO = 'A' THEN
               (NVL(TPR.PVENDA1, 0) * NVL(E.QTUNIT, 1) * DECODE(E.FATORPRECO, 0, 1, NULL, 1, E.FATORPRECO))
          ELSE (NVL(TPR.PVENDAATAC1, 0) * NVL(E.QTUNIT, 1) * DECODE(E.FATORPRECO, 0, 1, NULL, 1, E.FATORPRECO))
        END PVENDAATAC,
        ...
        CASE
           ...
           ELSE NVL(NVL(NVL(TPR.PCOMREP1, E.PCOMREP1), P.PCOMREP1), 0)
        END COMISSAO,
      ...
      FROM PROD_ALTERADOS PA
		   ...
		   LEFT JOIN PARAM_REGIAO PR ON PR.CODFILIAL = E.CODFILIAL
           LEFT JOIN PCTABPR TPR ON TPR.CODPROD = E.CODPROD AND TPR.NUMREGIAO = PR.NUMREGIAO
