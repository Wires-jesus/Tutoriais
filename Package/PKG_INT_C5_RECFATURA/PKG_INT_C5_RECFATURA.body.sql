CREATE OR REPLACE PACKAGE BODY PKG_INT_C5_RECFATURA IS
   ...
   PROCEDURE processar_rec_faturas(
		p_seqdocto NUMBER Default 0, 
   ...
   CURSOR c_recfatura IS
	...
	FUNCTION retornar_xml_recfatura(r_recfatura c_recfatura%ROWTYPE)
      ...
      BEGIN
			SELECT     
				XMLELEMENT("Recebimento",
					XMLFOREST(C5.CODFILIAL AS "Codfilial",
   ...
   BEGIN
      ...
      SELECT 
			XMLELEMENT("EsquemaExportacao",
				XMLELEMENT("RecebimentosDeFatura",
					XMLELEMENT("DefinicaoRecebimentoDeFatura", l_recfatura, l_xmltypeprest)
				)
			) 
			esquemaexportacao
		INTO l_xmlesquema
