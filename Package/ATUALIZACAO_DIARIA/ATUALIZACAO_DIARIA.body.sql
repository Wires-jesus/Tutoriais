...
PROCEDURE P_PC_ARMAZENARSALDOSESTOQUE(PDTPROCESSAMENTO IN DATE
  ...
  IS
    ...
    *********************************************************************************/
    ...
    TYPE tpQTTRANSITOBENEFIC IS TABLE OF PCHISTESTFILA.QTTRANSITOBENEFIC%TYPE INDEX BY BINARY_INTEGER;
    ...
    vtQTTRANSITOBENEFIC              tpQTTRANSITOBENEFIC;
...
USING (
    ...
    VENDAS_FILTRADAS AS (
        SELECT /*+ MATERIALIZE */ 
            CODFILIAL, CODPROD, DTSAIDA, VLVENDA, VLCUSTOFINB, VLCUSTOREAL, 
            VLCUSTOREP, VLCUSTOCONT, QTVENDA, CODOPER, NUMNOTA, QTENT, 
            VLENT, VLDEVOLCLI, QTBONIFIC, VLBONIFIC, QTDEVOLCLI, ICMSRETIDO, 
            VLIPI, VLREPASSE, VLCUSTOFINBONIF, ICMSRETIDOBONIFIC, VLIPIBONIFIC, 
            VLVERBACMV, VLVERBACMVCLI, NUMPED
        FROM VIEW_VENDAS_RESUMO_FATURAMENTO V
        WHERE V.DTSAIDA BETWEEN :PDTINICIO AND :PDTTERMINO
          AND NVL(V.CONDVENDA, -1) IN (1, 5, -1, 7, 9, 11, 14)
          AND V.CODFISCAL NOT IN (522, 622, 722, 532, 632, 732)
          ';
     
          IF (PLISTACODIGO IS NOT NULL) THEN
            V_SQLAUX := V_SQLAUX || ' AND v.CODPROD IN ( ' || PLISTACODIGO || ')';
          END IF;
          
     V_SQLAUX := V_SQLAUX || '
          AND EXISTS (SELECT 1 FROM FILIAIS F WHERE F.CODIGO = V.CODFILIAL)
    ),
    
    VERBAS AS (
        SELECT /*+ MATERIALIZE */ A.NUMPED, A.CODPROD, 
               SUM(NVL(A.VLVERBACMV, 0) * NVL(I.QT, 0)) AS VLVERBACMVAVULSO_SUM
        FROM PCAPLICVERBAPEDI A
        JOIN PCPEDI I 
          ON A.NUMPED = I.NUMPED AND A.CODPROD = I.CODPROD
        JOIN (SELECT DISTINCT NUMPED, CODPROD FROM VENDAS_FILTRADAS) VF 
          ON A.NUMPED = VF.NUMPED AND A.CODPROD = VF.CODPROD
          ';
     
          IF (PLISTACODIGO IS NOT NULL) THEN
            V_SQLAUX := V_SQLAUX || ' AND I.CODPROD IN ( ' || PLISTACODIGO || ')';
          END IF;
          
     V_SQLAUX := V_SQLAUX || '
        GROUP BY A.NUMPED, A.CODPROD
    ),
    ...
    VENDAS AS (
        SELECT /*+ MATERIALIZE */ 
            ...
            SUM(NVL(B.VLVERBACMVAVULSO_SUM, 0)) AS VLVERBACMVAVULSO
        FROM VENDAS_FILTRADAS V
        LEFT JOIN VERBAS B 
          ON B.NUMPED = V.NUMPED AND B.CODPROD = V.CODPROD
        GROUP BY V.CODFILIAL, V.CODPROD, V.DTSAIDA
    ),
    
    ESTOQUE AS (
            SELECT /*+ MATERIALIZE */ 
               CODFILIAL, CODPROD, DATA, QTESTGER, CUSTOFIN, CUSTOREAL, CUSTOREP, CUSTOCONT
                FROM PCHISTESTFILA
             ...
             V_SQLAUX := V_SQLAUX || '     UNION ALL
                SELECT H.CODFILIAL, H.CODPROD, H.DATA, H.QTESTGER, H.CUSTOFIN, H.CUSTOREAL, H.CUSTOREP, H.CUSTOCONT
                FROM PCHISTEST H
                WHERE H.DATA BETWEEN :PDTINICIO AND :PDTTERMINO ';
                IF (PLISTACODIGO IS NOT NULL) THEN
                  V_SQLAUX := V_SQLAUX || ' AND CODPROD IN ( ' || PLISTACODIGO || ')';
                END IF;
          ...
          AND NOT EXISTS (
              SELECT 1 
              FROM PCHISTESTFILA F
              WHERE F.DATA BETWEEN :PDTINICIO AND :PDTTERMINO
                AND F.CODFILIAL = H.CODFILIAL
                AND F.CODPROD = H.CODPROD
                AND F.DATA = H.DATA )  
    ...
    BASE_UNIFICADA AS (
       ...
       SELECT 
            NVL(V.CODFILIAL, E.CODFILIAL) AS CODFILIAL, 
            NVL(V.CODPROD, E.CODPROD) AS CODPROD, 
            NVL(V.DTMOV, E.DATA) AS DTMOV, 
            P.CODFORNEC, P.CODEPTO, P.CODPRODPRINC, P.CLASSE, P.CODSEC,
            NVL(V.VLVENDA, 0) AS VLVENDA, 
            NVL(V.VLCUSTOFIN, 0) AS VLCUSTOFIN, 
            NVL(V.VLCUSTOREAL, 0) AS VLCUSTOREAL, 
            NVL(V.VLCUSTOREP, 0) AS VLCUSTOREP, 
            NVL(V.VLCUSTOCONT, 0) AS VLCUSTOCONT, 
            NVL(V.QTVENDA, 0) AS QTVENDA, 
            NVL(V.QTNOTA, 0) AS QTNOTA, 
            NVL(V.VLENT, 0) AS VLENT, 
            NVL(V.QTENT, 0) AS QTENT, 
            NVL(V.QTDEVOLCLI, 0) AS QTDEVOLCLI, 
            NVL(V.VLDEVOLCLI, 0) AS VLDEVOLCLI, 
            NVL(V.QTBONIFIC, 0) AS QTBONIFIC, 
            NVL(V.VLBONIFIC, 0) AS VLBONIFIC, 
            NVL(V.ST, 0) AS ST, 
            NVL(V.VLIPI, 0) AS VLIPI, 
            NVL(V.VLREPASSE, 0) AS VLREPASSE, 
            NVL(V.VLCUSTOFINBONIF, 0) AS VLCUSTOFINBONIF, 
            NVL(V.STBONIFICACAO, 0) AS STBONIFICACAO, 
            NVL(V.VLIPIBONIFICACAO, 0) AS VLIPIBONIFICACAO, 
            NVL(V.VLVERBACMV, 0) AS VLVERBACMV, 
            NVL(V.VLVERBACMVCLI, 0) AS VLVERBACMVCLI, 
            NVL(V.VLVERBACMVAVULSO, 0) AS VLVERBACMVAVULSO,
            NVL(E.QTESTGER, 0) AS QTESTGER, 
            NVL(E.CUSTOFIN, 0) AS CUSTOFIN, 
            NVL(E.CUSTOREAL, 0) AS CUSTOREAL, 
            NVL(E.CUSTOREP, 0) AS CUSTOREP, 
            NVL(E.CUSTOCONT, 0) AS CUSTOCONT
        FROM VENDAS V
        FULL OUTER JOIN ESTOQUE E 
          ON V.CODFILIAL = E.CODFILIAL AND V.CODPROD = E.CODPROD AND V.DTMOV = E.DATA
        JOIN FILIAIS F 
          ON F.CODIGO = NVL(V.CODFILIAL, E.CODFILIAL) 
        JOIN PRODUTOS P 
          ON P.CODPROD = NVL(V.CODPROD, E.CODPROD)
    )
...
WHEN NOT MATCHED THEN 
	...
	begin      
       IF PCODFILIAL = '99' THEN
         PKG_PARAMETRO_CONTABIL .SET_CODFILIAL(NULL);
       ELSE   
        PKG_PARAMETRO_CONTABIL .SET_CODFILIAL(PCODFILIAL);
       END IF; 
