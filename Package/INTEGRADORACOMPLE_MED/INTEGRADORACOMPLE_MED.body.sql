...
/*((pdCUSTOFINEST) - ((pdCUSTOFINEST) * pdPERDESCCUSTO_ITEM / 100) +
  ...
  ***********************************************************************************************/
  ...
  PROCEDURE PRC_ALIMENTAR_VAR_PCEST
  (
  ...
  BEGIN
    ...
    EXCEPTION
      WHEN OTHERS THEN
        V_PERC_ICMS_ULTENT            := 0;
        V_PERC_ICMS_ANTECIPADO_ULTENT := 0;
        
    END;

    IF SQL%ROWCOUNT > 0
    THEN
      V_VARIAVEL.NOME  := '[PERICMULTENT]';
      V_VARIAVEL.VALOR := V_PERC_ICMS_ULTENT;
  ...
  PROCEDURE PRC_ALIMENTAR_VAR_PCTABPR
  (
    P_CODIGO_PRODUTO IN NUMBER,
    ...
    V_VARIAVEL                     FORMULA.RVARIAVEL;
    V_VALOR_ULTENT_MES             PCTABPR.VLULTENTMES%TYPE;
    V_VALOR_ULTENT_CONTABIL_SEM_ST PCTABPR.VLULTENTCONTSEMST%TYPE;
  ...
  BEGIN
    ...
    BEGIN
      ...
      SELECT VLULTENTMES
           , VLULTENTCONTSEMST
        INTO V_VALOR_ULTENT_MES
           , V_VALOR_ULTENT_CONTABIL_SEM_ST
        FROM PCTABPR
       WHERE PCTABPR.CODPROD = P_CODIGO_PRODUTO
         AND PCTABPR.NUMREGIAO = P_NUMERO_REGIAO;
    
    EXCEPTION
      WHEN OTHERS THEN
        V_VALOR_ULTENT_MES             := 0;
        V_VALOR_ULTENT_CONTABIL_SEM_ST := 0;
    
    END;

    IF SQL%ROWCOUNT > 0
    THEN
      V_VARIAVEL.NOME  := '[VLULTENTMES]';
      V_VARIAVEL.VALOR := V_VALOR_ULTENT_MES;
      FORMULA.ATRIBUIVALOR(V_VARIAVEL, P_VARIAVEIS);
  ...
  BEGIN
    ...
    V_VARIAVEL.NOME := '[PRECOCOMPRA]';
    V_VARIAVEL.VALOR  := P_PRECO_COMPRA;
    
    FORMULA.ATRIBUIVALOR(V_VARIAVEL, P_VARIAVEIS);

    V_VARIAVEL.NOME := '[MARKUP_SISTEMATICA]';
    V_VARIAVEL.VALOR  := 35;
    ...
    THEN
      ...
      THEN
        ...
        IF V_FORMULA_ICMTAB IS NOT NULL
        THEN
          V_VARIAVEL.NOME  := '&IMPOSTOSCMVCALC&';
          V_VARIAVEL.VALOR := '#' || V_FORMULA_ICMTAB || '#';
          
          FORMULA.ATRIBUIVALOR(V_VARIAVEL, P_VARIAVEIS);
        END IF;

        FOR DADOS_COLECAO IN (
          SELECT PCTRIBUT.FORMULAICMTAB,
                 PCCOLECAO.NOMEVARIAVEL,
                 PCCOLECAO.VALOR
            FROM PCTRIBUT,
                 PCCOLECAOVARIAVEISTRIBI PCCOLECAO
           WHERE PCTRIBUT.CODCOLECAOVAR = PCCOLECAO.CODCOLECAOVAR(+)
        ...
        LOOP
          ...
          IF DADOS_COLECAO.NOMEVARIAVEL IS NOT NULL 
          THEN
            V_VARIAVEL.NOME  := '[' || DADOS_COLECAO.NOMEVARIAVEL || ']';
            V_VARIAVEL.VALOR := DADOS_COLECAO.VALOR;
    ...
    PRC_ALIMENTAR_VAR_PCEST(P_CODIGO_PRODUTO,
                            P_CODIGO_FILIAL,
                            P_VARIAVEIS);
  ...
  PROCEDURE PRC_CARREGAR_VARIAVEIS_CMV
  (
  ...
  BEGIN
    ...
    V_VARIAVEL.NOME  := '[CUSTOPRECIFIC]';
    V_VARIAVEL.VALOR := P_CUSTO_PRECIFICADO;    
    FORMULA.ATRIBUIVALOR(V_VARIAVEL, P_VARIAVEIS);

    V_VARIAVEL.NOME  := '[IMPOSTOSCMV]';
    V_VARIAVEL.VALOR := P_IMPOSTOS_CMV;    
    FORMULA.ATRIBUIVALOR(V_VARIAVEL, P_VARIAVEIS);

    V_VARIAVEL.NOME  := '[MARGEM_IDEAL]';
    V_VARIAVEL.VALOR := P_MARGEM_IDEAL;    
    FORMULA.ATRIBUIVALOR(V_VARIAVEL, P_VARIAVEIS);

    V_VARIAVEL.NOME  := '[COMISSAO]';
    V_VARIAVEL.VALOR := P_COMISSAO;    
    FORMULA.ATRIBUIVALOR(V_VARIAVEL, P_VARIAVEIS);

    V_VARIAVEL.NOME  := '[FRETE]';
    V_VARIAVEL.VALOR := P_FRETE;    
    FORMULA.ATRIBUIVALOR(V_VARIAVEL, P_VARIAVEIS);

    V_VARIAVEL.NOME  := '[FRETE_OUTROS]';
    V_VARIAVEL.VALOR := P_FRETE_OUTROS;    
    FORMULA.ATRIBUIVALOR(V_VARIAVEL, P_VARIAVEIS);

    V_VARIAVEL.NOME  := '[IMPOSTOFEDERAL]';
    V_VARIAVEL.VALOR := P_TAXA_VENDA;    
    FORMULA.ATRIBUIVALOR(V_VARIAVEL, P_VARIAVEIS);

  END PRC_CARREGAR_VARIAVEIS_CMV; 
  ...
  (
    ...
    V_PERC_IMPOSTOS_CMV NUMBER;
    V_VARIAVEIS         FORMULA.TBVARIAVEIS;

  BEGIN

    V_PERC_IMPOSTOS_CMV := 0;
    ...
    IF V_VALIDO
    THEN
      PRC_CARREGAR_VARIAVEIS(P_PRECO,
                             P_PRECO_COMPRA,
                             P_ICMS,
                             P_CODST,
                             P_CODIGO_PRODUTO,
  ...
  BEGIN
    ...
    BEGIN
      ...
      SELECT NVL(PERCMARGEMMIN, 0)
        INTO V_MARGEM_MINIMA
        FROM PCPRODFILIAL
       WHERE CODPROD = P_CODIGO_PRODUTO
         AND CODFILIAL = P_CODIGO_FILIAL;
    ...
    IF V_MARGEM_MINIMA > 0 THEN

      V_RESULTADO := TRUE;
      P_MARGEM_MINIMA := V_MARGEM_MINIMA;

    END IF;

    RETURN V_RESULTADO;

  END F_OBTER_MARGEM_MINIMA_ITEM;
  ...
  BEGIN
    ...
    IF P_PRECO_VENDA > 0 THEN
      ...
      V_RESULTADO := (P_PRECO_VENDA - P_VALOR_CUSTO_FINANCEIRO) / P_PRECO_VENDA * 100;

    END IF;

    RETURN V_RESULTADO;

  END F_OBTER_MARGEM_LUCRO_ITEM;
