create or replace package body FISCAL is
  ...
  EXCEPTION
   ...
   --Função de buscar informações de IPI
   ...
   begin
     ...
     V_AGREGARFCPBASEPISCOFINSSAIDA := pAGREGARFCPBASEPISCOFINSSAIDA;
      ...
      VCODTRIB := GET_CODTRIBEXCECAO_PISCOFINS(PCODTRIB
                                              ...
                                              ,PNCM);
      -- BUSCAR INFORMAÇÕES DA FIGURA TRIBUTÁRIA -----------------------------------
      begin
         begin
            V_USAVIGENCIA := 'N';
         ...
         begin
            ...
            then
               ...
               if ((not PDEVOLUCAO) and (NVL(NVL(VCSTPIS, VCSTCOFINS), 0) <= 0))
                  or ((PDEVOLUCAO) and (NVL(VCSTDEV, 0) <= 0))
               then
                  MSG := 'N:CST PIS/COFINS Pauta Mínima não informado para tributação: ' ||
...
--                          ,((NVL(M.BASEICMS, 0) + NVL(MC.VLBASEFRETE, 0) + NVL(MC.VLBASEOUTROS, 0)) * (NVL(NVL(M.PERCICMCP, M.PERCICM), 0) / 100)) AS VLICMS
   ...
   begin
      ...
      -- GERAR VALORES DE PIS/COFINS SEM A VENDA, INDO NOVAMENTE NA TRIBUTAÇÃO
   ...
   update PCNFSAIDPREFAT
      ...
      then
         -- APLICAR TRIBUTAÇÃO NO CONHECIMENTO DE ENTRADA
         declare
      ...
      --  FIM APLICAR TRIBUTAÇÃO NO CONHECIMENTO DE ENTRADA
...
--RATEIO OUTRAS DESPESAS E FRETE (ILQUIAS 20/07/2011)
   ...
   begin
    ...
    WITH TABELA AS(
     ...
     GEROU_OUTROS_FRETE_ULTIMO_REG := 'N' ;
      ...
      -- ATUALIZAR INFORMAÇÃO DO RECÁLCULO REALIZADA.
      update PCNFSAIDPREFAT
      ...
      for DADOS in (SELECT M.NUMTRANSVENDA NUMTRANSACAO,
        ...
        if ((DADOS.CLI_PRODUTOR_RURAL = 'S') or
          ...
          if ((VMOTIVODESONERACAO > 0) and (((dados.VLDESCSUFRAMA + DADOS.VLDESCICMISENCAO) > 0) or ((DADOS.VLREDUCAO_ACRESCIMO_ICMS +
            ...
            MSG := MSG ||
                   'S:NÃO FOI ENCONTRADO CADASTRO DE TRIBUTAÇÃO, VERIFIQUE ROTINA 4003! CST NÃO ENCONTRADO: ' ||
                   DADOS.SITTRIBUT;
          end if;
        end if;
      end loop;
 ...
 FUNCTION OBTER_ALIQUOTAS_PISCOFINS(P_CODPROD   IN NUMBER,
  ...
  BEGIN
    ...
    EXCEPTION
      WHEN NO_DATA_FOUND THEN
        P_MENSAGEM_RETORNO := 'Não foi encontrado cliente com o código informado (' ||
                             P_CODIGO_CLIENTE || ').';
        RETURN FALSE;
    END;
  ...
  -----------------------------------------------------------------------------------------
  FUNCTION GET_SQL_BASE_TRIBUTACAO(P_PARAMETROS IN TIPO_TRIBUT_REFORMA,
                                 P_TIPO_LOCAL_CONSUMO    IN VARCHAR2,
                                 P_LOCAL_CONSUMO         IN VARCHAR2
  ) RETURN VARCHAR2 IS
  ...
  BEGIN
    V_SQL := '
        ...
        FROM (SELECT PCTRIBUTACAO.CODIGO_TRIBUTACAO,
                     NVL(PCTRIBUTACAO.CODTRIBREGULAR,0) CODTRIBREGULAR,
    ...
    RETURN V_SQL;
  END GET_SQL_BASE_TRIBUTACAO;
  -----------------------------------------------------------------------------------------
  FUNCTION GET_SQL_FILTRO_TRIB_EXATA(P_PARAMETROS IN TIPO_TRIBUT_REFORMA) 
  RETURN VARCHAR2 IS
    V_SQL_FILTRO VARCHAR2(32767);
  BEGIN
    V_SQL_FILTRO := '
                AND ((-- REGRA 1 - Somente CFOP + PRODUTO + NCM
                           EXISTS (SELECT 1 FROM PCTRIBUTACAO_FILTRO_CFOP    WHERE CODIGO_TRIBUTACAO = PCTRIBUTACAO.CODIGO_TRIBUTACAO AND DTEXCLUSAO IS NULL AND CODFISCAL = ' || P_PARAMETROS.CFOP || ')
                       AND EXISTS (SELECT 1 FROM PCTRIBUTACAO_FILTRO_PRODUTO WHERE CODIGO_TRIBUTACAO = PCTRIBUTACAO.CODIGO_TRIBUTACAO AND DTEXCLUSAO IS NULL AND CODPROD = ' || P_PARAMETROS.CODPROD || ')
                       AND EXISTS (SELECT 1 FROM PCTRIBUTACAO_FILTRO_NCM     WHERE CODIGO_TRIBUTACAO = PCTRIBUTACAO.CODIGO_TRIBUTACAO AND DTEXCLUSAO IS NULL AND NCM = ' || P_PARAMETROS.NCM || ') )
                    OR (-- REGRA 2 - Somente CFOP + NCM
                           EXISTS (SELECT 1 FROM PCTRIBUTACAO_FILTRO_CFOP    WHERE CODIGO_TRIBUTACAO = PCTRIBUTACAO.CODIGO_TRIBUTACAO AND DTEXCLUSAO IS NULL AND CODFISCAL = ' || P_PARAMETROS.CFOP || ')
                       AND EXISTS (SELECT 1 FROM PCTRIBUTACAO_FILTRO_NCM     WHERE CODIGO_TRIBUTACAO = PCTRIBUTACAO.CODIGO_TRIBUTACAO AND DTEXCLUSAO IS NULL AND NCM = ' || P_PARAMETROS.NCM || ')
                       AND NOT EXISTS (SELECT 1 FROM PCTRIBUTACAO_FILTRO_PRODUTO WHERE CODIGO_TRIBUTACAO = PCTRIBUTACAO.CODIGO_TRIBUTACAO AND DTEXCLUSAO IS NULL AND CODPROD = ' || P_PARAMETROS.CODPROD || ') )
                    OR (-- REGRA 3 - Somente CFOP + PRODUTO
                           EXISTS (SELECT 1 FROM PCTRIBUTACAO_FILTRO_CFOP    WHERE CODIGO_TRIBUTACAO = PCTRIBUTACAO.CODIGO_TRIBUTACAO AND DTEXCLUSAO IS NULL AND CODFISCAL = ' || P_PARAMETROS.CFOP || ')
                       AND EXISTS (SELECT 1 FROM PCTRIBUTACAO_FILTRO_PRODUTO WHERE CODIGO_TRIBUTACAO = PCTRIBUTACAO.CODIGO_TRIBUTACAO AND DTEXCLUSAO IS NULL AND CODPROD = ' || P_PARAMETROS.CODPROD || ')
                       AND NOT EXISTS (SELECT 1 FROM PCTRIBUTACAO_FILTRO_NCM     WHERE CODIGO_TRIBUTACAO = PCTRIBUTACAO.CODIGO_TRIBUTACAO AND DTEXCLUSAO IS NULL AND NCM = ' || P_PARAMETROS.NCM || ') )
                    OR (-- REGRA 4 - Somente CFOP
                           EXISTS (SELECT 1 FROM PCTRIBUTACAO_FILTRO_CFOP    WHERE CODIGO_TRIBUTACAO = PCTRIBUTACAO.CODIGO_TRIBUTACAO AND DTEXCLUSAO IS NULL AND CODFISCAL = ' || P_PARAMETROS.CFOP || ') 
                       AND NOT EXISTS (SELECT 1 FROM PCTRIBUTACAO_FILTRO_PRODUTO WHERE CODIGO_TRIBUTACAO = PCTRIBUTACAO.CODIGO_TRIBUTACAO AND DTEXCLUSAO IS NULL AND CODPROD = ' || P_PARAMETROS.CODPROD || ')
                       AND NOT EXISTS (SELECT 1 FROM PCTRIBUTACAO_FILTRO_NCM     WHERE CODIGO_TRIBUTACAO = PCTRIBUTACAO.CODIGO_TRIBUTACAO AND DTEXCLUSAO IS NULL AND NCM = ' || P_PARAMETROS.NCM || ') )
                    OR (-- REGRA 5 - Somente NCM
                           EXISTS (SELECT 1 FROM PCTRIBUTACAO_FILTRO_NCM     WHERE CODIGO_TRIBUTACAO = PCTRIBUTACAO.CODIGO_TRIBUTACAO AND DTEXCLUSAO IS NULL AND NCM = ' || P_PARAMETROS.NCM || ')
                       AND NOT EXISTS (SELECT 1 FROM PCTRIBUTACAO_FILTRO_CFOP    WHERE CODIGO_TRIBUTACAO = PCTRIBUTACAO.CODIGO_TRIBUTACAO AND DTEXCLUSAO IS NULL AND CODFISCAL = ' || P_PARAMETROS.CFOP || ')
                       AND NOT EXISTS (SELECT 1 FROM PCTRIBUTACAO_FILTRO_PRODUTO WHERE CODIGO_TRIBUTACAO = PCTRIBUTACAO.CODIGO_TRIBUTACAO AND DTEXCLUSAO IS NULL AND CODPROD = ' || P_PARAMETROS.CODPROD || ') )
                    OR (-- REGRA 6 - Somente PRODUTO + NCM
                           EXISTS (SELECT 1 FROM PCTRIBUTACAO_FILTRO_PRODUTO WHERE CODIGO_TRIBUTACAO = PCTRIBUTACAO.CODIGO_TRIBUTACAO AND DTEXCLUSAO IS NULL AND CODPROD = ' || P_PARAMETROS.CODPROD || ')
                       AND EXISTS (SELECT 1 FROM PCTRIBUTACAO_FILTRO_NCM     WHERE CODIGO_TRIBUTACAO = PCTRIBUTACAO.CODIGO_TRIBUTACAO AND DTEXCLUSAO IS NULL AND NCM = ' || P_PARAMETROS.NCM || ')
                       AND NOT EXISTS (SELECT 1 FROM PCTRIBUTACAO_FILTRO_CFOP    WHERE CODIGO_TRIBUTACAO = PCTRIBUTACAO.CODIGO_TRIBUTACAO AND DTEXCLUSAO IS NULL AND CODFISCAL = ' || P_PARAMETROS.CFOP || ') )
                    OR (-- REGRA 7 - Somente PRODUTO
                           EXISTS (SELECT 1 FROM PCTRIBUTACAO_FILTRO_PRODUTO WHERE CODIGO_TRIBUTACAO = PCTRIBUTACAO.CODIGO_TRIBUTACAO AND DTEXCLUSAO IS NULL AND CODPROD = ' || P_PARAMETROS.CODPROD || ')
                       AND NOT EXISTS (SELECT 1 FROM PCTRIBUTACAO_FILTRO_CFOP    WHERE CODIGO_TRIBUTACAO = PCTRIBUTACAO.CODIGO_TRIBUTACAO AND DTEXCLUSAO IS NULL AND CODFISCAL = ' || P_PARAMETROS.CFOP || ') 
                       AND NOT EXISTS (SELECT 1 FROM PCTRIBUTACAO_FILTRO_NCM     WHERE CODIGO_TRIBUTACAO = PCTRIBUTACAO.CODIGO_TRIBUTACAO AND DTEXCLUSAO IS NULL AND NCM = ' || P_PARAMETROS.NCM || ') )
                    OR (-- REGRA 8 - Sem Filtros compativeis
                           NOT EXISTS (SELECT 1 FROM PCTRIBUTACAO_FILTRO_CFOP    WHERE CODIGO_TRIBUTACAO = PCTRIBUTACAO.CODIGO_TRIBUTACAO AND DTEXCLUSAO IS NULL)
                       AND NOT EXISTS (SELECT 1 FROM PCTRIBUTACAO_FILTRO_PRODUTO WHERE CODIGO_TRIBUTACAO = PCTRIBUTACAO.CODIGO_TRIBUTACAO AND DTEXCLUSAO IS NULL)
                       AND NOT EXISTS (SELECT 1 FROM PCTRIBUTACAO_FILTRO_NCM     WHERE CODIGO_TRIBUTACAO = PCTRIBUTACAO.CODIGO_TRIBUTACAO AND DTEXCLUSAO IS NULL) ) )
                 -- ORDENAÇÃO --
                 ORDER BY
                       CASE WHEN EXISTS (SELECT 1 FROM PCTRIBUTACAO_FILTRO_CFOP WHERE CODIGO_TRIBUTACAO = PCTRIBUTACAO.CODIGO_TRIBUTACAO AND DTEXCLUSAO IS NULL AND CODFISCAL = ' || P_PARAMETROS.CFOP || ')
                             AND EXISTS (SELECT 1 FROM PCTRIBUTACAO_FILTRO_PRODUTO WHERE CODIGO_TRIBUTACAO = PCTRIBUTACAO.CODIGO_TRIBUTACAO AND DTEXCLUSAO IS NULL AND CODPROD = ' || P_PARAMETROS.CODPROD || ')
                             AND EXISTS (SELECT 1 FROM PCTRIBUTACAO_FILTRO_NCM WHERE CODIGO_TRIBUTACAO = PCTRIBUTACAO.CODIGO_TRIBUTACAO AND DTEXCLUSAO IS NULL AND NCM = ' || P_PARAMETROS.NCM || ') 
                            THEN 1
                            WHEN EXISTS (SELECT 1 FROM PCTRIBUTACAO_FILTRO_CFOP WHERE CODIGO_TRIBUTACAO = PCTRIBUTACAO.CODIGO_TRIBUTACAO AND DTEXCLUSAO IS NULL AND CODFISCAL = ' || P_PARAMETROS.CFOP || ')
                             AND EXISTS (SELECT 1 FROM PCTRIBUTACAO_FILTRO_NCM WHERE CODIGO_TRIBUTACAO = PCTRIBUTACAO.CODIGO_TRIBUTACAO AND DTEXCLUSAO IS NULL AND NCM = ' || P_PARAMETROS.NCM || ')
                             AND NOT EXISTS (SELECT 1 FROM PCTRIBUTACAO_FILTRO_PRODUTO WHERE CODIGO_TRIBUTACAO = PCTRIBUTACAO.CODIGO_TRIBUTACAO AND DTEXCLUSAO IS NULL AND CODPROD = ' || P_PARAMETROS.CODPROD || ') 
                            THEN 2
                            WHEN EXISTS (SELECT 1 FROM PCTRIBUTACAO_FILTRO_CFOP WHERE CODIGO_TRIBUTACAO = PCTRIBUTACAO.CODIGO_TRIBUTACAO AND DTEXCLUSAO IS NULL AND CODFISCAL = ' || P_PARAMETROS.CFOP || ')
                            ...
                            WHEN EXISTS (SELECT 1 FROM PCTRIBUTACAO_FILTRO_CFOP WHERE CODIGO_TRIBUTACAO = PCTRIBUTACAO.CODIGO_TRIBUTACAO AND DTEXCLUSAO IS NULL AND CODFISCAL = ' || P_PARAMETROS.CFOP || ')
                             ...
                             AND NOT EXISTS (SELECT 1 FROM PCTRIBUTACAO_FILTRO_NCM WHERE CODIGO_TRIBUTACAO = PCTRIBUTACAO.CODIGO_TRIBUTACAO AND DTEXCLUSAO IS NULL AND NCM = ' || P_PARAMETROS.NCM || ') 
                            THEN 4
                            WHEN EXISTS (SELECT 1 FROM PCTRIBUTACAO_FILTRO_NCM WHERE CODIGO_TRIBUTACAO = PCTRIBUTACAO.CODIGO_TRIBUTACAO AND DTEXCLUSAO IS NULL AND NCM = ' || P_PARAMETROS.NCM || ')
                             AND NOT EXISTS (SELECT 1 FROM PCTRIBUTACAO_FILTRO_CFOP WHERE CODIGO_TRIBUTACAO = PCTRIBUTACAO.CODIGO_TRIBUTACAO AND DTEXCLUSAO IS NULL AND CODFISCAL = ' || P_PARAMETROS.CFOP || ')
                             AND NOT EXISTS (SELECT 1 FROM PCTRIBUTACAO_FILTRO_PRODUTO WHERE CODIGO_TRIBUTACAO = PCTRIBUTACAO.CODIGO_TRIBUTACAO AND DTEXCLUSAO IS NULL AND CODPROD = ' || P_PARAMETROS.CODPROD || ') 
                            THEN 5
                            WHEN EXISTS (SELECT 1 FROM PCTRIBUTACAO_FILTRO_PRODUTO WHERE CODIGO_TRIBUTACAO = PCTRIBUTACAO.CODIGO_TRIBUTACAO AND DTEXCLUSAO IS NULL AND CODPROD = ' || P_PARAMETROS.CODPROD || ')
                             AND EXISTS (SELECT 1 FROM PCTRIBUTACAO_FILTRO_NCM WHERE CODIGO_TRIBUTACAO = PCTRIBUTACAO.CODIGO_TRIBUTACAO AND DTEXCLUSAO IS NULL AND NCM = ' || P_PARAMETROS.NCM || ')
                             AND NOT EXISTS (SELECT 1 FROM PCTRIBUTACAO_FILTRO_CFOP WHERE CODIGO_TRIBUTACAO = PCTRIBUTACAO.CODIGO_TRIBUTACAO AND DTEXCLUSAO IS NULL AND CODFISCAL = ' || P_PARAMETROS.CFOP || ') 
                            THEN 6
                            WHEN EXISTS (SELECT 1 FROM PCTRIBUTACAO_FILTRO_PRODUTO WHERE CODIGO_TRIBUTACAO = PCTRIBUTACAO.CODIGO_TRIBUTACAO AND DTEXCLUSAO IS NULL AND CODPROD = ' || P_PARAMETROS.CODPROD || ')
                             ...
                             AND NOT EXISTS (SELECT 1 FROM PCTRIBUTACAO_FILTRO_NCM WHERE CODIGO_TRIBUTACAO = PCTRIBUTACAO.CODIGO_TRIBUTACAO AND DTEXCLUSAO IS NULL AND NCM = ' || P_PARAMETROS.NCM || ') 
                            THEN 7
                            WHEN NOT EXISTS (SELECT 1 FROM PCTRIBUTACAO_FILTRO_CFOP WHERE CODIGO_TRIBUTACAO = PCTRIBUTACAO.CODIGO_TRIBUTACAO AND DTEXCLUSAO IS NULL)
                             AND NOT EXISTS (SELECT 1 FROM PCTRIBUTACAO_FILTRO_PRODUTO WHERE CODIGO_TRIBUTACAO = PCTRIBUTACAO.CODIGO_TRIBUTACAO AND DTEXCLUSAO IS NULL)
                             AND NOT EXISTS (SELECT 1 FROM PCTRIBUTACAO_FILTRO_NCM WHERE CODIGO_TRIBUTACAO = PCTRIBUTACAO.CODIGO_TRIBUTACAO AND DTEXCLUSAO IS NULL) 
                            THEN 8
                            ELSE 99
    ...
    RETURN V_SQL_FILTRO;
  END GET_SQL_FILTRO_TRIB_EXATA;
  -----------------------------------------------------------------------------------------
FUNCTION GET_SQL_FILTRO_TRIB_FLEXIVEL(P_PARAMETROS IN TIPO_TRIBUT_REFORMA) RETURN VARCHAR2 IS
  V_SQL_FILTRO VARCHAR2(32767);
  BEGIN
    V_SQL_FILTRO := '
                    AND (
    ...
    RETURN V_SQL_FILTRO;
  END GET_SQL_FILTRO_TRIB_FLEXIVEL;
  -----------------------------------------------------------------------------------------
  ...
  RETURN TIPO_TRIBUT_REFORMA IS
    ...
    -- Se chamada direto na Fiscal, o objeto deverá ser preenchido. 
    IF V_PARAMETROS.TIPO_PESQUISA_TRIBUTACAO IS NULL THEN
      V_PARAMETROS.TIPO_PESQUISA_TRIBUTACAO := PARAMFILIAL.OBTERCOMOVARCHAR2('PESQTRIBUTREFORMA');
    END IF;
    ...
    -- Verifica o modo de pesquisa da tributação definido no parâmetro 4932: F = Pesquisa Flexível | E = Pesquisa Exata.
    V_SQL := GET_SQL_BASE_TRIBUTACAO(P_PARAMETROS         => P_PARAMETROS, 
                                     P_TIPO_LOCAL_CONSUMO => P_TIPO_LOCAL_CONSUMO, 
                                     P_LOCAL_CONSUMO      => P_LOCAL_CONSUMO);

   IF V_PARAMETROS.TIPO_PESQUISA_TRIBUTACAO <> 'F' THEN
       V_SQL := V_SQL || GET_SQL_FILTRO_TRIB_EXATA(P_PARAMETROS => P_PARAMETROS);
    ELSE 
       V_SQL := V_SQL || GET_SQL_FILTRO_TRIB_FLEXIVEL(P_PARAMETROS => P_PARAMETROS);
    END IF;

    EXECUTE IMMEDIATE V_SQL
      INTO V_CODIGO_TRIBUTACAO_CBSIBS,
           V_CODIGO_TRIBUTACAO_REGULAR,
      ...
      IF V_PARAMETROS.FORMULA_VALOR_TRIBUTO_CBS IS NULL OR V_PARAMETROS.FORMULA_VALOR_TRIBUTO_CBS = '' THEN
        V_PARAMETROS.FORMULA_VALOR_TRIBUTO_CBS     := V_BASE_CALCULO_PADRAO;
      END IF;

      IF V_PARAMETROS.FORMULA_VALOR_TRIBUTO_IBS_UF IS NULL OR V_PARAMETROS.FORMULA_VALOR_TRIBUTO_IBS_UF = '' THEN
              V_PARAMETROS.FORMULA_VALOR_TRIBUTO_IBS_UF  := V_BASE_CALCULO_PADRAO;
      END IF;

      IF V_PARAMETROS.FORMULA_VALOR_TRIBUTO_IBS_MUN IS NULL OR V_PARAMETROS.FORMULA_VALOR_TRIBUTO_IBS_MUN = '' THEN
         V_PARAMETROS.FORMULA_VALOR_TRIBUTO_IBS_MUN := V_BASE_CALCULO_PADRAO;
      END IF;

      IF V_PARAMETROS.FORMULA_VALOR_TRIBUTO_IS IS NULL OR V_PARAMETROS.FORMULA_VALOR_TRIBUTO_IS = '' THEN
      ...
      WHEN NO_DATA_FOUND THEN
        BEGIN
          PKG_DEBUGGING_FWPC.LOG('Não foi encontrado nenhum registro na tabela PCTRIBUTACAO com os parâmetros:' || CHR(10)
                               ...
                               ||' P_PARAMETROS.CODPROD:'||' '||P_PARAMETROS.CODPROD || CHR(10) 
                               ||' P_PARAMETROS.NCM:'||' '|| P_PARAMETROS.NCM || CHR(10)
                               ||' P_PARAMETROS.CFOP:'||' '|| P_PARAMETROS.CFOP || CHR(10)
                               ||' V_PARAMETROS.TIPO_PESQUISA_TRIBUTACAO:'||' '|| V_PARAMETROS.TIPO_PESQUISA_TRIBUTACAO 
                                , 'S');
          RETURN FALSE;
        END;
  ...
  RETURN TIPO_TRIBUT_REFORMA IS
    ...
    PKG_DEBUGGING_FWPC.LOG('Tributação Regular - Iniciando consulta na PCTRIBUTACAO:' || CHR(10)
      ...
      FROM (SELECT PCTRIBUTACAO.CODIGO_TRIBUTACAO,
            ...
            WHERE PCTRIBUTACAO.TIPO_IMPOSTO = P_PARAMETROS.TIPO_IMPOSTO
              AND PCTRIBUTACAO.DTEXCLUSAO IS NULL
              AND PCTRIBUTACAO.CODIGO_TRIBUTACAO = P_PARAMETROS.TRIBUTACAO_REGULAR.COD_TRIB_REGULAR
            );

      IF (P_PARAMETROS.TIPO_IMPOSTO = 'CBSIBS') THEN
        V_PARAMETROS.TRIBUTACAO_REGULAR.COD_TRIB_REGULAR      := V_CODIGO_TRIBUTACAO_CBSIBS;
        V_PARAMETROS.TRIBUTACAO_REGULAR.CST_TRIB_REG          := V_CST_CBSIBS;
        V_PARAMETROS.TRIBUTACAO_REGULAR.COD_FORMULA_BASE_TRIB_REG := V_COD_FORMULA_BASE_CBSIBS;
        V_PARAMETROS.TRIBUTACAO_REGULAR.CCLASSTRIB_REG        := V_CCLASSTRIB_CBSIBS;
        V_PARAMETROS.TRIBUTACAO_REGULAR.ALIQ_EFET_REG_IBS_UF  := V_PERC_IBS_UF;
        V_PARAMETROS.TRIBUTACAO_REGULAR.ALIQ_EFET_REG_IBS_MUN := V_PERC_IBS_MUN;
        V_PARAMETROS.TRIBUTACAO_REGULAR.ALIQ_EFET_REG_CBS     := V_PERC_CBS;
  ...
  RETURN TIPO_TRIBUT_REFORMA IS
    ...
    VFORMULA_TRIBUTO_REGULAR  PCFORMULA.FORMULA%TYPE;
    VTVARIAVEIS        FORMULA.TBVARIAVEIS;
    RESULTADO          FORMULA.TBVARIAVEIS;
    ...
    BEGIN
      ...
      V_PARAMETROS.FORMULA_VALOR_TRIBUTO_IS := FORMULA.SubstituiVariaveisF(V_PARAMETROS.FORMULA_VALOR_TRIBUTO_IS,VTVARIAVEIS);
    ...
    PROCEDURE CALCULAR_VALOR_CBSIBS IS
      ...
      V_ALIQ_EFETIVA_CBS_DIFERIM     NUMBER(7,4); -- Variavel temporária para calculo do diferimento
      V_ALIQ_EFETIVA_IBS_UF          NUMBER(7,4);
      V_ALIQ_EFETIVA_IBS_UF_DIFERIM  NUMBER(7,4); -- Variavel temporária para calculo do diferimento
      V_ALIQ_EFETIVA_IBS_MUN         NUMBER(7,4);
      V_ALIQ_EFETIVA_IBS_MUN_DIFERIM NUMBER(7,4); -- Variavel temporária para calculo do diferimento
    BEGIN
      -- Calcular tributo
      ...
      if (V_PARAMETROS.PERC_RED_CBS > 0) then
        V_ALIQ_EFETIVA_CBS := (V_PARAMETROS.PERC_CBS * (1-(V_PARAMETROS.PERC_RED_CBS/100)));
      ...
      IF V_PARAMETROS.COMPRA_GOVERNAMENTAL.TIPO_ORGAOPUBLICO <> '0' AND 
       V_PARAMETROS.COMPRA_GOVERNAMENTAL.PERC_RED_ORGAO_PUB >= 0 THEN
         ...
         -- Reduzindo aliquotas efetivas para tributação com COMPRA_GOVERNAMENTAL
      ...
      -- Reduzindo aliquotas efetivas para tributação com DIFERIMENTO
      IF V_PARAMETROS.DADOS_DIFERIMENTO_CBS.PERC_DIFERIMENTO > 0 THEN 
         ...
         V_ALIQ_EFETIVA_IBS_MUN_DIFERIM := (V_ALIQ_EFETIVA_IBS_MUN * (1-(V_PARAMETROS.DADOS_DIFERIMENTO_CBS.PERC_DIFERIMENTO/100)));
      END IF;
       ...
       VARIAVEL.NOME  := '[ALIQUOTA_IBS_UF]';
       VARIAVEL.VALOR := V_ALIQ_EFETIVA_IBS_UF;
       FORMULA.ATRIBUIVALOR(VARIAVEL, VTVARIAVEIS);

       VARIAVEL.NOME  := '[ALIQUOTA_IBS_MUN]';
       VARIAVEL.VALOR := V_ALIQ_EFETIVA_IBS_MUN;
       FORMULA.ATRIBUIVALOR(VARIAVEL, VTVARIAVEIS);

       V_PARAMETROS.FORMULA_VALOR_TRIBUTO_CBS     := FORMULA.SubstituiVariaveisF(V_PARAMETROS.FORMULA_VALOR_TRIBUTO_CBS,VTVARIAVEIS);
 ...
 -- 
    PROCEDURE CALCULAR_VALOR_TRIBUTO_REGULAR IS
      V_ALIQ_EFET_REG_CBS     NUMBER(7,4);
      V_ALIQ_EFET_REG_IBS_UF  NUMBER(7,4);
      V_ALIQ_EFET_REG_IBS_MUN NUMBER(7,4);
    BEGIN
      -- Calcular tributo
      VFORMULA_TRIBUTO_REGULAR  := FORMULA.SUBSTITUIFORMULAS(P_PARAMETROS.TRIBUTACAO_REGULAR.COD_FORMULA_BASE_TRIB_REG, RESULTADO);
      VFORMULA_TRIBUTO_REGULAR  := FORMULA.BUSCAVALOR(RESULTADO, V_TIPO_IMPOSTO);

      V_PARAMETROS.TRIBUTACAO_REGULAR.FORMULA_BASE_CALCULO_TRIB_REG := VFORMULA_TRIBUTO_REGULAR;

      VARIAVEL.NOME  := '&BASE_CALCULO_CBSIBS&';
      VARIAVEL.VALOR := V_PARAMETROS.TRIBUTACAO_REGULAR.FORMULA_BASE_CALCULO_TRIB_REG;
      FORMULA.ATRIBUIVALOR(VARIAVEL, VTVARIAVEIS);

      V_PARAMETROS.TRIBUTACAO_REGULAR.VALOR_BASE_TRIB_REG := FORMULA.CALCULARSUBFORMULA(VFORMULA_TRIBUTO_REGULAR, VTVARIAVEIS);

      V_ALIQ_EFET_REG_CBS     := V_PARAMETROS.TRIBUTACAO_REGULAR.ALIQ_EFET_REG_CBS;
      ...
      IF V_PARAMETROS.COMPRA_GOVERNAMENTAL.TIPO_ORGAOPUBLICO <> '0' AND 
         ...
         V_PARAMETROS.COMPRA_GOVERNAMENTAL.VALOR_CBS_COMPRA_GOV    := ROUND(((V_PARAMETROS.TRIBUTACAO_REGULAR.VALOR_BASE_TRIB_REG * V_ALIQ_EFET_REG_CBS)/100),10);
         V_PARAMETROS.COMPRA_GOVERNAMENTAL.PERC_IBS_UF_COMPRA_GOV  := V_ALIQ_EFET_REG_IBS_UF;
         V_PARAMETROS.COMPRA_GOVERNAMENTAL.VALOR_IBS_UF_COMPRA_GOV := ROUND(((V_PARAMETROS.TRIBUTACAO_REGULAR.VALOR_BASE_TRIB_REG * V_ALIQ_EFET_REG_IBS_UF)/100),10);
         V_PARAMETROS.COMPRA_GOVERNAMENTAL.PERC_IBS_MUN_COMPRA_GOV := V_ALIQ_EFET_REG_IBS_MUN;
         V_PARAMETROS.COMPRA_GOVERNAMENTAL.VALOR_IBS_MUN_COMPRA_GOV:= ROUND(((V_PARAMETROS.TRIBUTACAO_REGULAR.VALOR_BASE_TRIB_REG * V_ALIQ_EFET_REG_IBS_MUN)/100),10);
      END IF;

       VARIAVEL.NOME  := '[ALIQUOTA_CBS]';
       VARIAVEL.VALOR := V_ALIQ_EFET_REG_CBS;
       FORMULA.ATRIBUIVALOR(VARIAVEL, VTVARIAVEIS);

       VARIAVEL.NOME  := '[ALIQUOTA_IBS_UF]';
       VARIAVEL.VALOR := V_ALIQ_EFET_REG_IBS_UF;
       FORMULA.ATRIBUIVALOR(VARIAVEL, VTVARIAVEIS);

       VARIAVEL.NOME  := '[ALIQUOTA_IBS_MUN]';
       VARIAVEL.VALOR := V_ALIQ_EFET_REG_IBS_MUN;
       FORMULA.ATRIBUIVALOR(VARIAVEL, VTVARIAVEIS);

       V_PARAMETROS.TRIBUTACAO_REGULAR.VTRIB_REG_CBS        := ROUND(((V_PARAMETROS.TRIBUTACAO_REGULAR.VALOR_BASE_TRIB_REG * V_ALIQ_EFET_REG_CBS)/100),10);
       V_PARAMETROS.TRIBUTACAO_REGULAR.VTRIB_REG_IBS_UF     := ROUND(((V_PARAMETROS.TRIBUTACAO_REGULAR.VALOR_BASE_TRIB_REG * V_ALIQ_EFET_REG_IBS_UF)/100),10);
       V_PARAMETROS.TRIBUTACAO_REGULAR.VTRIB_REG_IBS_MUN    := ROUND(((V_PARAMETROS.TRIBUTACAO_REGULAR.VALOR_BASE_TRIB_REG * V_ALIQ_EFET_REG_IBS_MUN)/100),10);
  ...
  BEGIN
    ...
    PKG_DEBUGGING_FWPC.LOG('Inicio do processo de cálculo dos novos tributos','S');
      ...
      IF ((P_PARAMETROS.COD_FORMULA_BASE_CBSIBS IS NOT NULL) AND
         ...
         VARIAVEL.NOME  := '&VLSEGURO&';
         VARIAVEL.VALOR := CASE WHEN P_PARAMETROS.VALOR_SEGURO IS NULL THEN 0 ELSE P_PARAMETROS.VALOR_SEGURO END;
         FORMULA.ATRIBUIVALOR(VARIAVEL, VTVARIAVEIS);

         VARIAVEL.NOME  := '&VLDESCONTO&';
         VARIAVEL.VALOR := CASE WHEN P_PARAMETROS.VALOR_DESCONTO IS NULL THEN 0 ELSE P_PARAMETROS.VALOR_DESCONTO END;
         FORMULA.ATRIBUIVALOR(VARIAVEL, VTVARIAVEIS);

         VARIAVEL.NOME  := '&VLVII&';
         VARIAVEL.VALOR := CASE WHEN P_PARAMETROS.VALOR_VII IS NULL THEN 0 ELSE P_PARAMETROS.VALOR_VII END;
         FORMULA.ATRIBUIVALOR(VARIAVEL, VTVARIAVEIS);

         VARIAVEL.NOME  := '&VLPIS&';
         VARIAVEL.VALOR := CASE WHEN P_PARAMETROS.VALOR_PIS IS NULL THEN 0 ELSE P_PARAMETROS.VALOR_PIS END;
         FORMULA.ATRIBUIVALOR(VARIAVEL, VTVARIAVEIS);

         VARIAVEL.NOME  := '&VLCOFINS&';
         VARIAVEL.VALOR := CASE WHEN P_PARAMETROS.VALOR_COFINS IS NULL THEN 0 ELSE P_PARAMETROS.VALOR_COFINS END;
         FORMULA.ATRIBUIVALOR(VARIAVEL, VTVARIAVEIS);

         VARIAVEL.NOME  := '&VLICMS&';
         VARIAVEL.VALOR := CASE WHEN P_PARAMETROS.VALOR_ICMS IS NULL THEN 0 ELSE P_PARAMETROS.VALOR_ICMS END;
         FORMULA.ATRIBUIVALOR(VARIAVEL, VTVARIAVEIS);

         VARIAVEL.NOME  := '&VLICMSUFDEST&';
         VARIAVEL.VALOR := CASE WHEN P_PARAMETROS.VALOR_ICMSUFDEST IS NULL THEN 0 ELSE P_PARAMETROS.VALOR_ICMSUFDEST END;
         FORMULA.ATRIBUIVALOR(VARIAVEL, VTVARIAVEIS);
         ...
         IF (P_PARAMETROS.TIPO_IMPOSTO = 'CBSIBS') THEN
          ...
          IF NVL(P_PARAMETROS.TRIBUTACAO_REGULAR.COD_TRIB_REGULAR,0) > 0 AND 
              NVL(TO_NUMBER(P_PARAMETROS.TRIBUTACAO_REGULAR.CCLASSTRIB_REG),0) > 0 THEN 
  ...
  RETURN TIPO_TRIBUT_REFORMA IS
    ...
    V_UF               VARCHAR2(2);
    V_TIPO_ORGAOPUBLICO VARCHAR2(2);
    V_PERC_RED_ORG_PUB NUMBER(7,4);
  ...
  BEGIN
    BEGIN
      ...
      IF NOT FIL_OPTANTESIMPLESNACIONAL(P_PARAMETROS.CODFILIAL) THEN 
        ...
        PKG_DEBUGGING_FWPC.LOG('Finalizando o processo de cálculo CBS com os seguintes dados:' || CHR(10) ||
  ...
  BEGIN
    BEGIN
      ...
      IF NOT FIL_OPTANTESIMPLESNACIONAL(P_PARAMETROS.CODFILIAL) THEN     
        ...
        PKG_DEBUGGING_FWPC.LOG('Finalizando o processo de cálculo IS com os seguintes dados:'|| CHR(10) ||
                               ' Código Tributação: '||V_DADOS_TRIBUTACAO.CODIGO_TRIBUTACAO_IS|| CHR(10) ||
                               ' Código da Base de Cálculo: '||V_DADOS_TRIBUTACAO.COD_FORMULA_BASE_CALCULO_IS|| CHR(10) ||
                               ' CST: '||V_DADOS_TRIBUTACAO.CST_IS|| CHR(10) ||
                               ' CClassTrib: '||V_DADOS_TRIBUTACAO.CCLASSTRIB_IS|| CHR(10) ||
  ...
  BEGIN
    ...
    IF P_USASUFIXO = 'S' THEN
        V_DADOS_TRIBUTACAO.FORMULA_VALOR_TRIBUTO_CBS     := '(&BASE_CALCULO_CBSIBS& * [ALIQUOTA_CBS] / 100)';
        V_DADOS_TRIBUTACAO.FORMULA_VALOR_TRIBUTO_IBS_UF  := '(&BASE_CALCULO_CBSIBS& * [ALIQUOTA_IBS_UF] / 100)';
        V_DADOS_TRIBUTACAO.FORMULA_VALOR_TRIBUTO_IBS_MUN := '(&BASE_CALCULO_CBSIBS& * [ALIQUOTA_IBS_MUN] / 100)';
    END IF;
...
END;
-- Alteração 16/07/2026 - Implementado parametro para definição da pesquisa da tributação - v
