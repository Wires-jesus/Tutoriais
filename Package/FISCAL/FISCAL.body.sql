...
--RATEIO OUTRAS DESPESAS E FRETE (ILQUIAS 20/07/2011)
 ...
 FUNCTION GET_DESCRICAO_NATUREZA_OP(
  ...
  FUNCTION GET_DADOS_NOVOS_TRIBUTOS(P_PARAMETROS in TIPO_TRIBUT_REFORMA,  
                                        P_MSG      out varchar2)
    ...
    BEGIN    
      SELECT NVL(PCTRIBUTACAO_EXCECAO.BASE_CALCULO, PCTRIBUTACAO.BASE_CALCULO) BASE_CALCULO,
       ...
       WHERE PCTRIBUTACAO.CODIGO_TRIBUTACAO = PCTRIBUTACAO_EXCECAO.CODIGO_TRIBUTACAO(+)
         ...
         AND LOCAL_CONSUMO = TO_CHAR(P_PARAMETROS.CODIGO_MUNICIPIO)
         AND STATUS = 'A'
  ...
  BEGIN
    ...
    IF V_PARAMETROS.BASE_CALCULO_COD_FORMULA IS NOT NULL THEN
        ...
        VARIAVEL.NOME  := '[VALOR_ICMS_DESONERADO]';
        VARIAVEL.VALOR := CASE WHEN P_PARAMETROS.VALOR_ICMS_DESONERADO IS NULL THEN 0 ELSE P_PARAMETROS.VALOR_ICMS_DESONERADO END;
        FORMULA.ATRIBUIVALOR(VARIAVEL, VTVARIAVEIS);
        ...
        VARIAVEL.NOME  := '[VALOR_OUTRAS_DESPESAS]';
        VARIAVEL.VALOR := CASE WHEN P_PARAMETROS.VALOR_OUTRAS_DESPESAS IS NULL THEN 0 ELSE P_PARAMETROS.VALOR_OUTRAS_DESPESAS END;
        FORMULA.ATRIBUIVALOR(VARIAVEL, VTVARIAVEIS);
                
        VARIAVEL.NOME  := '[VALOR_DESCONTO]';
