 ...
 FUNCTION GET_DESCRICAO_NATUREZA_OP(
  ...
  BEGIN
    ...
    BEGIN    
      ...
      SELECT CODIGO_TRIBUTACAO,
        ...
        FROM (
          SELECT PCTRIBUTACAO.CODIGO_TRIBUTACAO,
            ...
            FROM PCTRIBUTACAO
            LEFT JOIN PCTRIBUTACAO_EXCECAO
              ON PCTRIBUTACAO.CODIGO_TRIBUTACAO = PCTRIBUTACAO_EXCECAO.CODIGO_TRIBUTACAO
            LEFT JOIN PCTRIBUTACAO_FILTRO_PRODUTO
              ON PCTRIBUTACAO.CODIGO_TRIBUTACAO = PCTRIBUTACAO_FILTRO_PRODUTO.CODIGO_TRIBUTACAO
            LEFT JOIN PCTRIBUTACAO_FILTRO_NCM
              ON PCTRIBUTACAO.CODIGO_TRIBUTACAO = PCTRIBUTACAO_FILTRO_NCM.CODIGO_TRIBUTACAO
           WHERE TIPO_IMPOSTO = P_PARAMETROS.TIPO_IMPOSTO
             AND LOCAL_CONSUMO = P_PARAMETROS.CODIGO_MUNICIPIO
             ...
             AND NVL(PCTRIBUTACAO_EXCECAO.CONSUMIDOR_FINAL, NVL(PCTRIBUTACAO.CONSUMIDOR_FINAL, 'N')) IN (NVL(P_PARAMETROS.CONSUMIDOR_FINAL,'N'), 'N')
             AND NVL(PCTRIBUTACAO_EXCECAO.TIPO_EMPRESA, NVL(PCTRIBUTACAO.TIPO_EMPRESA, 'N')) IN (NVL(P_PARAMETROS.TIPO_EMPRESA,'N'), 'N')
             AND NVL(PCTRIBUTACAO_EXCECAO.TIPO_PESSOA, NVL(PCTRIBUTACAO.TIPO_PESSOA, 'N')) IN (NVL(P_PARAMETROS.TIPO_PESSOA,'N'), 'N')
             AND NVL(PCTRIBUTACAO_EXCECAO.ORGAO_PUBLICO, NVL(PCTRIBUTACAO.ORGAO_PUBLICO, 'N')) IN (NVL(P_PARAMETROS.ORGAO_PUBLICO,'N'), 'N')
             AND NVL(PCTRIBUTACAO_EXCECAO.ORIGEM_MERCADORIA, NVL(PCTRIBUTACAO.ORIGEM_MERCADORIA, 'N')) IN (NVL(P_PARAMETROS.ORIGEM_MERCADORIA,'N'), 'N')
             AND NVL(PCTRIBUTACAO_EXCECAO.TIPO_MERC, NVL(PCTRIBUTACAO.TIPO_MERC, 'N')) IN (NVL(P_PARAMETROS.TIPO_MERC,'N'), 'N')
           ORDER BY
             CASE WHEN NVL(PCTRIBUTACAO_EXCECAO.CONSUMIDOR_FINAL, NVL(PCTRIBUTACAO.CONSUMIDOR_FINAL,'N')) = NVL(P_PARAMETROS.CONSUMIDOR_FINAL,'N') THEN 1 ELSE 2 END,
             CASE WHEN NVL(PCTRIBUTACAO_EXCECAO.TIPO_EMPRESA, NVL(PCTRIBUTACAO.TIPO_EMPRESA,'N')) = NVL(P_PARAMETROS.TIPO_EMPRESA,'N') THEN 1 ELSE 2 END,
             CASE WHEN NVL(PCTRIBUTACAO_EXCECAO.TIPO_PESSOA, NVL(PCTRIBUTACAO.TIPO_PESSOA,'N')) = NVL(P_PARAMETROS.TIPO_PESSOA,'N') THEN 1 ELSE 2 END,
             CASE WHEN NVL(PCTRIBUTACAO_EXCECAO.ORGAO_PUBLICO, NVL(PCTRIBUTACAO.ORGAO_PUBLICO,'N')) = NVL(P_PARAMETROS.ORGAO_PUBLICO,'N') THEN 1 ELSE 2 END,
             CASE WHEN NVL(PCTRIBUTACAO_EXCECAO.ORIGEM_MERCADORIA, NVL(PCTRIBUTACAO.ORIGEM_MERCADORIA,'N')) = NVL(P_PARAMETROS.ORIGEM_MERCADORIA,'N') THEN 1 ELSE 2 END,
             CASE WHEN NVL(PCTRIBUTACAO_EXCECAO.TIPO_MERC, NVL(PCTRIBUTACAO.TIPO_MERC,'N')) = NVL(P_PARAMETROS.TIPO_MERC,'N') THEN 1 ELSE 2 END,
      ...
      PKG_DEBUGGING_FWPC.LOG('OK; Tributação localizada na rotina 4000 pelo seguinte local de consumo: '||P_PARAMETROS.CODIGO_MUNICIPIO||'.'||
                             ' V_CODIGO_TRIBUTACAO: '||V_CODIGO_TRIBUTACAO||
                             ' V_BASE_CALCULO_FORMULA: '||V_BASE_CALCULO_FORMULA||
                             ' V_ALIQUOTA_FORMULA: '||V_ALIQUOTA_FORMULA||
                             ' V_VALOR_ALIQUOTA: '||V_VALOR_ALIQUOTA||
                             ' V_CLASSTRIB: '||V_CLASSTRIB||
                             ' V_SOMATOTALNF: '||V_SOMATOTALNF,
                             'S');
    EXCEPTION
      WHEN NO_DATA_FOUND THEN
        BEGIN                  
          ...
          SELECT CODIGO_TRIBUTACAO,
           ...
           WHERE ROWNUM = 1;
             ...
             PKG_DEBUGGING_FWPC.LOG('OK; Tributação localizada na rotina 4000 pelo seguinte local de consumo UF: '||V_UF||' .'||
                                    ' V_CODIGO_TRIBUTACAO: '||V_CODIGO_TRIBUTACAO||
                                    ' V_BASE_CALCULO_FORMULA: '||V_BASE_CALCULO_FORMULA||
                                    ' V_ALIQUOTA_FORMULA: '||V_ALIQUOTA_FORMULA||
                                    ' V_VALOR_ALIQUOTA: '||V_VALOR_ALIQUOTA||
            ...
            WHEN NO_DATA_FOUND THEN              
              ...
              SELECT CODIGO_TRIBUTACAO,
               ...
               WHERE ROWNUM = 1;
                 ...
                 PKG_DEBUGGING_FWPC.LOG('OK; Tributação localizada na rotina 4000 pelo seguinte local de consumo Nação: BR'||
                             ' V_CODIGO_TRIBUTACAO: '||V_CODIGO_TRIBUTACAO||
                             ' V_BASE_CALCULO_FORMULA: '||V_BASE_CALCULO_FORMULA||
                             ' V_ALIQUOTA_FORMULA: '||V_ALIQUOTA_FORMULA||
                             ' V_VALOR_ALIQUOTA: '||V_VALOR_ALIQUOTA||
    ...
    V_PARAMETROS.SOMATOTALNF              := V_SOMATOTALNF;
  ...
  BEGIN
    ...
    IF V_PARAMETROS.FORMULA_VALOR_TRIBUTO IS NULL OR V_PARAMETROS.FORMULA_VALOR_TRIBUTO = '' THEN
       V_PARAMETROS.FORMULA_VALOR_TRIBUTO := '(&BASE_CALCULO& * [ALIQUOTA])';
    END IF;    
  ...
  BEGIN
    ...
    IF P_USASUFIXO = 'S' THEN
        V_DADOS_TRIBUTACAO.FORMULA_VALOR_TRIBUTO := '(&BASE_CALCULO_CBS& * [ALIQUOTA_CBS] / 100)';  
    END IF;
