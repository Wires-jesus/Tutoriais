  ...
  RETURN TIPO_TRIBUT_REFORMA IS
    ...
    BEGIN
      ...
      IF V_PARAMETROS.COMPRA_GOVERNAMENTAL.TIPO_ORGAOPUBLICO IS NOT NULL AND V_PARAMETROS.COMPRA_GOVERNAMENTAL.PERC_RED_ORGAO_PUB > 0 THEN
         V_PARAMETROS.ALIQ_EFETIVA_CBS     := (V_PARAMETROS.ALIQ_EFETIVA_CBS     * (1-(V_PARAMETROS.COMPRA_GOVERNAMENTAL.PERC_RED_ORGAO_PUB/100)));
         ...
         V_PARAMETROS.ALIQ_EFETIVA_IBS_MUN := (V_PARAMETROS.ALIQ_EFETIVA_IBS_MUN * (1-(V_PARAMETROS.COMPRA_GOVERNAMENTAL.PERC_RED_ORGAO_PUB/100)));
      END IF;

       VARIAVEL.NOME  := '[ALIQUOTA_CBS]';
       VARIAVEL.VALOR := V_PARAMETROS.ALIQ_EFETIVA_CBS;
       FORMULA.ATRIBUIVALOR(VARIAVEL, VTVARIAVEIS);

       VARIAVEL.NOME  := '[ALIQUOTA_IBS_UF]';
       VARIAVEL.VALOR := V_PARAMETROS.ALIQ_EFETIVA_IBS_UF;
       FORMULA.ATRIBUIVALOR(VARIAVEL, VTVARIAVEIS);

       VARIAVEL.NOME  := '[ALIQUOTA_IBS_MUN]';
       VARIAVEL.VALOR := V_PARAMETROS.ALIQ_EFETIVA_IBS_MUN;
       FORMULA.ATRIBUIVALOR(VARIAVEL, VTVARIAVEIS);
