    ...
    BEGIN    
      ...
      SELECT PCTRIBUTACAO.CODIGO_TRIBUTACAO,             
        ...
        FROM PCTRIBUTACAO,
             ...
             PCTRIBUTACAO_FILTRO_PRODUTO,
             PCTRIBUTACAO_FILTRO_NCM
       WHERE PCTRIBUTACAO.CODIGO_TRIBUTACAO = PCTRIBUTACAO_EXCECAO.CODIGO_TRIBUTACAO(+)       
         AND PCTRIBUTACAO.CODIGO_TRIBUTACAO = PCTRIBUTACAO_FILTRO_PRODUTO.CODIGO_TRIBUTACAO(+)
         AND PCTRIBUTACAO.CODIGO_TRIBUTACAO = PCTRIBUTACAO_FILTRO_NCM.CODIGO_TRIBUTACAO(+)      
         AND TIPO_IMPOSTO  = P_PARAMETROS.TIPO_IMPOSTO
         AND LOCAL_CONSUMO = P_PARAMETROS.CODIGO_MUNICIPIO
         AND DTEXCLUSAO IS NULL  
         AND (NVL(PCTRIBUTACAO.TIPO_OPERACAO, NULL) IS NULL OR NVL(PCTRIBUTACAO.TIPO_OPERACAO, NULL) = P_PARAMETROS.TIPO_OPERACAO)         
         AND (NVL(PCTRIBUTACAO_FILTRO_PRODUTO.CODPROD,0) = 0 OR NVL(PCTRIBUTACAO_FILTRO_PRODUTO.CODPROD,0) = P_PARAMETROS.CODPROD)
         AND (NVL(PCTRIBUTACAO_FILTRO_NCM.NCM, NULL) IS NULL OR NVL(PCTRIBUTACAO_FILTRO_NCM.NCM, NULL) = P_PARAMETROS.NCM)
         AND (NVL(PCTRIBUTACAO_EXCECAO.CONSUMIDOR_FINAL, NVL(PCTRIBUTACAO.CONSUMIDOR_FINAL,'N')) = 'N' OR NVL(PCTRIBUTACAO_EXCECAO.CONSUMIDOR_FINAL, NVL(PCTRIBUTACAO.CONSUMIDOR_FINAL,'N')) = P_PARAMETROS.CONSUMIDOR_FINAL)
         AND (NVL(PCTRIBUTACAO_EXCECAO.TIPO_EMPRESA, NVL(PCTRIBUTACAO.TIPO_EMPRESA,'N')) = 'N' OR NVL(PCTRIBUTACAO_EXCECAO.TIPO_EMPRESA, NVL(PCTRIBUTACAO.TIPO_EMPRESA,'N')) = P_PARAMETROS.TIPO_EMPRESA)
         AND (NVL(PCTRIBUTACAO_EXCECAO.TIPO_PESSOA, NVL(PCTRIBUTACAO.TIPO_PESSOA,'N')) = 'N' OR NVL(PCTRIBUTACAO_EXCECAO.TIPO_PESSOA, NVL(PCTRIBUTACAO.TIPO_PESSOA,'N')) = P_PARAMETROS.TIPO_PESSOA)
         AND (NVL(PCTRIBUTACAO_EXCECAO.ORGAO_PUBLICO, NVL(PCTRIBUTACAO.ORGAO_PUBLICO,'N')) = 'N' OR NVL(PCTRIBUTACAO_EXCECAO.ORGAO_PUBLICO, NVL(PCTRIBUTACAO.ORGAO_PUBLICO,'N')) = P_PARAMETROS.ORGAO_PUBLICO)
         AND (NVL(PCTRIBUTACAO_EXCECAO.ORIGEM_MERCADORIA, NVL(PCTRIBUTACAO.ORIGEM_MERCADORIA,'N')) = 'N' OR NVL(PCTRIBUTACAO_EXCECAO.ORIGEM_MERCADORIA, NVL(PCTRIBUTACAO.ORIGEM_MERCADORIA,'N')) = P_PARAMETROS.ORIGEM_MERCADORIA)
         AND (NVL(PCTRIBUTACAO_EXCECAO.TIPO_MERC, NVL(PCTRIBUTACAO.TIPO_MERC,'N')) = 'N' OR NVL(PCTRIBUTACAO_EXCECAO.TIPO_MERC, NVL(PCTRIBUTACAO.TIPO_MERC,'N')) = P_PARAMETROS.TIPO_MERC)   
         AND ROWNUM = 1;                   
  ...
  BEGIN
    ...
    IF V_PARAMETROS.CODCLI > 0 THEN
      BEGIN
        ...
        PKG_DEBUGGING_FWPC.LOG('OK: Encontrado código de municipio '||V_CODIGO_MUNICIPIO||' para o codcli: '||V_PARAMETROS.CODCLI,'S');
      EXCEPTION
        WHEN NO_DATA_FOUND THEN
        BEGIN                     
    ...
    IF V_PARAMETROS.CODFORNEC > 0 THEN       
      BEGIN
        ...
        PKG_DEBUGGING_FWPC.LOG('OK: Encontrado dados para o fornecedor codfornec: '||V_PARAMETROS.CODFORNEC,'S');        
      EXCEPTION
        WHEN NO_DATA_FOUND THEN
        BEGIN  
          PKG_DEBUGGING_FWPC.LOG('ERRO: Não encontrado para o fornecedor codfornec: '||V_PARAMETROS.CODFORNEC||' '||SQLERRM,'S');          
        END;  
        WHEN OTHERS THEN
        BEGIN            
