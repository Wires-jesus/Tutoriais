     ...
     USING (
         ...
         FROM (
             ...
             ROWID RID,
				   ...
				   ROW_NUMBER() OVER (PARTITION BY SEQFAMILIA, NRODIVISAO ORDER BY NROCARGA DESC) RN
...
PROCEDURE carrega_tb_regraincentivo(p_id IN pccontroleconsinco.id%TYPE) AS
  BEGIN
   ...
   WHERE IDREF IN (SELECT NVL(L.CODFILIAL,'99')||561||L.CODDESCONTO  
                       ...
                       WHERE TRUNC(SYSDATE) BETWEEN L.DTINICIO AND L.DTFIM
					   UNION ALL
    				SELECT NVL(D.CODFILIAL,'99')||561||D.CODDESCONTO 
                      FROM PCDESCONTO D
                     WHERE D.DTFIM < TRUNC(SYSDATE))
...
BEGIN
  UPDATE MONITORPDVMIDDLE.TB_CLIENTECREDITO CL
     SET CL.ATIVO = 'N'
   WHERE CL.ATIVO = 'S'
     AND EXISTS (SELECT 1 FROM PCCLIENT C, PCFINALIZADORA F,
                (SELECT S.ULTIMAEXECUCAO FROM PCCONTROLECONSINCO S
                   WHERE UPPER(S.OBJETOREFERENCIA) = 'PKG_SINC_PDV_CONSINCO.CARREGA_TB_CLIENTECREDITO') DTPADRAO
           WHERE CL.SEQPESSOA = C.CODCLI
             AND CL.NROFORMAPAGTO = F.CODFINALIZADORA
             AND C.CODCLI NOT IN (1, 2, 3)
             AND F.VERIFICALIMITE = 'N'
...
BEGIN
  ...
  WHERE CCT.ATIVO = 'S'
	...
	OR CCT.SEQCENARIO IN (SELECT SEQCENARIO FROM PCDEPARATRIBUTACAOC5 DEPARA, PCTRIBUTACAO T WHERE T.CODIGO_TRIBUTACAO = DEPARA.CODIGO_TRIBUTACAO AND UPPER(T.TIPO_DOCUMENTO) NOT LIKE '%CBNFCE%'));
...
BEGIN
  ...
  SET PONTOSBUSCA = (SELECT NVL(SUM(PONTOSBUSCA), 101) 
                     FROM MONITORPDVMIDDLE.tb_cctcenariocondicao CC 
                     WHERE CC.SEQCENARIO = C.SEQCENARIO 
                       AND CC.SEQCONDICAO IN (1, 3, 4, 8) 
                       AND CC.PONTOSBUSCA <> 1)
  WHERE C.ATIVO = 'S';
...
BEGIN
  ...
  WHEN MATCHED THEN
    ...
    WHERE NVL(CC.ATIVO, '-') <> NVL(S.ATIVO, '-')
       ...
       OR NVL(CC.INDISENTO, '-') <> NVL(S.INDISENTO, '-')
...
BEGIN
  ...
  WHEN MATCHED THEN
    ...
    WHERE NVL(CC.ATIVO, '-') <> NVL(S.ATIVO, '-')
  WHEN NOT MATCHED THEN
    INSERT (SEQCENARIO, PRODUTO, NCM, FAMILIA, CGO, ORIGEM, DESTINO, INDTIPOENTIDADEORIGEM, INDTIPOENTIDADEDESTINO, NBS, ATIVO)
    VALUES (S.SEQCENARIO, S.PRODUTO, S.NCM, S.FAMILIA, S.CGO, S.ORIGEM, S.DESTINO, S.INDTIPOENTIDADEORIGEM, S.INDTIPOENTIDADEDESTINO, S.NBS, S.ATIVO);
