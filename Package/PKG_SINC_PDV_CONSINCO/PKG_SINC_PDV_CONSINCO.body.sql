...
BEGIN
  ...
  INSERT INTO PCDEVLOGCONSINCO  (dv_name, dv_message, dv_message_2, dv_date, dv_timestamp)
  VALUES ('pkg_sinc_PDV_Consinco', 'CARREGA_TB_PRECOAPARTIRGRUPOPESSOA', 'CARREGA_TB_PRECOAPARTIRGRUPOPESSOA OK', SYSDATE, CURRENT_TIMESTAMP);
	  ...
	  BEGIN
        ...
        VALUES
          ('pkg_sinc_PDV_Consinco',
           ...
           'CARREGA_TB_PRECOAPARTIRGRUPOPESSOA ALERTA',
           SYSDATE,
           CURRENT_TIMESTAMP);
        COMMIT;
	  END;
 ...
 UPDATE MONITORPDVMIDDLE.TB_LIMITEVENDAFAMILIA
  ...
  MERGE INTO MONITORPDVMIDDLE.TB_LIMITEVENDAFAMILIA TB_LIMITEVENDAFAMILIA
    USING (SELECT L.SEQFAMILIA,
				   ...
				   (NVL(MAX(L.QTMAXVENDA), 0) * NVL(MAX(L.QTUNIT), 1)) QTDLIMITE, 
				   CASE
					 WHEN (NVL(MAX(L.QTMAXVENDA), 0) * NVL(MAX(L.QTUNIT), 1)) > 0 THEN
					  'S'
					 ELSE
					  'N'
				   END ATIVO
