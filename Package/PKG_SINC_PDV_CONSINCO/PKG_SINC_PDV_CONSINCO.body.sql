...
BEGIN
  ...
  WHERE TB_COMBOITEM.ROWID IN (
    SELECT 
      T.ROWID
    FROM MONITORPDVMIDDLE.TB_COMBOITEM T
    LEFT JOIN VW_INT_C5_BRINDE_ITENS V
    ON (T.SEQCOMBO = V.SEQCOMBO AND T.SEQITEM = V.SEQITEM)
    WHERE
      V.SEQITEM IS NULL 
      AND T.SEQCOMBO IN (SELECT SEQCOMBO FROM VW_INT_C5_BRINDE_ITENS)
      AND T.ATIVO = 'S'
  );
