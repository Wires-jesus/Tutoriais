...
BEGIN
  ...
  WHERE 
    T.ATIVO = 'S'
    AND T.SEQPRECOAPARTIR = 1;

  MERGE INTO MONITORPDVMIDDLE.TB_PRECOAPARTIRPESSOA TB_PRECOAPARTIRPESSOA
    USING (SELECT SEQPRECOAPARTIR, SEQPESSOA, ATIVO FROM VW_INT_C5_PRECOAPARTIRPESSOA) T
    ON (TB_PRECOAPARTIRPESSOA.SEQPRECOAPARTIR = T.SEQPRECOAPARTIR)
  WHEN MATCHED THEN
    UPDATE SET
      TB_PRECOAPARTIRPESSOA.SEQPESSOA = T.SEQPESSOA,
...
BEGIN
  ...
  SET ATIVO = 'N'
  WHERE SEQPRECOAPARTIR = 1
  AND ATIVO = 'S';

  MERGE INTO MONITORPDVMIDDLE.TB_PRECOAPARTIREMPRESA TB_PRECOAPARTIREMPRESA
    USING (SELECT SEQPRECOAPARTIR, NROEMPRESA, ATIVO FROM VW_INT_C5_PRECOAPARTIREMPRESA) T
    ON (TB_PRECOAPARTIREMPRESA.SEQPRECOAPARTIR = T.SEQPRECOAPARTIR)
  WHEN MATCHED THEN
    UPDATE SET
      TB_PRECOAPARTIREMPRESA.NROEMPRESA = T.NROEMPRESA, 
...
PROCEDURE CARREGA_TB_PRECOAPARTIRSEGMENTO(P_ID IN PCCONTROLECONSINCO.ID%TYPE) AS
BEGIN
  MERGE INTO MONITORPDVMIDDLE.TB_PRECOAPARTIRSEGMENTO TB_PRECOAPARTIRSEGMENTO
    USING (SELECT SEQPRECOAPARTIR, NROSEGMENTO, ATIVO FROM VW_INT_C5_PRECOAPARTIRSEGMENTO) T
    ON (TB_PRECOAPARTIRSEGMENTO.SEQPRECOAPARTIR = T.SEQPRECOAPARTIR)
  WHEN MATCHED THEN
    UPDATE SET
      TB_PRECOAPARTIRSEGMENTO.NROSEGMENTO = T.NROSEGMENTO, 
...
BEGIN
  MERGE INTO MONITORPDVMIDDLE.TB_PRECOAPARTIRPERIODO TB_PRECOAPARTIRPERIODO
    USING (SELECT SEQPRECOAPARTIR, DTAHORINICIO, DTAHORFIM, ATIVO FROM VW_INT_C5_PRECOAPARTIRPERIODO) T
