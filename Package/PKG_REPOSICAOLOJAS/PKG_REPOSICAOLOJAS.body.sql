  ...
  ******************************************************************************/
        ...
        ****************/
        ...
        IF (B_AGENDA_VALIDA) THEN
          ...
          ELSE
            ...
            IF (NVL(V_BLOQUEIAVENDAESTPENDENTE,'N') = 'S') THEN                   
              V_QTD_EST_DEST := V_QTD_EST_DEST - NVL(PRODUTO.QTPENDENTE_D,0);
            END IF;
            ...
            FOR viIdxLote IN vtLotes.FIRST..vtLotes.LAST LOOP
              ...
              IF (NVL(vUTILIZAVENDAPOREMBALAGEM,'N') <> 'S') and (N_QTD_SUGERIDA_D > 0) THEN
                ...
                IF vVALIDARMULTIPLOVENDA = 'S' AND vAPLICARVALIDACAOMULTIPLO = 'S' THEN
                  ...
                  if (NVL(N_QTD_TRANSFERIR_O,0) - MOD(NVL(N_QTD_TRANSFERIR_O,0),NVL(vMULTIPLOPRODUTO,0))) < N_QTD_TRANSFERIR_O then
                    N_QTD_TRANSFERIR_O := (NVL(N_QTD_TRANSFERIR_O,0) - MOD(NVL(N_QTD_TRANSFERIR_O,0),NVL(vMULTIPLOPRODUTO,0))) + NVL(vMULTIPLOPRODUTO,0);
                  else
                    N_QTD_TRANSFERIR_O := (NVL(N_QTD_TRANSFERIR_O,0) - MOD(NVL(N_QTD_TRANSFERIR_O,0),NVL(vMULTIPLOPRODUTO,0)));
                  end if;
  ...
  BEGIN
    ...
    */
    ...
    Para o processo de Rota de Abastecimento de Compra
    *****************************************************/	
	IF (NVL(P_AUTOMATICO_MANUAL, 'M') IN ('A', 'M')) AND (P_TIPO_SUGESTAO = 'R') THEN
      ...
      ' INSERT INTO PCMED_TEMP_TRANSF_ATAC_VAR
		...
		SELECT     
			DADOSFILIAL_O.NUMSUGESTAOREP
		  , DADOSFILIAL_O.DTGERACAO
		  , DADOSFILIAL_O.CODFILIAL_O
		  , DADOSFILIAL_O.CODPROD_O
		  , DADOSFILIAL_O.DESCRICAO     
		  , DADOSFILIAL_O.CODMARCA_O
		  , DADOSFILIAL_O.MARCA
		  , DADOSFILIAL_O.QTUNITCX_O
		  , DADOSFILIAL_O.QTD_EST_O
		  , DADOSFILIAL_O.QTD_SUGERIDA_O
		  , DADOSFILIAL_O.QTD_TRANSFERIR_O          
		  , DADOSFILIAL_O.CODFILIAL_D  
		  , DADOSFILIAL_O.PRIORIDADE_D    
		  , PRO_D.CODPROD CODPROD_D
		  , PRO_D.DESCRICAO DESCRICAO_D
		  , ''E'' TIPOSUGESTAO_D
		  , ''E - Estoque Mín/Máx'' DESCTIPOSUGESTAO_D
		  , NVL(EST_D.ESTMIN, 0) ESTOQUEMIN_D
		  , NVL(EST_D.ESTMAX, 0) ESTOQUEMAX_D
		  , NVL(EST_D.QTESTGER, 0) QTESTGER_D
		  , NVL(EST_D.QTPEDIDA, 0) QTPEDIDA_D
		  , NVL(EST_D.QTTRANSITO, 0) QTD_TRANSITO_D
		  , NVL(EST_D.QTBLOQUEADA, 0) - NVL(EST_D.QTINDENIZ, 0) QTD_BLOQSEMAVARIA_D
		  , PRO_D.CLASSE CLASSE_D
		  , (CASE WHEN NVL(REG_D.REGIAOZFM, ''N'') = ''N'' THEN NVL(PRO_D.PRECOMAXCONSUM, 0) ELSE NVL(PRO_D.PRECOMAXCONSUMZFM, 0) END) PMC_D
		  , NVL(EST_D.CUSTOULTENT, 0) CUSTOULTENT_D  
		  , NVL(EST_D.CUSTOULTENT, 0) PVENDA_D  
		  , 0 SUG_FRACIONADA_D
		  , PRO_D.QTUNITCX QTUNITCX_D
		  , NVL(PFL_D.PERCARREDONDA,0) PERCARREDONDA_D
		  , DADOSFILIAL_O.QTD_SUGERIDA_O QTD_SUGERIDA_D
		  , DADOSFILIAL_O.QTD_TRANSFERIR_O QTD_TRANSFERIR_D
		  , (NVL(DADOSFILIAL_O.QTD_SUGERIDA_O, 0) * NVL(DADOSFILIAL_O.QTD_TRANSFERIR_O, 0)) VLR_TRANSFERIR_D 
		  , ''XX'' CODEQUIPE
		  , DADOSFILIAL_O.CODCATEGORIA
		  , DADOSFILIAL_O.CODSEC
		  , DADOSFILIAL_O.CODEPTO
		  , DADOSFILIAL_O.CODFORNEC
		  , DADOSFILIAL_O.EMBALAGEM
		  , DADOSFILIAL_O.UNIDADE
		  , (CASE WHEN TAB_D.CODST > 0 THEN ''S'' ELSE ''N'' END) TRIBUTACAO
		  , TAB_D.CODST 
		  , EST_D.QTVENDMES QTVENDMES_D
		  , EST_D.QTVENDMES1 QTVENDMES1_D
		  , EST_D.QTVENDMES2 QTVENDMES2_D
		  , EST_D.QTVENDMES3 QTVENDMES3_D
		  , DADOSFILIAL_O.CODFAB_O
		  , PRO_D.CODFAB CODFAB_D
		  , DADOSFILIAL_O.CODAUXILIAR_O 
		  , PRO_D.CODAUXILIAR CODAUXILIAR_D
		  , ''0'' QUEBRA1
		  , ''N'' QUEBRA2
		  , ''0'' QUEBRA3
		  , ''C'' QUEBRA4
		  , EST_D.QTGIRODIA QTGIRODIA_D
		  , NVL(EST_D.ESTIDEAL, 0) QTDIASREP_D
		  , EST_D.QTPENDENTE QTPENDENTE_D
		  , (CASE WHEN FIL_D.CONSIDERAESTPENDSUGCOMPRA = ''S'' THEN ''Sim'' ELSE ''Não'' END) DESCCONSIDERAESTPENDSUG_D
		  , DADOSFILIAL_O.PRAZOENTREGA_O
		  , 0 PERCARREDONDA_O
		  , 0 CXFORNEC_TRANSFERIR_O
		  , ''R'' TIPO_SUG_LOJA_CD
		  , ''T'' TIPO_SUG_COMPRA_TRANSF
		  , ''Transferência'' DESC_TIPO_SUG_COMPRA_TRANSF
		  , 0 CODFORNECPRIORIDADE
		  , '''' DESC_FORNECPRIORIDADE
		  , 0 CODDESC_FORNECPRIORIDADE
		  , 0 QTSUGCOMPRA_D
		  , 0 QTIMPORT_D
		  , (CASE WHEN NVL(DADOSFILIAL_O.QTUNITCX_O, 0) > 1 THEN ''S'' ELSE ''N'' END) EMB_POR_FORNECEDOR_O  
		  , CASE CLI_D.TIPOCUSTOTRANSF
			  WHEN ''E'' THEN NVL(DADOSFILIAL_O.N_CUSTOREP_O, 0)
			  WHEN ''R'' THEN NVL(DADOSFILIAL_O.N_CUSTOREAL_O, 0)
			  WHEN ''C'' THEN NVL(DADOSFILIAL_O.N_CUSTOCONT_O, 0)
			  WHEN ''F'' THEN NVL(DADOSFILIAL_O.N_CUSTOFIN_O, 0)
			  WHEN ''U'' THEN NVL(DADOSFILIAL_O.N_CUSTOULTENT_O, 0)
			  WHEN ''V'' THEN NVL(DADOSFILIAL_O.N_VALORULTENT_O, 0)
			  WHEN ''S'' THEN NVL(DADOSFILIAL_O.N_CUSTOREALSEMST_O, 0)
			  WHEN ''T'' THEN NVL(DADOSFILIAL_O.N_VLULTENTCONTSEMST_O, 0)
			  WHEN ''O'' THEN NVL(DADOSFILIAL_O.N_CUSTOFORNEC_O, 0)
			  ELSE DADOSFILIAL_O.N_CUSTOULTENT_O
			END PRECO_TRANSF_O 
		  , DADOSFILIAL_O.NUMPED
		  , 0 QTPED
		  , 0 PRECOPED
		  , '''' OBSERVACAOREJEICAO
		  , 0 NUMSUGESTAO
		  , DADOSFILIAL_O.CODPROD_O CODPROD_TRANSITO_O
		  , DADOSFILIAL_O.DESCRICAO DESCRICAO_TRANSITO_O
		  , DADOSFILIAL_O.CODCLI
		  , DADOSFILIAL_O.FANTASIA NOMEFANTASIACLIENTE
		  , 0 QTFALTAPED
		  , '''' INTEGRADORA
		  , '''' NOMEINTEGRADORA
		  , 0 QTOPERLOG_O
		  ...
		  , '''' NOMEINTEGRADORAESPELHONF
		  , '''' INFOQUEBRAS
		  , DADOSFILIAL_O.CODFILIAL_O CODFILIALRETIRA_O
		  , DADOSFILIAL_O.ESTMIN_O
		  , DADOSFILIAL_O.ESTMIN_O ESTOQUEMIN_O
		  , DADOSFILIAL_O.ESTMIN_O ESTOQUEMINSUG_O
		  , '''' FORMULA_QT_SUGERIDA_D
		  , DADOSFILIAL_O.TIPOCUSTOTRANSF
		  , 0 QTCANCESTOQUEMINSUG_O
		  , '''' FORMULA 
		FROM (
		SELECT 
		  ...
		  , PRO.UNIDADE
		  , PRO.CODFAB CODFAB_O
		  , PRO.CODAUXILIAR CODAUXILIAR_O
		  , FRC.PRAZOENTREGA PRAZOENTREGA_O  
		  , NVL(EST.CUSTOREP, 0) N_CUSTOREP_O
		  , NVL(EST.CUSTOREAL, 0) N_CUSTOREAL_O
		  , NVL(EST.CUSTOCONT, 0) N_CUSTOCONT_O
		  , NVL(EST.CUSTOFIN, 0) N_CUSTOFIN_O
		  , NVL(EST.CUSTOULTENT, 0) N_CUSTOULTENT_O
		  , NVL(EST.VALORULTENT, 0) N_VALORULTENT_O
		  , NVL(EST.CUSTOREALSEMST, 0) N_CUSTOREALSEMST_O
		  , NVL(EST.VLULTENTCONTSEMST, 0) N_VLULTENTCONTSEMST_O
		  , NVL(EST.CUSTOFORNEC, 0) N_CUSTOFORNEC_O          
		  , RCM.NUMPED
		  , FIL.CODCLI
		  , FIL.FANTASIA
		  , NVL(EST.ESTMIN, 0) ESTMIN_O
		  , PRO.TIPOCUSTOTRANSF
		...
		WHERE (RCM.CODROTA            = RCC.CODROTA(+))
		  ...
		  AND (EST.CODFILIAL          = RCC.CODFILIALDESTINO)
		  AND (FIL.CODFORNEC          = FRC.CODFORNEC)
		  AND (FIL.CODIGO             = EST.CODFILIAL)
		  AND (RCM.DTESTORNOENTRADA IS NULL)
		...
		WHERE (DADOSFILIAL_O.CODPROD_O = PRO_D.CODPROD(+))
		  AND (DADOSFILIAL_O.CODPROD_O = EST_D.CODPROD(+))
		  AND (EST_D.CODFILIAL(+)      = DADOSFILIAL_O.CODFILIAL_D)
		  AND (FIL_D.CODIGO(+)         = DADOSFILIAL_O.CODFILIAL_D)  
		  AND (CLI_D.CODCLI(+)         = FIL_D.CODCLI)
		  AND (PRC_D.CODPRACA(+)       = CLI_D.CODPRACA)
		  AND (REG_D.NUMREGIAO(+)      = PRC_D.NUMREGIAO)
    ...
    Se Geração da Reposição de forma Automática ou Manual
    *****************************************************/
    ELSIF (NVL(P_AUTOMATICO_MANUAL,'M') IN ('A','M')) THEN
