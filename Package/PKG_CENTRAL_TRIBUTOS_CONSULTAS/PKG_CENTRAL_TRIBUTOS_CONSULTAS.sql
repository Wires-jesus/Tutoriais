CREATE OR REPLACE PACKAGE PKG_CENTRAL_TRIBUTOS_CONSULTAS AS
  ...
  CURSOR C_DADOS_NF_SAIDA_PREFAT(P_CODFILIAL IN VARCHAR2,
    ...
    (SELECT NVL(N.CODFILIALNF,N.CODFILIAL) CODFILIAL,
            ...
            CLIENTE.TIPOEMPRESA,
            CLIENTE.SULFRAMA,
            CLIENTE.DTVENCSUFRAMA,   
            NVL(CLIENTE.CONSUMIDORFINAL, 'N') CONSUMIDORFINAL,
            NVL(CLIENTE.ORGAOPUB, 'N') ORGAOPUB,
            NVL(CLIENTE.ORGAOPUBFEDERAL, 'N') ORGAOPUBFEDERAL,
            NVL(N.UF, CLIENTE.ESTENT) AS ESTENT,
            M.ROWID AS ID_PCMOV,
            'S' AS TIPOMOV,            
            'S' AS PREFAT, 
            FI.UF AS UFORIGEM,
  ...
  CURSOR C_DADOS_NF_ENTRADA(P_CODFILIAL IN VARCHAR2,
    ...
    (SELECT NVL(N.CODFILIALNF,N.CODFILIAL) CODFILIAL,
            ...
            M.CODFISCAL,
            M.NBM NCM, 
						CASE WHEN N.TIPODESCARGA IN ('N','F') THEN
							M.PUNIT
						ELSE
							M.PUNITCONT
						END PUNITCONT,  
            ...
            0 VLVII, 
						CASE WHEN N.TIPODESCARGA IN ('N','F') THEN
								DECODE(NVL(MC.VLPISCALCDI,0),0, NVL(M.VLCREDPIS,0),NVL(MC.VLPISCALCDI,0)) 
						 ELSE
								DECODE(NVL(M.VLCREDPIS,0),0,NVL(M.VLPIS,0),NVL(M.VLCREDPIS,0))
						END VLPIS,
						CASE WHEN N.TIPODESCARGA IN ('N','F') THEN
								DECODE(NVL(MC.VLCOFINSCALCDI,0),0,NVL(M.VLCREDCOFINS,0),NVL(MC.VLCOFINSCALCDI,0))
						ELSE
								DECODE(NVL(M.VLCREDCOFINS,0),0,NVL(M.VLCOFINS,0),NVL(M.VLCREDCOFINS,0))  
						END VLCOFINS,
            -- VLICMS --
            CASE WHEN ( NVL(PARAMFILIAL.OBTERCOMOVARCHAR2('UTILIZAPRECOPERSNFIMP', NVL(N.CODFILIALNF,N.CODFILIAL)),'N') = 'S') 
                       AND (N.TIPODESCARGA = 'F') 
                 THEN CASE WHEN NVL(PARAMFILIAL.OBTERCOMOVARCHAR2('PRECOPERSNFIMP_ICMS', NVL(N.CODFILIALNF,N.CODFILIAL)),'N') = 'N' 
                           THEN (NVL(M.BASEICMS, 0) + NVL(MC.VLBASEFRETE,0) + NVL(MC.VLBASEOUTROS,0)) *
                               (NVL(NVL(M.PERCICMCP, M.PERCICM), 0) / 100) -
                               (NVL(M.BASEICMS, 0) + NVL(MC.VLBASEFRETE,0) + NVL(MC.VLBASEOUTROS,0)) *
                                    (NVL(NVL(M.PERCICMCP, M.PERCICM), 0) / 100) *
                                    (NVL(M.PERCDESCICMSDIF, NVL(MC.PERDIFEREIMENTOICMS,0)) / 100)
                      ELSE 0 END
            ELSE (NVL(M.BASEICMS, 0) + NVL(MC.VLBASEFRETE,0) + NVL(MC.VLBASEOUTROS,0)) *
                      (NVL(NVL(M.PERCICMCP, M.PERCICM), 0) / 100) - 
                 (NVL(M.BASEICMS, 0) + NVL(MC.VLBASEFRETE,0) + NVL(MC.VLBASEOUTROS,0)) *
                           (NVL(NVL(M.PERCICMCP, M.PERCICM), 0) / 100) *
                           (NVL(M.PERCDESCICMSDIF, NVL(MC.PERDIFEREIMENTOICMS,0)) / 100)
