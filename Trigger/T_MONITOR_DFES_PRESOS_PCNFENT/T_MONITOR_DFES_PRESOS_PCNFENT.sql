CREATE OR REPLACE TRIGGER T_MONITOR_DFES_PRESOS_PCNFENT
  BEFORE UPDATE OF ESPECIE, SITUACAONFE ON PCNFENT
...
BEGIN
  ...
  IF (VTIPODOC IS NOT NULL AND (VSITUACAO IN (103, 104, 1) OR (VSITUACAO = 1001 AND VREJEITADOSEFAZ = 'N'))) THEN
     ...
     END IF;
  ELSIF (VSITUACAO IN (100, 150)) THEN
