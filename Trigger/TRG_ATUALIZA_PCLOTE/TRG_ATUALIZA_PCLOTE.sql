    ...
    IF VSESTOQUEPORLOTE = 'S' THEN
      ...
      BEGIN
        SELECT NVL(PCPARAMFILIAL.VALOR, 'N')
          ...
          FROM PCPARAMFILIAL
         WHERE PCPARAMFILIAL.CODFILIAL = NVL(:OLD.CODFILIAL, :NEW.CODFILIAL)
      ...
      IF SUBSTR(NVL(:NEW.CODOPER, :OLD.CODOPER), 1, 1) <> 'E' AND NVL(:NEW.ROTINACAD, VSPROGRAMA) NOT LIKE '%1124%' THEN
        ...
        IF VICONTADOR > 0 THEN
          SELECT NVL(PCPEDC.CONDVENDA, 0) CONDVENDA,
                 PCPEDC.POSICAO, 
