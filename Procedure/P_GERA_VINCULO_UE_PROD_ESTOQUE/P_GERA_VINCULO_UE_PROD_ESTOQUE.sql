CREATE OR REPLACE PROCEDURE P_GERA_VINCULO_UE_PROD_ESTOQUE(PCODFILIAL IN VARCHAR2,
   ...
   ------------------------------------------------------------------------
      ...
      -----------------------------------------------------------------------------------------------------
      /*CALCULANDO A MÉDIA DO ICMS ST*/
      IF (V_VL_TOTAL_BASEST > 0) AND
         (V_VL_TOTAL_ST > 0) THEN
        V_VL_MEDIA_BASEST := (V_VL_TOTAL_BASEST / PQTESTOQUE);
        V_VL_MEDIA_ST     := (V_VL_TOTAL_ST / PQTESTOQUE);
      ELSIF (V_VL_TOTAL_BASESTFORANF > 0) AND
            (V_VL_TOTAL_STFORANF > 0) THEN
        V_VL_MEDIA_BASEST := (V_VL_TOTAL_BASESTFORANF / PQTESTOQUE);
        V_VL_MEDIA_ST     := (V_VL_TOTAL_STFORANF / PQTESTOQUE);
      ELSE
        V_VL_MEDIA_BASEST := (V_VL_TOTAL_BASESTBCR / PQTESTOQUE);
        V_VL_MEDIA_ST     := (V_VL_TOTAL_STSTBCR / PQTESTOQUE);
      END IF;
