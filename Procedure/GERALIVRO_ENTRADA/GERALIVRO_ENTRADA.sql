           ...
           CASE
            WHEN (A.ROTINACAD NOT LIKE '%1423%') AND
               ...
               THEN SUM(ROUND(
                             ...
                             ,2))
            ELSE 
              SUM(CASE WHEN NVL(A.GERANFDEVCLI,'N') = 'N' AND XML.NUMTRANSITEM  IS NULL THEN
                     ROUND(  B.QTCONT * (NVL(B.PUNITCONT,0) - NVL(B.VLIPI,0) - NVL(B.ST,0) - NVL(MC.VLFECP,0)) 
                           + NVL(B.QTCONT, 0) * NVL(B.ST, 0)
                           + NVL(B.QTCONT, 0) * NVL(B.VLIPI, 0)
                           + NVL(B.QTCONT, 0) * NVL(B.VLOUTROS, 0)
                           + NVL(B.QTCONT, 0) * NVL(MC.VLFECP, 0)
                           -- Não incluir VLFRETE já incluso no GERAR_DESPESA_FRETE_NFE se Devolução de Cliente
                         , 2) 
              ELSE
...
--------------------------------------------------------------------------------
    ...
    ----------------------------------------------------------------
           ,(SELECT sum(ROUND( DECODE( NVL(M.GRAVADOUNITARIO,'S'), 'S', M.QTCONT, 1) * M.VLDIFALIQUOTA, 2))
               FROM PCMOVCIAP M
    ...
    ----------------------------------------------------------------
           ,(SELECT sum(ROUND( DECODE( NVL(M.GRAVADOUNITARIO,'S'), 'S', M.QTCONT, 1) * M.VLCREDITO, 2))
               FROM PCMOVCIAP M
              WHERE M.NUMTRANSENT = A.NUMTRANSENT
                AND M.NUMNOTA = A.NUMNOTA
                AND M.SITTRIBUT = B.SITTRIBUT
      ...
      IF V_VALIDA_NF_CONTABILIZADA = 'S' THEN
        ...
        ELSE
           ...
           IF ((V_NUMNOTA <> V_LISTA_NOTAS(I).NUMNOTA) OR 
              (V_NUMTRANSENT <> V_LISTA_NOTAS(I).NUMTRANSENT)) THEN
                DELETE FROM PCNFBASEENT
                    WHERE DTENTRADA BETWEEN V_LISTA_NOTAS(I).DTENT AND V_LISTA_NOTAS(I).DTENT
                        ...
                        AND CODCONT = DECODE(V_LISTA_NOTAS(I).NUMSQL,6,CODCONT,V_LISTA_NOTAS(I).CODCONT);
            END IF;
           V_NF_CONTABILIZADA:= 0;
        END IF;
        
        V_NUMNOTA := V_LISTA_NOTAS(I).NUMNOTA;
        V_NUMTRANSENT := V_LISTA_NOTAS(I).NUMTRANSENT;
      END IF;
