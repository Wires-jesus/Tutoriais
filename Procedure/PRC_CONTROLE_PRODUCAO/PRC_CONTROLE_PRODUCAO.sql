CREATE OR REPLACE PROCEDURE PRC_CONTROLE_PRODUCAO(PCODFILIAL                     in varchar2,
                                                  ...
                                                  PSOMENTEENTRADAS               in varchar2 default 'N') IS
  ---------------------------------------------------------------------------------
...
BEGIN
  BEGIN
    SELECT COUNT(*) INTO vnCONTADOR FROM PCDADOS1070_TEMP;
  EXCEPTION
    WHEN OTHERS THEN
      vnCONTADOR := 0;
  END;
  ...
  V_DATA_CANCELAMENTO_FIM := TO_DATE(TO_CHAR(PDTFIM, 'DD/MM/YYYY') ||
                                     ' 23:59:59',
    ...
    SELECT 'E' TIPO,
           ...
           CASE
             WHEN PCMOV.CODOPER IN ('ED') THEN
              DECODE(NVL(PCMOVCOMPLE.CUSTOULTENTCONT, 0),
                     0,
                     PCMOV.CUSTOCONT,
             ...
             ELSE
              NVL(PCMOVCOMPLE.CUSTOULTENTCONT,
                  ROUND(((((GREATEST(NVL(PCMOV.QTESTANT, 0), 0) +
                        ...
                        NVL(PCMOV.CUSTOCONTANT, 0))) / PCMOV.QTCONT),
                        6))
           END BASECUSTOCONT,
           --------------------------------------------------------------------------------
           PCMOV.CUSTOCONT,
           ...
           CASE
             WHEN PORDENAR_PCMOVLOG = 'N' THEN
              PCMOV.HORALANC
             WHEN PORDENAR_PCMOVLOG = 'S' AND
                  PCMOV.DTMOV = TRUNC(PCMOV.DTMOVLOG) THEN
           ...
           ROUND(PCMOV.PUNITCONT -
                 ...
                 PNUMCASAS_UNIT) VALORITEMNOTA_ENT,
          ...
          ,
           ...
           (DECODE(NVL(PCMOVCOMPLE.VLICMSCUSTOFRETEFOB, 0),
                    0,
                    NVL(PCMOVCOMPLE.VLICMSFRETEFOB, 0),
                    NVL(PCMOVCOMPLE.VLICMSCUSTOFRETEFOB, 0)) +
           NVL(PCMOVCOMPLE.VLCREDPISFRETEFOB, 0) +
           NVL(PCMOVCOMPLE.VLCREDCOFINSFRETEFOB, 0))) VLFRETEFOB_LIQ,
           NVL(ROUND(((NVL(PCMOVCOMPLE.PTABELACUSTO,
                           DECODE(PCMOV.PTABELA,
                                  0,
                                  PCMOV.PUNITCONT,
                                  PCMOV.PTABELA)) -
                     NVL(PCMOVCOMPLE.VLDESCONTOCUSTO,
                           NVL(PCMOV.VLDESCONTO, 0))) +
                     ...
                     NVL(PCMOVCOMPLE.VLDESPDENTRONFCUSTO,
                          NVL(PCMOV.VLDESPDENTRONF,
                              CASE
                                WHEN (NVL(PCMOV.VLOUTRASDESP, 0) - NVL(PCMOV.VLSEGURO, 0)) > 0 THEN
                                 (NVL(PCMOV.VLOUTRASDESP, 0) - NVL(PCMOV.VLSEGURO, 0))
                                ELSE
                                 0
                              END)) + NVL(PCMOV.VLIMPORTACAO, 0) +
                     NVL(PCMOV.ST, 0) +
                     NVL(PCMOVCOMPLE.VLSISCOMEXCUSTO,
                          NVL(PCMOV.VLSISCOMEX, 0)) +
                     NVL(PCMOVCOMPLE.VLAFRMMCUSTO, 0) +
                     NVL(PCMOVCOMPLE.VLADUANEIRACUSTO,
                          NVL(PCMOV.VLADUANEIRA, 0)) +
                     NVL(PCMOVCOMPLE.VLOUTRASDESPIMPCUSTO,
                       ...
                       WHEN PCNFENT.TIPODESCARGA IN ('N', 'I') THEN
                        ...
                        END) + (CASE
                          WHEN (NVL(PCMOVCOMPLE.VLCOFINSCALCDI, 0) > 0) THEN
                           (NVL(PCMOVCOMPLE.VLCOFINSCALCDI, 0) -
                           NVL(PCMOV.VLCREDCOFINS, 0))
                          ELSE
                           0
                        END)
                       ELSE
                        0
                     END + NVL(PCMOVCOMPLE.VLOUTROSCUSTOSCUSTO,
                                NVL(PCMOV.VLDESPDENTRONF,
                                    CASE
                                      WHEN (NVL(PCMOV.VLOUTRASDESP, 0) - NVL(PCMOV.VLSEGURO, 0)) > 0 THEN
                                       (NVL(PCMOV.VLOUTRASDESP, 0) - NVL(PCMOV.VLSEGURO, 0))
                                    ...
                                    END)) +
                     (NVL(PCMOV.VLFRETECONHEC, 0) -
                     (NVL(VLICMSCUSTOFRETEFOB, 0) +
                     NVL(PCMOVCOMPLE.VLCREDPISFRETEFOB, 0) +
                     NVL(PCMOVCOMPLE.VLCREDCOFINSFRETEFOB, 0)))),
                     6),
          ...
          ,
           ...
           PCMOV.NUMTRANSITEM,
           PCMOV.STATUS,
           NVL(PCMOV.CUSTOULTENTFIN, 0) AS CUSTOULTENTFIN,
           NVL(PCMOVCOMPLE.CUSTOULTENTSEMST, 0) AS CUSTOULTENTSEMST,
           NVL(PCMOVCOMPLE.CUSTOULTENTFINSEMST, 0) AS CUSTOULTENTFINSEMST,
           NVL(PCMOVCOMPLE.CUSTOULTENTLIQ, 0) AS CUSTOULTENTLIQ,
           NVL(PCMOVCOMPLE.CUSTOFINSEMST, 0) AS CUSTOFINSEMST,
           NVL(PCMOVCOMPLE.CUSTOREALLIQ, 0) AS CUSTOREALLIQ
     ...
     WHERE PCMOV.NUMNOTA = PCNFENT.NUMNOTA
       ...
       AND DECODE(PATIVIOMOBULIZADO,
                  'N',
                  NVL(PCMOV.TIPOMERCDEPTO, 'X'),
                  'XX') <> 'IM'
       AND ((PDESCONSIDERANFEDENEGADA = 'N') OR
           ((PDESCONSIDERANFEDENEGADA = 'S') AND
           NVL(PCNFENT.SITUACAONFE, '0') NOT IN (110, 205, 301, 302)))
       AND PCMOV.CODPROD BETWEEN PCODPROD1 AND PCODPROD2
       AND DECODE(PDESCONS_ITEM_BRINDE,
                  'S',
                  NVL(PCMOV.TIPOMERC, 'XX'),
                  'XX') <> 'BD'
       AND PCMOV.CODOPER NOT in ('EP', 'EA', 'EX')
       AND NVL(PCMOV.MOVESTOQUECONTABIL, 'S') = 'S'
       AND (CASE
             WHEN PSTATUSPROD = 'T' THEN
              'S'
             WHEN PSTATUSPROD = 'A' AND (PCPRODUT.DTEXCLUSAO IS NULL OR
                  PCPRODUT.DTEXCLUSAO > PDTFIM) THEN
              'S'
             WHEN PSTATUSPROD = 'I' AND (PCPRODUT.DTEXCLUSAO IS NOT NULL AND
                  PCPRODUT.DTEXCLUSAO <= PDTFIM) THEN
              'N'
             ELSE
              'N'
  ...
  */ -- GerarLogP2
  ...
  insert into PCDADOS1070_TEMP
    ...
    SELECT TAB.TIPO,
           ...
           TAB.HISTORICO,
           TAB.PUNITCONT,
           TAB.VALORITEMNOTA_ENT,
           TAB.VALORITEMNOTA_SAID,
           TAB.NUMTRANSENT,
           TAB.NUMTRANSVENDA,
           TAB.VLIPI,
           TAB.ST,
           TAB.STGUIA,
           TAB.DTCANCEL,
           TAB.SITUACAOTRIBUTARIA,
           TAB.TIPODESCARGA,
           TAB.MINUTOLANC,
           TAB.CUSTOCONTANT,
           TAB.ROTINACAD,
           TAB.VLFRETEFOB_LIQ,
           TAB.CUSTO_SEM_IPI_PIS_COFINS,
           TAB.ID_PCMOV,
           TAB.ID_PCMOVCOMPLE,
           TAB.NUMTRANSORIGEM,
           TAB.VLR_CRED_ICMS,
           ...
           TAB.NCM,
           TAB.CUSTOFISCAL,
           TAB.CUSTOULTENTFISCAL,
           TAB.NUMTRANSITEM,
           TAB.STATUS,
           TAB.CUSTOULTENTFIN,
           TAB.CUSTOULTENTSEMST,
           TAB.CUSTOULTENTFINSEMST,
           TAB.CUSTOULTENTLIQ,
           TAB.CUSTOFINSEMST,
           TAB.CUSTOREALLIQ
      FROM (SELECT 'EC' TIPO, --- NF ENTRADA CANCELADA
                   PCMOV.SEQMOV,
                   --- Verificando devolução -----------------------------------------------------
                   CASE
                     WHEN PCMOV.CODOPER IN ('ED') THEN
                     ...
                     ELSE
                      (SELECT NVL(C.CUSTOULTENTCONT,
                                  ROUND(((((GREATEST(NVL(V.QTESTANT, 0), 0) +
                                        ...
                                        NVL(V.CUSTOCONTANT, 0))) / V.QTCONT),
                                        6))
                         FROM PCMOV V, PCNFENT E, PCMOVCOMPLE C
                        WHERE V.NUMTRANSENT = E.NUMTRANSENT
                          AND V.NUMTRANSITEM = C.NUMTRANSITEM(+)
                          AND NVL(V.CODFILIALNF, V.CODFILIAL) =
                  ...
                  ,
                   ...
                   NVL(ROUND(((NVL(PCMOVCOMPLE.PTABELACUSTO,
                             ...
                             NVL(PCMOVCOMPLE.VLDESCONTOCUSTO,
                                   NVL(PCMOV.VLDESCONTO, 0))) +
                             NVL(PCMOVCOMPLE.VLFRETENFCUSTO,
                                  NVL(PCMOV.VLFRETE, 0)) +
                             NVL(PCMOVCOMPLE.VLSEGUROCUSTO,
                                  NVL(PCMOV.VLSEGURO, 0)) +
                             NVL(PCMOVCOMPLE.VLCAPATAZIACUSTO, 0) +
                             NVL(PCMOVCOMPLE.VLDESPDENTRONFCUSTO,
                                  NVL(PCMOV.VLDESPDENTRONF,
                                      CASE
                                        WHEN (NVL(PCMOV.VLOUTRASDESP, 0) -
                                             NVL(PCMOV.VLSEGURO, 0)) > 0 THEN
                                         (NVL(PCMOV.VLOUTRASDESP, 0) - NVL(PCMOV.VLSEGURO, 0))
                                        ELSE
                                         0
                                      END)) + NVL(PCMOV.VLIMPORTACAO, 0) +
                             ...
                             + CASE
                               WHEN PCNFENT.TIPODESCARGA IN ('N', 'I') THEN
                                (CASE
                                  WHEN (NVL(PCMOVCOMPLE.VLPISCALCDI, 0) > 0) THEN
                                   (NVL(PCMOVCOMPLE.VLPISCALCDI, 0) -
                                   NVL(PCMOV.VLCREDPIS, 0))
                                  ELSE
                                   0
                                END) + (CASE
                                  WHEN (NVL(PCMOVCOMPLE.VLCOFINSCALCDI, 0) > 0) THEN
             ...
             WHERE PCMOV.NUMNOTA = PCNFENT.NUMNOTA
               ...
               AND PCNFENT.DTENT BETWEEN TO_DATE(PDTINICIO) - 30 AND
                   TO_DATE(PDTFIM)
               ...
               AND ((PDESCONSIDERANFEDENEGADA = 'N') OR
                   ((PDESCONSIDERANFEDENEGADA = 'S') AND
                   NVL(PCNFENT.SITUACAONFE, '0') NOT IN
                   (110, 205, 301, 302)))
               AND NVL(PCMOV.MOVESTOQUECONTABIL, 'S') = 'S'
               AND (CASE
                     WHEN PSTATUSPROD = 'T' THEN
                     ...
                     WHEN PSTATUSPROD = 'A' AND (PCPRODUT.DTEXCLUSAO IS NULL OR
                          PCPRODUT.DTEXCLUSAO > PDTFIM) THEN
                      'S'
                     WHEN PSTATUSPROD = 'I' AND
                          (PCPRODUT.DTEXCLUSAO IS NOT NULL AND
                          PCPRODUT.DTEXCLUSAO <= PDTFIM) THEN
                      'N'
                     ELSE
                      'N'
  ...
  */ -- GerarLogP2
  ...
  insert into PCDADOS1070_TEMP
    ...
    SELECT 'E' TIPO,
           PCMOV.SEQMOV,
           NVL(PCMOVCOMPLE.CUSTOULTENTCONT,
               ROUND(((((GREATEST(NVL(PCMOV.QTESTANT, 0), 0) + PCMOV.QTCONT) *
  ...
  insert into PCDADOS1070_TEMP
    ...
    SELECT 'EF' TIPO,
           ...
           */ -- Conforme testes, deverá ser considerado o parametro da movimentação para manter o resultado registrado em cada lançamento. Gleibe/Fabio
           ROUND(PCMOV.VLFRETECONHEC - ((CASE
                   WHEN NVL(PCMOVCOMPLE.APROVEITACREDPISCOFINSCONT, 'N') = 'S' THEN
                    (VLCREDPISFRETEFOB + VLCREDCOFINSFRETEFOB)
                   ELSE
                    0
                 END) + (CASE
                   WHEN NVL(PCMOV.APROVEITACREDICMS, 'N') = 'S' THEN
                    PCMOVCOMPLE.VLICMSFRETEFOB
                   ELSE
                    0
                 END)),
                 6) AS BASECUSTOCONT,
  ...
  insert into PCDADOS1070_TEMP
    ...
    SELECT 'ES' TIPO,
           ...
           ROUND(PCMOVCOMPLE.VLNFSERVICO - ( -- CASE SOMANDO PIS E COFINS, CASO PARAMETRO COMO SIM.
                  (CASE
                    WHEN NVL(PCNFENT.CALCREDPISSERVICOCONT, 'N') = 'S' THEN
                     (VLCREDPISFRETEFOB + VLCREDCOFINSFRETEFOB)
                    ELSE
                     0
                  END)),
           ...
           PCMOV.CODFISCAL,
           ROUND(PCMOV.QT, 2) QTCONT,
           DECODE(SUBSTR(PCMOV.CODOPER, 1, 1), 'E', ROUND(PCMOV.QT, 6), 0) QTENTRADA,
           0 QTSAIDA,
           0 QTSAIDA_DENTRO,
           0 QTSAIDA_FORA,
           DECODE(PCMOV.CODOPER,
  ...
  */ -- GerarLogP2
  ...
  insert into PCDADOS1070_TEMP
    ...
    (TIPO,
     ...
     CUSTOFINSEMST,
     CUSTOREALLIQ)
    SELECT 'ER' TIPO, -- ENTRADA DE ESTORNO REF.FATURAMENTO DE CONSIGNADO. Esse lançamento faz a contra partida da segunda nota de saída do consignado.
           PCMOV.SEQMOV,
           0 BASECUSTOCONT,
           0 CUSTOCONT,
           0 CUSTOFIN,
  ...
  */ -- GerarLogP2
  ...
  --- SAIDA ------------------------------------------------------------------------------------------
  IF NOT PSOMENTEENTRADAS = 'S' THEN
    insert into PCDADOS1070_TEMP
      ...
      SELECT 'S' TIPO,
             ...
             NVL(DECODE(SUBSTR(PCMOV.CODOPER, 1, 1),
                 ...
                 0) QTSAIDA,
             case
               when PCMOV.CODFISCAL between 6000 AND 7000 then
                0
               else
             ...
             DECODE(PCMOV.CODOPER,
                    ...
                    'OUTRAS') HISTORICO,
             NVL(ROUND(DECODE(NVL(PCMOV.BRINDE, 'N'),
                              'S',
                              PCMOV.PBONIFIC,
                              PCMOV.PUNITCONT),
                       PNUMCASAS_UNIT),
                 0) PUNITCONT,
             0 VALORITEMNOTA_ENT,
             ROUND(DECODE(NVL(PCMOV.BRINDE, 'N'),
                          'S',
                          PCMOV.PBONIFIC,
                          PCMOV.PUNITCONT) -
                   DECODE(PCMOV.CODOPER, 'SD', NVL(PCMOV.VLFRETE, 0), 0),
                   PNUMCASAS_UNIT) VALORITEMNOTA_SAID,
             0 NUMTRANSENT,
             PCMOV.NUMTRANSVENDA,
             NVL(PCMOV.VLIPI, 0) VLIPI,
             NVL(PCMOV.ST, 0) ST,
       ...
       WHERE PCMOV.NUMNOTA = PCNFSAID.NUMNOTA
         ...
         AND PCMOV.STATUS in ('A', 'AB')
         AND NVL(PCNFSAID.CONDVENDA, 0) not in
             (3,
              6,
              DECODE(PCNFSAID.FORNECENTREGA, 'S', -1, 7),
              12,
              DECODE(PVENDAMANIF_COMTV14, 'S', 13, 14))
            --AND NVL(PCMOV.MOVESTOQUECONTABIL,'S') = 'S'
         AND ((NVL(PCMOV.MOVESTOQUECONTABIL, 'S') = 'S') OR
             (DECODE(PVENDAMANIF_COMTV14, 'S', 'S', 'N') = 'S'))
         ...
         AND ((PDESCONSIDERANFEDENEGADA = 'N') OR
             ((PDESCONSIDERANFEDENEGADA = 'S') AND
             NVL(PCNFSAID.SITUACAONFE, '0') NOT IN (110, 205, 301, 302)))
         AND PCMOV.CODPROD BETWEEN PCODPROD1 AND PCODPROD2
         AND CASE
               WHEN PCMOV.CODOPER = 'SD' THEN
                PCNFSAID.NUMTRANSVENDA
             END NOT IN
             (SELECT F.NUMTRANSVENDA
         ...
         AND DECODE(PDESCONS_ITEM_BRINDE,
                    'S',
                    NVL(PCNFSAID.NFBRINDE, 'N'),
                    'N') <> 'S'
         AND (CASE
               WHEN PSTATUSPROD = 'T' THEN
                'S'
               WHEN PSTATUSPROD = 'A' AND
                    (PCPRODUT.DTEXCLUSAO IS NULL OR PCPRODUT.DTEXCLUSAO > PDTFIM) THEN
                'S'
               WHEN PSTATUSPROD = 'I' AND
                    (PCPRODUT.DTEXCLUSAO IS NOT NULL AND PCPRODUT.DTEXCLUSAO <= PDTFIM) THEN
                'N'
               ELSE
                'N'
             END = 'S');
  ...
  IF NOT PSOMENTEENTRADAS = 'S' THEN
    insert into PCDADOS1070_TEMP
      ...
      (TIPO,
       ...
       CUSTOFINSEMST,
       CUSTOREALLIQ)
      SELECT 'SC' TIPO, -- Notas fiscais canceladas
             PCMOV.SEQMOV,
             0 BASECUSTOCONT,
             PCMOV.CUSTOCONT,
             PCMOV.CUSTOFIN,
       ...
       WHERE PCMOV.NUMNOTA = PCNFSAID.NUMNOTA
         ...
         AND PCNFSAID.DTCANCEL BETWEEN PDTINICIO AND
             V_DATA_CANCELAMENTO_FIM
         AND NVL(PCNFSAID.CODFILIALNF, PCNFSAID.CODFILIAL) = PCODFILIAL
         AND NVL(PCMOV.CODFILIALNF, PCMOV.CODFILIAL) = PCODFILIAL
         AND PCMOV.QTCONT < 0
         AND NVL(PCNFSAID.CONDVENDA, 0) not in
  ...
  */ -- GerarLogP2
  ...
  insert into PCDADOS1070_TEMP
    ...
    SELECT 'AE' TIPO,
           ...
           ------------------------------------------------
           CASE
             WHEN PCMOV.CODOPER LIKE 'E%' THEN
  ...
  */ -- GerarLogP2
  ...
  insert into PCDADOS1070_TEMP
    ...
    (TIPO,
     ...
     CUSTOFINSEMST,
     CUSTOREALLIQ)
    SELECT 'AC' TIPO, ---- Entrada produção cancelada
           PCMOV.SEQMOV,
           (CASE
             WHEN PCMOV.CODOPER = 'EP' THEN
              NVL(PCMOV.CUSTOULTENT, 0)
           ...
           DECODE(PCMOV.CODOPER,
                  'EA',
                  'ENT.AJUSTE - CANCELADA',
                  'EP',
                  'ENT.PRODUÇÃO - CANCELADA',
                  'EX',
                  'DEV.AVULSA PRODUÇÃO - CANCELADA',
                  'RA',
                  'REQ. AVULSA PRODUÇÃO',
                  'SA',
                  'SAÍDA AJUSTE - CANCELADA',
                  'SP',
                  'SAÍDA DE PRODUÇÃO - CANCELADA',
                  'OUTRAS DE PRODUÇÃO - CANCELADA') OBSERVACAO,
           PCMOV.CODPROD,
           PCMOV.CODOPER,
           ------------------------------------------------
           CASE
     ...
     WHERE PCMOV.NUMTRANSITEM = PCMOVCOMPLE.NUMTRANSITEM(+)
       ...
       AND DECODE(PATIVIOMOBULIZADO,
                  ...
                  'XX') <> 'IM'
       AND not exists
     (SELECT 1 FROM PCNFSAID WHERE NUMTRANSVENDA = PCMOV.NUMTRANSVENDA)
       AND PCMOV.STATUS in ('A', 'AB')
  ...
  IF NOT PSOMENTEENTRADAS = 'S' THEN
    insert into PCDADOS1070_TEMP
      ...
      (TIPO,
       ...
       CUSTOFINSEMST,
       CUSTOREALLIQ)
      SELECT 'SM' TIPO, -- Requisição CANCELADA
             PCMOV.SEQMOV,
             0 BASECUSTOCONT,
             PCMOV.CUSTOCONT,
             PCMOV.CUSTOFIN,
