    ...
    select '01' NUMSQL, 
           ...
           (CASE WHEN (vPARAM_GERARICMSLIVFISCFOP = 'S') AND
               ...
               SUM(ROUND(DECODE(V_GERABASENORMALQUANDOST, 'N',
                                DECODE(NVL(B.ST,0), 0, DECODE(NVL(DXML.VBC,0),0,DECODE(NVL(A.CHAVENFE,'X'),'X',(B.QTCONT * NVL(B.BASEICMS,0)), ((B.QTCONT * NVL(B.BASEICMS,0)) + (B.QTCONT * NVL(MC.VLBASEFRETE,0)) + (B.QTCONT * NVL(MC.VLBASEOUTROS,0)))),NVL(DXML.VBC,0)),
                                   DECODE(C.ESTENT, V_UFFILIAL,
                                          DECODE(NVL(DXML.VBC,0),0,DECODE(NVL(A.CHAVENFE,'X'),'X',(B.QTCONT * NVL(B.BASEICMS,0)), ((B.QTCONT * NVL(B.BASEICMS,0)) + (B.QTCONT * NVL(MC.VLBASEFRETE,0)) + (B.QTCONT * NVL(MC.VLBASEOUTROS,0)))),NVL(DXML.VBC,0)), 0)),
                                          DECODE(NVL(DXML.VBC,0),0,DECODE(NVL(A.CHAVENFE,'X'),'X',(B.QTCONT * NVL(B.BASEICMS,0)), ((B.QTCONT * NVL(B.BASEICMS,0)) + (B.QTCONT * NVL(MC.VLBASEFRETE,0)) + (B.QTCONT * NVL(MC.VLBASEOUTROS,0)))),NVL(DXML.VBC,0))),2))
            ELSE
           ...
           (CASE WHEN (vPARAM_GERARICMSLIVFISCFOP = 'S') AND
                      (B.CODFISCAL IN (5929, 6929)) THEN
               SUM(ROUND(DECODE(V_GERABASENORMALQUANDOST, 'N',
                                DECODE(NVL(B.ST,0), 0, DECODE(NVL(DXML.VBC,0),0,DECODE(NVL(A.CHAVENFE,'X'),'X',(B.QTCONT * NVL(B.BASEICMS,0)), (ROUND(B.QTCONT * NVL(B.BASEICMS,0),2) + ROUND(B.QTCONT * NVL(MC.VLBASEFRETE,0),2) + ROUND(B.QTCONT * NVL(MC.VLBASEOUTROS,0),2))),NVL(DXML.VBC,0)),
                                DECODE(C.ESTENT, V_UFFILIAL,
                                       DECODE(NVL(DXML.VBC,0),0,DECODE(NVL(A.CHAVENFE,'X'),'X', (B.QTCONT * NVL(B.BASEICMS,0)), (ROUND(B.QTCONT * NVL(B.BASEICMS,0),2) + ROUND(B.QTCONT * NVL(MC.VLBASEFRETE,0),2) + ROUND(B.QTCONT * NVL(MC.VLBASEOUTROS,0),2))),NVL(DXML.VBC,0)),0)),
                                       DECODE(NVL(DXML.VBC,0),0,DECODE(NVL(A.CHAVENFE,'X'),'X', (B.QTCONT * NVL(B.BASEICMS,0)), (ROUND(B.QTCONT * NVL(B.BASEICMS,0),2) + ROUND(B.QTCONT * NVL(MC.VLBASEFRETE,0),2) + ROUND(B.QTCONT * NVL(MC.VLBASEOUTROS,0),2))),NVL(DXML.VBC,0))),2))
            ELSE
               (CASE WHEN (NVL(MC.PERDIFEREIMENTOICMS, 0) = 100) THEN
                   0
                ELSE
           ...
           (CASE WHEN (vPARAM_GERARICMSLIVFISCFOP = 'S') AND
                      (B.CODFISCAL IN (5929, 6929)) THEN
               SUM(DECODE(NVL(DXML.VICMS,0),0,FISCAL.GET_DADOS_ICMS(P_CODFILIAL, 'V', 'NF', B.ROWID, C.ESTENT, A.CHAVENFE,'S',DECODE(A.FINALIDADENFE,'C','S','N')),NVL(DXML.VICMS,0)))
            ELSE
               SUM( DECODE( NVL(B.GERAICMSLIVROFISCAL,'S'), 'S', DECODE(NVL(DXML.VICMS, 0), 0, FISCAL.GET_DADOS_ICMS(P_CODFILIAL, 'V', 'NF', B.ROWID, C.ESTENT, A.CHAVENFE,'S',DECODE(A.FINALIDADENFE,'C','S','N')), NVL(DXML.VICMS,0)),0))     
            END) VLICMS,
           ------------------------------------------------------------------
           --Cria? de par?tro na 132 (GERARICMSLIVFISCFOP) - FIS-8312
           (CASE WHEN (vPARAM_GERARICMSLIVFISCFOP = 'S') AND
           ...
           SUM(NVL(DXML.VPROD, 
               ROUND(B.QTCONT * (NVL(B.PUNITCONT,0) - NVL(B.ST,0) - NVL(B.VLIPI,0) - NVL(MC.VLIPIDEVFORNEC,0)-
                    ...
                    DECODE(NVL(MC.PRECOUTILIZADONFE, 
                             NVL(NVL(DECODE(NVL(PF.PRECOUTILIZADONFE,'N'), 'N', '', PF.PRECOUTILIZADONFE), 
                                     C.PRECOUTILIZADONFE), 
                                 NVL(PARAMFILIAL.OBTERCOMOVARCHAR2('PRECOUTILIZADONFE', F.CODIGO), 'L'))), 'B', 
                             ROUND((NVL(B.QTCONT,0) * NVL(B.VLDESCONTO,0)), 2), 0)                           
                 )) VL_PRODUTO
      from PCNFSAID A,
           PCMOV  B,
           PCMOVCOMPLE   MC,
                                       ...
                                       'B', 
                                        ROUND(CASE WHEN (NVL(B.VLDESCONTO,0) > 0) THEN NVL(B.VLDESCONTO, 0) ELSE 
                                               CASE WHEN (NVL(B.PERCDESC,0) > 0) THEN 
                                                 ROUND((B.PTABELA * (NVL(B.PERCDESC,0) / 100)),2) ELSE 0  
                                               END 
                                             END, 2), 
                                     0)    
                             - NVL(B.ST, 0)
...
GROUP BY   TAB.NUMSQL,
    ...
    select '04' NUMSQL, 
     ...
     AND B.DTMOV BETWEEN P_DATA1 AND P_DATA2
       ...
       and COALESCE(A.CODFILIALNF, A.CODFILIAL) = P_CODFILIAL
       and COALESCE(B.CODFILIALNF, B.CODFILIAL) = P_CODFILIAL
  ...
  BEGIN
     ...
     where A.NUMTRANSVENDA = B.NUMTRANSVENDA
       ...
       and B.QTCONT > 0
       and A.ESPECIE in ('NF', 'CO', 'CF', 'CP')
       and COALESCE(A.CODFILIALNF, A.CODFILIAL) = PCODFILIAL
       and COALESCE(B.CODFILIALNF, B.CODFILIAL) = PCODFILIAL
