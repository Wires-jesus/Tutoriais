  ...
  -- 01 - NOTAS FISCAIS DE VENDA
    select '01' NUMSQL, 
               ...
               (CASE WHEN (NVL(MC.PERDIFEREIMENTOICMS, 0) = 100) THEN
                ...
                ELSE
                   CASE WHEN (vPARAM_RECALCBASEICMSDIFERIDO = 'S') AND
                     ...
                     SUM(DECODE(NVL(DXML.VBC,0),0,ROUND(B.QTCONT * (NVL(B.BASEICMS,0) * ((100 - MC.PERDIFEREIMENTOICMS)/100)),2),NVL(DXML.VBC,0)))
                   ELSE
           ...
           (CASE WHEN (vPARAM_GERARICMSLIVFISCFOP = 'S') AND
            ...
            ELSE
               SUM(DECODE(NVL(DXML.VICMS,0),0,DECODE(NVL(B.GERAICMSLIVROFISCAL,'S'), 'S', FISCAL.GET_DADOS_ICMS(P_CODFILIAL, 'V', 'NF', B.ROWID, C.ESTENT, A.CHAVENFE),0),NVL(DXML.VICMS,0)))
            END) VLICMS,
           ------------------------------------------------------------------
           --Cria? de par?tro na 132 (GERARICMSLIVFISCFOP) - FIS-8312
           (CASE WHEN (vPARAM_GERARICMSLIVFISCFOP = 'S') AND
